...
**Healthy looks like:**
- `/flights` is a year-by-year personal log (2023–2026, plus All years) of scheduled vs actual times and delays, with each year’s total on the year chips
...
- An unintelligible Krisp/mobile transcript fails at capture quality (`lib/transcript-quality.js`) and is reported as a monumental ingest error; it is not synthesised into questions, facts, or tasks
...
## Knowledge Layer
**Purpose:** The derived substrate the CRM/wiki/project pages are *views* over — not another table of hand-entered records. Compiled continuously from raw sources so connections are made from the whole corpus after ingestion, not from the thin context available at capture time. Files: `lib/retrieval.js` (embeddings via Mac Ollama), `lib/atoms.js` (atoms), `lib/atom-dedup.js` (receipt-backed semantic review decisions), `lib/synthesis.js` (the compiler called only after canonical CRM gates), `lib/task-router.js`, `lib/attribution-reconciliation.js`, `lib/knowledge-lint.js`. Jobs: `embed_backfill`, `atoms_backfill` (historical name; normally queues canonical review for CRM facts), `synthesis_run` (nightly compatibility scheduler delegating to `crm_knowledge_engine` without a second retry chain), `task_route_run` (daily), `attribution_reconciliation` (regular change-driven whole-meeting pass), `knowledge_lint_run` (weekly). Model calls use the **subscription plane** (`lib/feature-runners.js`); `embeddings`, `atom_extractor`, `entity_linker`, and the explicit `gpt-5.6-terra` attribution reconciler are admin/registry slots — not OpenRouter.

**Healthy looks like:**
- `embeddings` count tracks the corpus; changing the embeddings model causes a gradual, model-aware re-index. Historical batches re-embed canonical emails and messages (not task rows) only from 23:00–02:30 Dublin, with the last batch starting by 01:45 so the Mac remains available for daytime model work.
- Incomplete/unreadable raw evidence remains visible in admission health but does not count as runnable embedding backlog; an embedder outage respects the per-pass attempt limit
...
- Re-running atom dedup spends no model call for an unchanged pair already receipted as `same` or `different`
...
**WhatsApp / Apple messaging capture:** Hermes (Baileys) passively posts routed WhatsApp evidence and the Mac `com.mclellan.hub.messages-capture` LaunchAgent reads new Apple Messages text bubbles (received and sent) read-only; both post to `POST /api/messaging/capture` (`lib/messaging-capture.js` → `messaging_messages`). Apple capture initializes at the current provider row, never silently imports history, and skips reactions/system/attachment-only rows. The complete source envelope preserves chat/sender/direction/message provenance plus any operator-supplied routing hints. Before admission or model work, `lib/contact-identity.js` resolves a unique canonical name/alias/address into the envelope; ambiguous near names remain unlinked. Source kind `messaging_message` is consumed by `crm_knowledge_engine` like email/meetings — triage, duplicate review, atoms, high-confidence task projection. Automatic selection holds a new bubble for five minutes so a reply can land; triage/projection then see same-`chat_id` neighbours as context, not as the source. A later turn can complete an earlier same-chat task through `crm_action_resolution`. Setup: `docs/whatsapp-hermes-capture.md` and `docs/apple-messages-capture.md`. The Apple worker heartbeats to Hub; stale/error state appears in daily INGEST. Intentional `/crm` notes still use the Hermes `dchat-crm` skill → `/api/crm/webhook`; family chat bulk path must not.
...
**Contract:** `admitSource(user, sourceKind, rowOrId, { ingester })` writes a `knowledge_receipts` row at stage `source_admitted` — `done` when complete, `review` when readable but partial, `error` when nothing readable was captured (including an unintelligible meeting transcript), `skipped` when the source is a deliberate exclusion (the Hub's own report mail). A skipped receipt records `exclusion_reason`, including `hub_generated_report`; repeated standard `Re:`/`Fw:`/`Fwd:` prefixes are transport metadata and cannot make a forwarded Hub briefing fresh task-bearing evidence. Ordinary self-sent/forwarded commitments remain eligible. Idempotent per source revision; a later body/transcript backfill is a new revision and is admitted again, so a repaired source can become complete instead of being marked broken forever.
...
- A single unintelligible meeting transcript (Krisp dumping thousands of words of noise as one speaker turn) produces a `NEEDS YOU` line and a `MONUMENTAL TRANSCRIPT FAILURE` on `/crm/meeting-intake`. It must not become `/crm/questions` cards, CRM facts, or tasks
...
**Daily briefing (fixed contract):** At 05:30 Europe/Dublin, `scripts/build-newsletter-digest-briefing.js` reads the previous Dublin calendar day's received messages from the `douglasnewsletters@agentmail.to` inbox. It excludes Hub's old sent `Newsletter digest` wrapper, creates exactly one `newsletter_digest_briefing` ChatGPT Go/Codex (`gpt-5.6-luna`) job with one model attempt only, and does not scan missed dates or poll during the day. Completion files the PDF, source provenance and prompt under `data/newsletter-briefings/<edition>/`, then emails the PDF via AgentMail. The edition is coverage-date keyed (2026-08-25 = 001), so a manual repair cannot result in a second daily send. The input allowance is sized for the full previous-day editorial source packet; sponsor/tracking/housekeeping removal is done by the briefing prompt, not by silently dropping source content.
