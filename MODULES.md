...
**Healthy looks like:**
- `embeddings` count tracks the corpus; changing the embeddings model causes a gradual, model-aware re-index. Historical batches run only from 23:00–02:30 Dublin (last batch starts by 01:45) so the Mac remains available for daytime model work.
