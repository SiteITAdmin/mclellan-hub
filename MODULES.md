...
**Contract:** `admitSource(user, sourceKind, rowOrId, { ingester })` writes a `knowledge_receipts` row at stage `source_admitted` — `done` when complete, `review` when readable but partial, `error` when nothing readable was captured, `skipped` when the source is a deliberate exclusion (the Hub's own report mail). A skipped receipt records `exclusion_reason`, including `hub_generated_report`; repeated standard `Re:`/`Fw:`/`Fwd:` prefixes are transport metadata and cannot make a forwarded Hub briefing fresh task-bearing evidence. Ordinary self-sent/forwarded commitments remain eligible. Idempotent per source revision; a later body/transcript backfill is a new revision and is admitted again, so a repaired source can become complete instead of being marked broken forever.
