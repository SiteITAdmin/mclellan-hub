...
The Boox reference planner (`lib/boox-planner.js`, `boox_planner_publish` nightly at 04:50 Dublin) renders the next 90 days as one hyperlinked PDF for the 13.3" Note Max and pushes it into the Drive folder the device syncs, replacing the same Drive file each night. It is a derived view — no planner table, no new storage, no task or calendar writes — built from consecutive 28-day planner snapshots because the snapshot API is capped at 31 days. It is **read-only on purpose**: e-ink handwriting is keyed to the file it was drawn on, so a nightly rebuild cannot also be a writing surface. Writing stays in an ordinary Boox notebook and returns through the Boox → Drive ingest path (`docs/boox-drive-ingest.md`). Do not add form fields, blank note pages, or a save-my-notes path to this PDF, and do not let it create anything. See ARCHITECTURE.md → Boox reference planner.
