...
Atom review deduplication is also receipt-backed. `lib/atom-dedup.js` records both `same` and `different` semantic decisions for the exact atom-content revision in `knowledge_receipts`; an unchanged pair must never spend a second `crm_duplicate_review` call. A saved `same` decision also preserves the original canonical atom so a mechanically retired duplicate cannot outrank and retire its survivor on a later pass. `synthesis_run` is nightly compatibility work only; the change-driven `crm_knowledge_engine` job owns backlog recovery and is the only rapid retry chain. Embedding backfill counts only complete, indexable evidence as remaining work — incomplete raw captures stay visible through admission health but cannot force a one-minute corpus-rescan loop.

Successful work can still be a failure when it repeats without new evidence. `lib/work-loop-guard.js` fingerprints the bounded prompt packet without storing its contents, blocks a fourth identical atom-pair duplicate-review success within six hours before it reaches the subscription runner, and checks `synthesis_run` every 15 minutes against its nightly contract. An overactive scheduler is contained by retaining one next-window successor and retiring extra pending rows as visible failed audit records. Containment writes ordinary `hub_remediation` receipts, so the next daily report names the repair; novel high volume and action/source duplicate reviews are reported but never automatically stopped.
...
Person identity at meeting ingest is resolved before model attribution. `lib/entity-resolution.js` (`resolveMeetingEntities`, feature `entity_resolution`) runs the shared deterministic source-identity pass over spoken/transcribed names first, filling missing links and correcting a contradictory preliminary model assignment when the source name uniquely identifies a contact; only unresolved names reach a bounded fail-closed model adjudication scoped to the meeting's people. A confident model match rewrites `matched_contact` and is learned as a durable **alias** (the store is `contacts.aliases`, not a new table), so corrections stick and the next transcript is a free match; auto-learned aliases are receipted (`stage='entity_resolution'`) for a future correction pass. A bare first name is only persisted as an alias when unique across the CRM (two possible "Nick"s resolve per-meeting, never freeze). Human answers on `/crm/questions` also learn aliases (`learnAliasFromMeetingAnswer`). **Alias invariant (`aliasCollidesWithOtherContact`, enforced in `learnAlias` for every caller):** an alias is an alternate name for the *same* person (Dad = Alister); a name that already denotes a *different* contact can never be an alias of someone else — "Nick" is not an alias of Duncan Sackfield while Nick Chin exists (that is a CRM failure from a narrow read of one sentence, not a nickname). A transcript error between two known people (Alec/Alan) stays a per-transcript resolution, never a frozen alias. `scripts/repair-bad-aliases.js` removes existing colliding aliases and reverts the links they froze. Do not reintroduce exact-string-only linking, add a person-alias table, or surface a model's "not in the known CRM" warning as an answerable question once the name resolves. `scripts/repair-entity-resolution.js` re-links and corrects frozen extractions deterministically (no model calls, no corpus re-queue).

Source-participant identity is resolved even earlier, at the raw capture boundary, by `lib/contact-identity.js`. Gmail, sent Gmail, AgentMail, WhatsApp, and Apple Messages consult canonical names, `contacts.aliases`, contact email/phone fields, and active email/phone knowledge atoms **before any classifier or CRM model call**. Exact unique identity is written into the raw source context (`email_summaries.contact_id` or the messaging envelope); synthesis consumes that ID before considering names. A unique bare first name may route, and a composite observed name may combine corroborating tokens already belonging to one contact (for example `Kai` + `Mutenga`), but there is no fuzzy/substring person match. Duplicate aliases, near names, or conflicting name/address signals fail closed as ambiguous — Alan must never become Alec because a model saw the names before the identity store. `scripts/repair-source-identities.js` is report-only by default and can apply this deterministic anchor to existing raw rows without model calls or a corpus re-queue.
