...
Hub-generated briefings and governance reports are derived views, not fresh CRM evidence. `isCanonicalEvidenceExcluded` keeps them stored and searchable but outside triage and action projection. It strips repeated standard `Re:`/`Fw:`/`Fwd:` transport prefixes before matching known Hub report subjects, so forwarding an M365 Operations & Security Brief between Douglas's accounts cannot turn its Watchlist back into tasks. Do not broaden this into a blanket self-mail exclusion: ordinary sent or forwarded commitments remain canonical evidence. Admission receipts record the exclusion reason.
