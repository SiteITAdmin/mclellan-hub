...
Drag-to-move and duration-resize both run on Pointer Events (mouse + touch + pen in one path); native HTML5 drag-and-drop is not used because the blocks live inside an `overflow:auto` scroller where it fails. Tapping/clicking a task block (without dragging) opens it in `/crm/tasks/<id>`, which is the path to change its due date and clear the late marker on the next planner load (the marker is derived live from `event.date > task.due`). Dragging a task creates one ordinary Google Calendar event through `createCalendarEvent`, with private `hubSource=task_planner` / `hubTaskId=<local task id>` properties, and mirrors it into the existing `meetings` cache as occupancy only (`source='task_planner'`). `/crm/meetings` and calendar-sync-as-meetings exclude those rows — they are not CRM meetings. Dragging it again patches that event. Unscheduling or clearing the Planner checkbox deletes only the event/cache row. Scheduling windows are saved as JSON at `crm_context.task_planner_preferences`: work tasks are constrained to Monday–Friday work hours, and personal tasks to weekday evenings or configured weekend hours. The Auto-plan button is an explicit user effect: it orders selected unscheduled work by due date then priority, uses effort, skips opaque calendar events and past time, and writes only after confirmation. A due date is a target, not a hard stop: when no eligible slot remains on or before it, the planner continues into the next permitted free time and visibly marks the block late. Free events (`transparency=transparent`) do not occupy planner time and are not treated as appointments — they will not push tasks, appear as timed blocks, or print as meetings. All-day free banners stay in the all-day strip as a muted label. A timed free block that covers most of a working day (six hours or more) is rewritten on Google to a true all-day free event so the calendar appointment matches; an opaque all-day event still blocks the applicable window.

Existing task blocks are reconciled automatically every five minutes across a rolling 28-day window. Genuine opaque Calendar appointments are fixed; a colliding task block is patched forward into the next permitted free slot, and later task blocks cascade only when the moved block consumes their time. The same pass treats a future block that now sits before its `[after:]` / `[start:]` floor as a conflict — setting "Only after" on an already-scheduled task must move the block, not leave it on Monday while the meeting is Thursday. Saving that tag on `/crm/tasks/<id>` also triggers the same reflow immediately so the wait is not five minutes. This automatic path never opts in or places a previously unscheduled task. It preserves the existing Calendar event ID and `hubTaskId`, records `reason='calendar_conflict'` or `reason='dependency_floor'` in the normal `calendar_planner_effect` receipt, retains the late marker beyond the due date, and exposes the latest movement or failure on `/crm/planner` for 24 hours. The same five-minute pass calls `cleanupCompletedPlannerBlocks`, which deletes the calendar block of any task that is no longer open (`status!='needsAction'` or soft-deleted) so a finished task's time reopens on its own rather than lingering as an orphan block.

A task can depend on a meeting: set "Only after" on `/crm/tasks/<id>` (a picker of the next 14 days' timed calendar events) and the task carries `[after: cal:<eventId>]`. `resolveTaskFloor` turns that into an earliest-start floor from the linked event's live end time; `computeAutoPlan`, `computeReshuffle`, and `computeConflictReflow` never schedule or leave a still-future block before its floor, the planner blocks/inbox cards carry `data-not-before` and a 🔒 marker, a manual drag before the floor is refused client-side and again server-side in `scheduleTask` (`resolveTaskFloorRemote` via `calendar.events.get`). The floor follows the meeting if it is rescheduled and lapses once the meeting falls out of the loaded window (dependency satisfied).

A task can also carry a **start date** ("not before"): the "Start date" picker on `/crm/tasks/<id>` (and the quick-add form) writes a Google-visible `[start: YYYY-MM-DD]` tag. `resolveTaskFloor` returns the later of the `[after:]` floor and the start date, so nothing schedules a block before it. Separately, when the start date is more than `START_DEFER_LEAD_DAYS` (14) days out, the task is parked on a dedicated **"Later — Hub"** Google Tasks list (`ensureLaterList`, id cached in `crm_context.later_task_list_id`) so it stays out of Douglas's working Google list and out of the planner inbox (`getPlannerSnapshot` drops `isStartDeferred` tasks; `/crm/tasks` shows them in a "Starts later" section). It is never a separate store: the task is a normal Google task created through the single-door `createTask`, stays in the CRM dedup corpus, and keeps its `project_slug` (sync uses `COALESCE`). `desiredListForTask` routes it to/from the Later list in `createTask`/`updateTask`; `promoteDueStartTasks` (run off `google_tasks_sync` every 15 min) moves any parked task back to its home list — its project list, or the default — with the ID-stable `moveTaskToList` once the start date comes within the window, so it reappears ~2 weeks before it starts. No start-date column, no waiting table, no second promotion path.

Due and reminder fields remain source facts; dependency-aware urgency is a derived view. `applyTaskDependencyTiming` reads the compiled `meetings` cache for `[after: cal:<eventId>]` and the visible `[start:]` tag, then derives the later effective due date/deadline without changing either stored field. The task views, daily briefing, Boox planner, Planner late markers, and automatic reminder sweep consume that same timing. A previously active automatic reminder whose floor moves forward is retained as a cancelled `task-overdue-deferred` receipt and reactivated when the floor passes; the fire path independently rechecks the floor before delivery.
...
It carries its own writing surfaces: a Notes section with one page per genuine timed appointment (title, time, place, attendees resolved against `contacts` with their open-task count) and a set of numbered blank dot-grid pages, indexed and cross-linked, with a "Notes" link on every calendar row of every day page. Task blocks deliberately get no note page — a task already has a record, and a second place to write about it would compete with it.

Because it is written on, **each day gets its own Drive file**: `Hub Planner <YYYY-MM-DD>.pdf`. A day's file is created once, in the morning, and never rewritten, so handwriting synced back from the tablet cannot be clobbered by a later build. `publishBooxPlannerToDrive` looks the day's file up by name first and returns `skipped: 'already_published'` rather than replacing it; only an explicit `force` (the "Send to Drive now" button, after a confirmation naming the file) rewrites a day that already exists. Nothing is ever auto-deleted — an old file may carry notes — so the folder is an accumulating archive that the Boox → Drive ingest path can pick up later.

The window is assembled from consecutive 28-day planner snapshots because `getPlannerSnapshot` is capped at 31 days (it backs a week view and a live Calendar read); events are merged by ID and a task blocked out in a later chunk counts as scheduled. Every page has a hard content box and every list a row cap with a visible "+n more", so a busy day can never spill into an unstyled overflow page. Section pages that spill carry links to their own parts, above the list where the clipped box cannot hide them.

Layout is HTML rendered by Chrome (`page.pdf`), which preserves `#anchor` links as named PDF destinations and absolute URLs as URI actions — that is what makes the tab rail, month cells, day pages and note pages tappable in NeoReader. Verified against the generated file's annotations before the module was written; keep hrefs as plain anchors, never JS handlers. The nav reads Today · Week · Month · Tasks · Projects · People · Notes, where Week and Month land on the current week and month. Every dashboard tile on the cover links to a page listing exactly what it counts; a count with no page to land on is a bug, and the test suite asserts both directions — no dangling link, no orphaned page.

A task block on a page reads as its title alone. Lane, project, contact/company and priority are Hub routing metadata and are not printed — the row links to the task, which holds them. The only exception is "Assigned to others", where the person is the row's meaning, so the assignee stays. Appointments keep their location/kind line.

Delivery is a push, because the device pulls: `boox_planner_publish` runs nightly at 04:50 Dublin (after the synthesis jobs refresh project knowledge) into `onyx/NoteMax/Hub Planner`. Because a day's page is written once and never rebuilt, the job runs `reshufflePlannerTasks` **before** building, so blocks stranded in the past are rescued forward and are not printed stranded; the planner module itself stays read-only, and a reshuffle failure is logged without costing the PDF. The latest file's id and publish time live in `crm_context.boox_planner_drive_file` with the last error; `/crm/planner` shows both, and offers "Download PDF" (`GET /api/planner/boox-planner.pdf`) and "Send to Drive now" (`POST /api/planner/boox-planner/publish`). `BOOX_PLANNER_DRIVE_FOLDER_PATH`, `BOOX_PLANNER_NAME_PREFIX` and `BOOX_PLANNER_PUBLISH=0` configure/disable it.
...
`lib/work-loop-guard.js` is the operational backstop for a successful-but-wasteful regression. `model-transport` records only a SHA-256 fingerprint of the exact bounded prompt packet in `request_logs`; for atom-pair review (`model_key='crm_atom_duplicate_review'`), three identical successful packets in six hours open a six-hour circuit before a fourth subscription call. Action/source duplicate reviews share the runner but are not circuit-blocked because their fail-open path could create a duplicate task; they remain visible to volume reporting. The recurring `work_loop_guard` job checks every 15 minutes that the nightly `synthesis_run` has not executed more than twice in 24 hours; if it has, it keeps one successor at the next Dublin 03:00 window and retires extra pending rows without touching CRM evidence. Both actions write `hub_remediation` receipts. The daily Usage Watch also reports per-feature call/token volume, prompt concentration where fingerprints exist, scheduler cadence, and a stalled Mac subscription queue. Only exact atom-pair repeats and documented singleton cadence have automatic containment; high novel volume remains a warning.
...
fresh knowledge synthesis, while `action_revision_hash` remains anchored to the
faithful body/transcript/task content. Once `crm_action_projected` is done or in
review for that action revision, later enrichment closes automatic projection:
completed work yields audited dismissed/existing outcomes and unresolved work
stays human-gated. It does not call the projection model or task provider again. Captured messaging
bodies are immutable, so pre-action-revision legacy projection receipts are
also terminal. A genuinely changed raw action body, or a genuinely new source,
gets a new lane. Exact same-source evidence is also compared with earlier
terminal outcomes, so model paraphrase, a pipeline replay, or an embedding-model
change cannot resurrect a completed or deleted task.
...
WhatsApp/Apple messaging sources stay one bubble each. WhatsApp arrives through Hermes; Apple Messages is read from the Mac's `chat.db` by a read-only minute LaunchAgent, which initializes at the current provider row, captures new received and sent text, and resolves display labels from Contacts before posting the same raw endpoint. At Hub capture, the shared deterministic identity resolver anchors a unique canonical name/alias/address match into the source envelope before model work; ambiguity stays explicit and unlinked. Reactions, system events, and attachment-only rows are not evidence. Its durable cursor advances only after capture acceptance/de-duplication and its heartbeat is reported in daily INGEST. Automatic selection holds a new `messaging_message` for five minutes so a reply can be captured first. Triage, duplicate review, and action projection then receive same-`chat_id` neighbours as prompt context only — never as the source text, so a later reply cannot change the revision hash. After projection, `crm_action_resolution` may complete an open task created from an earlier message in that chat when this turn clearly answers it. Uncertain resolutions stay in the action queue and do not mark the answering source incomplete. Historical backfill still cannot create or resolve live tasks.
...
`lib/contact-identity.js` is the pre-model source identity boundary shared by Gmail, sent Gmail, AgentMail, WhatsApp, and Apple Messages. It resolves only a unique canonical name, alias, email/phone identity (including active identity atoms), unique first name, or corroborated composite alias; contradictory signals and duplicate/near names fail closed. The resolved contact ID is source context, not a relationship claim, and is available before classifiers, triage, atom extraction, and entity linking. Synthesis no longer performs deterministic partial-name matching. Existing source anchors are repaired dry-run-first with `scripts/repair-source-identities.js`.

`lib/entity-resolution.js` (`resolveMeetingEntities`, feature `entity_resolution`, Terra) links person names as spoken/transcribed to existing contacts *at ingest*, before anything downstream reads `matched_contact`. It replaced the failure where identity arrived only through preliminary model output: a garbled surname ("Alec Kangley"), a bare first name ("Ken"), or a confidently wrong assignment could freeze the wrong `matched_contact`. Resolution runs in `processIntake` right after extraction: the shared deterministic source-identity pass fills blanks and corrects contradictions first, then a bounded, fail-closed model adjudication (≥0.85 confidence, scoped to the meeting's people and each candidate's known atoms) handles what remains. A confident model match rewrites `matched_contact` across attendees/actions/updates **and is learned as a durable alias** on the contact, so the next transcript is a free match — the store is `contacts.aliases`, not a new table. A bare first name is only *persisted* as an alias when it is unique across the CRM: two people who could both be "Nick" resolve per-meeting from context but never freeze a global alias. This is enforced by the **alias invariant** (`aliasCollidesWithOtherContact`, checked in `learnAlias` for every caller — auto-resolver, human answer, task-owner): an alias is an alternate name for the *same* person (Dad = Alister); a name that already denotes a *different* contact (Nick Chin) can never become an alias of someone else (Duncan Sackfield) — that is a CRM failure from a narrow read of one sentence, not a nickname, and a transcript error between two known people (Alec/Alan) likewise stays a per-transcript resolution. `scripts/repair-bad-aliases.js` removes colliding aliases already stored and reverts the frozen `matched_contact` links they produced. Every auto-learned alias writes a `knowledge_receipts` row (`stage='entity_resolution'`, `derived_by='auto_resolver'`) for audit. Human answers can also learn a confirmed alias. The near-name quality board skips a transcript name that is itself a known person's first name ("Alan" is Alan Garland, not a mishearing of Alec Hirst), while a genuinely unknown near-name still flags. `scripts/repair-entity-resolution.js` re-links and corrects already-stored extractions deterministically (no model calls, no corpus re-queue) and strips stale warnings; `--model` opts into adjudicating still-unmatched names.

`lib/attribution-reconciliation.js` closes the later attribution gap that entity resolution cannot: an extraction may identify every person correctly yet still assign a mentioned person's action or fact to the speaker whose paragraph contained the mention. The recurring `attribution_reconciliation` job reads one processed meeting at a time with the complete transcript, extraction, meeting-derived atoms, `crm_action_outcomes`, linked open tasks, scheduled attendees, known contacts, and prior human corrections. Its dedicated registry slot is Codex `gpt-5.6-terra` at high effort (240k input); no OpenRouter fallback exists. A semantic state hash makes the frequent scan cheap: unchanged source and compiled attribution state produce no model call, while new links, changed outcomes/atoms, task ownership, or a revised transcript are rechecked. Any pass that mutates attribution requires one stabilization reread, allowing Terra to catch related extraction/outcome/atom copies omitted from the first response; a no-change result then closes the loop. Exact transcript quotations are mandatory. Missing links can auto-apply at ≥0.97; quote-backed unlinks of a compiled fact that is not about the linked person can auto-apply at ≥0.97; replacing an existing person link or changing participant status needs ≥0.995. `/crm/questions` only receives person-decidable leftovers: scheduled-attendee contradictions, person-vs-person owner changes below the auto threshold, action/attendee ambiguity, and genuine task-owner conflicts. Organisational facts wrongly hanging off a speaker are corrected or dropped — they are not a question queue. A confirmation at `/crm/questions` is source-scoped correction evidence, not an alias, and becomes authoritative on later passes. The pass updates frozen extraction links and splits mixed-provenance atoms rather than moving evidence from other sources. When an already-projected task is affected, its existing `crm_action_outcomes` row is the pending/applied/error outbox and `google-tasks.updateTask` is the only external writer; conflicting owner claims surface rather than silently picking one.
