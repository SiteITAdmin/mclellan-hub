...
`lib/entity-resolution.js` (`resolveMeetingEntities`, feature `entity_resolution`, Terra) links person names as spoken/transcribed to existing contacts *at ingest*, before anything downstream reads `matched_contact`. It replaced the failure where the only linker was an exact string match against `contacts.name`/`aliases`: a garbled surname ("Alec Kangley"), a bare first name with a possible collision ("Ken"), or a nickname left `matched_contact` null, so the same person turned up in `/crm/questions` forever — even after Douglas answered "that is Ken Murray" a dozen times, because the answer was never written back as reusable knowledge. Resolution runs in `processIntake` right after extraction: a deterministic exact/alias pass first, then a bounded, fail-closed model adjudication (≥0.85 confidence, scoped to the meeting's people and each candidate's known atoms) for what remains. A confident match rewrites `matched_contact` across attendees/actions/updates **and is learned as a durable alias** on the contact, so the next transcript is a free exact match — the store is `contacts.aliases`, not a new table. A bare first name is only *persisted* as an alias when it is unique across the CRM: two people who could both be "Nick" resolve per-meeting from context but never freeze a global alias. This is enforced by the **alias invariant** (`aliasCollidesWithOtherContact`, checked in `learnAlias` for every caller — auto-resolver, human answer, task-owner): an alias is an alternate name for the *same* person (Dad = Alister); a name that already denotes a *different* contact (Nick Chin) can never become an alias of someone else (Duncan Sackfield) — that is a CRM failure from a narrow read of one sentence, not a nickname, and a transcript error between two known people (Alec/Alan) likewise stays a per-transcript resolution. `scripts/repair-bad-aliases.js` removes colliding aliases already stored and reverts the frozen `matched_contact` links they produced (a wrongly-attributed action returns to unresolved rather than staying silently mis-owned). Every auto-learned alias writes a `knowledge_receipts` row (`stage='entity_resolution'`, `derived_by='auto_resolver'`) for audit and a future correction pass. Corrections stick from the other direction too: `answerMeetingQuestion` (`learnAliasFromMeetingAnswer`) turns "that is Ken Murray" into the alias `Ken`→Ken Murray when the answer names exactly one contact and is not a "different people" denial. The near-name quality board no longer manufactures false pairs on bare first names — `intakeClarificationIssues` skips a transcript name that is itself a known person's first name ("Alan" is Alan Garland, not a mishearing of Alec Hirst), while a genuinely unknown near-name ("Trina" vs the mononym "Triona") still flags — and stale identity warnings ("X is not in the known CRM") are suppressed once X resolves. `scripts/repair-entity-resolution.js` re-links already-stored extractions deterministically (no model calls, no corpus re-queue — respects the auto-process freeze) and strips their stale warnings; `--model` opts into adjudicating still-unmatched names.
