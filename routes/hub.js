...
router.post('/api/debrief/message', requireAuth, requireSameOrigin, async (req, res) => {
...
router.post('/api/meeting/submit', requireAuth, requireSameOrigin, uploadLimiter, meetingUpload.single('transcript'), async (req, res) => {
  ...
  if (req.file) {
    try {
      ...
      const md = await fileToMarkdown(req.file.originalname || 'transcript.txt', req.file.buffer);
      uploadedTranscript = md.markdown;
    } catch (err) {
