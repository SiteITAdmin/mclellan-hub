...
const SYSTEM_MODEL_GROUPS = [
  ...
  { id: 'background', label: 'Background processing', slots: [
    ...
    { feature: 'repair_agent', scope: 'system', label: 'Self-repair coding agent', note: 'The locally authenticated Grok CLI runs this model in the Mac-mini repair venue when writing bounded fixes for narrow errors. Grok 4.5 only; isolated git worktree, 12-turn/5-minute cap, human-reviewed via GitHub PR before any deploy.', fallback: 'grok-4.5' },
    ...
    { feature: 'daily_narrative', scope: 'system', label: 'Daily narrative', note: 'Writes the "TODAY" section of the Consigliere report — a short plain-English account of what actually came in (emails, new facts, meetings, documents), grounded strictly in that day\'s content. Same voice/quality bar as the Consigliere brief.', fallback: 'anthropic/claude-sonnet-4-6' },
  ...
  { id: 'suggestions', label: 'Suggestion engine', slots: [
    ...
    { feature: 'suggestion_contact',    scope: 'system', label: 'Contact reason suggester', note: 'Proposes a staged outreach reason for each person without an open task. May use clearly labelled hypotheses; nothing enters knowledge before acceptance.', fallback: 'prompt only (Suggestion engine model)' },
    ...
    { feature: 'suggestion_duplicate_review', scope: 'system', label: 'Suggestion duplicate reviewer', note: 'Blocks semantically repeated or already-tasked suggestions after synthesis. Prompt only — runs on the Suggestion engine model.', fallback: 'prompt only (Suggestion engine model)' },
  ...
  { id: 'knowledge', label: 'Knowledge layer', slots: [
    ...
    { feature: 'entity_resolution',       scope: 'system', label: 'Meeting entity resolution', note: 'Codex Terra — links person names spoken/transcribed in a meeting to known contacts (garbled surname "Alec Kangley" → Alec Hirst, bare "Ken" → Ken Murray). Fail-closed; confident matches are learned as durable aliases.', fallback: 'codex/gpt-5.6-terra (registry)' },
    { feature: 'attribution_reconciliation', scope: 'system', label: 'Attribution reconciliation', note: 'Codex GPT-5.6 Terra — change-driven whole-transcript pass that fills missing person links and corrects contradicted fact/action/task attribution with receipts and review gates.', fallback: 'codex/gpt-5.6-terra (registry)' },
    { feature: 'cross_entity_synthesis',  scope: 'system', label: 'Cross-entity synthesis',  note: 'Claude Sonnet via Mac worker — change-driven synthesis over a fixed 80-atom packet: newest atoms as the driver plus their whole-corpus semantic neighbours, so old atoms are reachable. Insights accumulate (bounded) rather than being rebuilt nightly. Writes insight atoms.', fallback: 'claude/sonnet (registry, 80-atom packet)' },
  ...
  { id: 'newsletter', label: 'Newsletter intelligence', slots: [
    ...
    { feature: 'newsletter_digest_briefing', scope: 'system', label: 'Newsletter intelligence brief', note: "Rewrites Douglas's daily Douglas Newsletters digest into the house briefing style (sponsors/tracking stripped, PDF emailed). ChatGPT Go/Codex on the Mac worker.", fallback: 'openai/gpt-5.6-luna' },
  ]},
