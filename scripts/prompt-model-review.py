...
def atomic_write_text(path: Path, content: str) -> None:
    """Publish a compiled report without exposing readers to a partial file."""
...
def safe_auto_install_all(
    report: dict[str, Any],
    target_db: Path,
    write_root: Path,
    threshold: float,
) -> dict[str, Any]:
    """Atomically stage every unique high-confidence candidate as disabled."""
    root = write_root.resolve()
    target = target_db.resolve()
    try:
        target.relative_to(root)
    ...
    for issue in report.get("issues", []):
        recommendations = issue.get("recommendations") or []
        if not issue.get("auto_installable") or not recommendations:
            continue
        candidate = recommendations[0]
        if float(candidate.get("score") or 0) < threshold:
            continue
        ...
        if not candidate_id:
            continue
        entry = candidates.setdefault(candidate_id, {"candidate": candidate, "issues": [], "config": issue.get("current_config") or {}})
        entry["issues"].append(issue.get("id"))
        ...
        with connection:
            for candidate_id, item in sorted(candidates.items()):
                ...
                existing = connection.execute(
                    "SELECT key,enabled FROM model_config WHERE model_id = ?", (candidate_id,)
                ).fetchone()
                if existing:
                    result = {"action": "already_present", "key": existing["key"], "enabled": existing["enabled"]}
                else:
                    base_key = slugify(candidate_id)
                    key = base_key
                    suffix = 2
                    while connection.execute("SELECT 1 FROM model_config WHERE key = ?", (key,)).fetchone():
                        key = f"{base_key}-{suffix}"
                    ...
                    values = {
                        "key": key,
                        "label": candidate.get("name") or candidate_id,
                        "endpoint": "openrouter",
    ...
    return {
        ...
        "inserted_disabled_count": sum(row["action"] == "inserted_disabled" for row in installations),
        "already_present_count": sum(row["action"] == "already_present" for row in installations),
        "slot_and_prompt_assignments_unchanged": True,
    }
...
def run_weekly(
    ...
    """Compile the review and stage safe candidates; live assignments stay untouched."""
    lock_file.parent.mkdir(parents=True, exist_ok=True)
    with lock_file.open("a+", encoding="utf-8") as lock:
        try:
            fcntl.flock(lock.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise ReviewError("A prompt/model governance review is already running") from exc
        catalog, source = load_catalog(catalog_json, catalog_url)
        if save_catalog:
            atomic_write_text(save_catalog, json_dump({"data": catalog}))
        policy = load_policy(policy_path)
        initial = review(repo, hub_db, catalog, source, as_of, policy)
        install_result = safe_auto_install_all(initial, hub_db, write_root, threshold)
        compiled = review(repo, hub_db, catalog, source, as_of, policy)
        compiled["run"] = {
            "reason": reason,
            "cadence": "weekly",
            "install_policy": "high-confidence candidates are added disabled; assignments and prompts never change",
            **install_result,
        }
        atomic_write_text(output, json_dump(compiled))
        if markdown:
            atomic_write_text(markdown, markdown_report(compiled))
        return compiled
...
def build_parser() -> argparse.ArgumentParser:
    ...
    weekly_parser = subparsers.add_parser("weekly", help="Run the compiled weekly review and stage safe candidates disabled")
    weekly_parser.add_argument("--repo", type=Path, required=True)
    weekly_parser.add_argument("--hub-db", type=Path, required=True)
    weekly_parser.add_argument("--write-root", type=Path, required=True)
    weekly_parser.add_argument("--catalog-json", type=Path)
    weekly_parser.add_argument("--catalog-url", default=DEFAULT_CATALOG_URL)
    weekly_parser.add_argument("--save-catalog", type=Path)
    weekly_parser.add_argument("--policy", type=Path)
    weekly_parser.add_argument("--as-of", type=parse_date, default=date.today())
    weekly_parser.add_argument("--threshold", type=float, default=0.72)
    weekly_parser.add_argument("--output", type=Path, required=True)
    weekly_parser.add_argument("--markdown", type=Path)
    weekly_parser.add_argument("--lock-file", type=Path, required=True)
    weekly_parser.add_argument("--reason", default="scheduled")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "auto-install":
            ...
            return 0
        compiled = run_weekly(
            args.repo.resolve(), args.hub_db.resolve(), args.write_root.resolve(),
            args.output.resolve(), args.markdown.resolve() if args.markdown else None,
            args.catalog_json.resolve() if args.catalog_json else None, args.catalog_url,
            args.save_catalog.resolve() if args.save_catalog else None,
            args.policy.resolve() if args.policy else None, args.as_of, args.threshold,
            args.lock_file.resolve(), args.reason,
        )
        print(json.dumps({"ok": True, "output": str(args.output), **compiled["summary"], **{
            "inserted_disabled_count": compiled["run"]["inserted_disabled_count"]
        }}, sort_keys=True))
        return 0
