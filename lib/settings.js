...
// Returns the stored prompt override for a slot, or defaultText if none set.
function getSystemPrompt(feature, userScope, defaultText) {
  try {
    const row = db.hub().prepare(
      'SELECT value FROM crm_context WHERE user = ? AND key = ?'
...
// Returns the stored prompt override (null if not set — for admin display).
function getSystemPromptOverride(feature, userScope) {
  try {
    const row = db.hub().prepare(
      'SELECT value FROM crm_context WHERE user = ? AND key = ?'
    ).get(userScope, PROMPT_PREFIX + feature);
...
// Saves (or clears) a prompt override for a slot.
function setSystemPromptOverride(feature, userScope, text) {
  const hub = db.hub();
  if (!text?.trim()) {
    hub.prepare('DELETE FROM crm_context WHERE user = ? AND key = ?')
...
module.exports = {
  ...
  getSystemPrompt, getSystemPromptOverride, setSystemPromptOverride,
};
