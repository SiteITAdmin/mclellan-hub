...
function composeReport({ date, monitorReport, monitorError, dbItems, briefingResult, briefingError }) {
  ...
  if (briefingError) panicReasons.push(`Daily briefing failed to build/send: ${briefingError.message}`);
...
async function runDailyIntelligencePipeline() {
  ...
  return { monitorReport, briefingResult, report };
}

module.exports = { runDailyIntelligencePipeline };
