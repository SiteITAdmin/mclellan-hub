...
function briefingDelivered(result) {
  if (result?.skipped && result?.reason === 'before-start-date') return true;
  return Boolean(result?.manifest?.sentAt);
}

function markLatestBriefingDelivered(manifest) {
  if (!manifest?.edition || !manifest?.date) return false;
  const hub = db.hub();
  const run = hub.prepare(`
    SELECT id FROM reg_monitor_runs
    WHERE run_date = ?
  ...
  hub.prepare(`
    UPDATE reg_monitor_runs
    SET briefing_edition = ?, briefing_ok = 1, briefing_error = NULL,
        items_included_in_briefing = ?
    WHERE id = ?
  `).run(manifest.edition, includedCount, run.id);
  return true;
}
...
function composeReport({ date, monitorReport, monitorError, dbItems, briefingResult, briefingError }) {
  ...
  for (const s of (monitorReport?.sourceSummary || [])) {
    const tag = s.discovery && !s.error
      ? `${s.linksExtracted || 0} official candidate(s) found via ${s.provider || 'search'}, ${s.newLinks || 0} new, ${s.relevantCount || 0} relevant`
      : s.baselined
  ...
  } else if (briefingResult?.queued) {
    lines.push(`PENDING — Daily Briefing ${briefingResult.meta?.edition || ''} is queued to the Mac subscription worker; delivery is not yet confirmed.`);
...
async function runDailyIntelligencePipeline() {
  ...
  const delivered = !briefingError && briefingDelivered(briefingResult);
  markRunReported(monitorReport?.runId, {
    edition: report.edition,
    ok: delivered,
    error: briefingError?.message || (briefingResult?.queued ? 'Briefing queued; awaiting Mini completion and delivery receipt' : null),
    includedCount: dbItems.filter(i => i.included_in_briefing).length,
  });

  return { monitorReport, briefingResult, report, briefingOk: delivered };
}
...
async function retryDailyBriefing() {
  ...
  const ok = !briefingError && briefingDelivered(briefingResult);
  ...
  if (ok && manifest) {
    ...
    // No bespoke "recovered" email here — sendStoredBriefing() (inside
    // sendTodayNakaiDailyBriefing) already emails Douglas a real content
    // confirmation the moment the briefing is actually written and sent,
    // whether that happens on this retry or already happened async via the
    // Mac-mini completion callback before this retry ran.
  }
...
module.exports = {
  ...
  markLatestBriefingDelivered,
  _test: { briefingDelivered },
};
