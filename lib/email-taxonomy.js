...
function ensureEmailTaxonomy(user) {
  ...
  // Fold classification rules from config into an existing database exactly
  // once, marked so a fresh install and a working install converge without a
  // manual DB edit — and so a rule Douglas later deletes via the admin page is
  // never resurrected by a restart. legacy_mappings are a one-time migration
  // fold, not a live source of truth, so they are not part of this seed.
  const seedKey = '_email_taxonomy_classification_seed_v1';
  const seeded = hub.prepare(
    'SELECT 1 FROM crm_context WHERE user = ? AND key = ?'
  ).get(user, seedKey);
  if (!seeded) {
    const seedRules = hub.transaction(() => {
      const upsert = hub.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, 1)
      `);
      for (const rule of taxonomyDefaults.classification_rules || []) {
        upsert.run(
      ...
      `).run(uuid(), user, seedKey, String(Math.floor(Date.now() / 1000)));
    });
    seedRules();
  }
