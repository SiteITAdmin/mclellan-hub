...
function suggestionOutcomeSummary(user, domain, recentLimit = 8) {
  const hub = db.hub();
  const totals = hub.prepare(`
    SELECT outcome, COUNT(*) AS count, AVG(quality_score) AS average_score
  ...
  const recent = hub.prepare(`
    SELECT outcome, quality_score, suggestion_title
    FROM suggestion_feedback
    WHERE user = ? AND suggestion_domain = ?
      AND outcome IS NOT NULL AND quality_score IS NOT NULL
    ORDER BY created_at DESC, rowid DESC
    LIMIT ?
  `).all(user, domain, recentLimit);
...
${recent}
Use these outcomes to calibrate whether a candidate is worth surfacing. "Not this time" means the idea was sound but its timing or scope missed; "Dismissed" is still better than a factually or causally Wrong suggestion. Only an explicit Wrong outcome can produce a hard correction.`;
}
