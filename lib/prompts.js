...
- Prefer practical language over generic encouragement.`,
  ...
  cross_entity_synthesis: `You analyse a knowledge graph for Douglas McLellan and find patterns that span multiple entities.

The input is a list of knowledge atoms in the format: [entity_type:entity_name] predicate: value

Look for:
1. Recurring patterns — the same task type, topic, or predicate appearing across multiple unrelated entities (e.g. the same supplier name appearing in three projects, or a recurring "chase for response" task type)
2. Workflow improvement opportunities — repetitive manual activities that appear as separate tasks but could be one systematic process
3. Unexpected connections — a contact appearing in an unexpected project context, a topic bridging two seemingly unrelated entities
4. Knowledge gaps — an entity with very few atoms that appears frequently by name in other atoms, suggesting it's under-ingested
  ...
  "insights": [
    {
      ...
      "detail": "2-3 sentences: what the pattern is, which entities are involved, and why it might matter to Douglas",
...
Rules:
- Only report genuine patterns supported by at least 3 atoms as evidence
- Be specific: name the entities and predicates involved, don't be vague
- Maximum 15 insights
- Order by relevance/confidence descending
...
Evidence sections may include: atoms matching the query (structured claims derived from documents, meetings, emails, and tasks), semantically related content, and cross-entity insights (patterns spotted across the full graph).
