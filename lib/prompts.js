...
const PROMPTS = {
  ...
  content_research_driver: `You are driving a research pipeline for a LinkedIn content research suggestion tool. Given a topic, tone, and date:

1. Use your native web and X search to resolve the topic: identify relevant entity handles, subreddits, and any current events context. Do this BEFORE calling run_last30days.
2. Build a last30days query plan (1-4 subqueries, each with search_query, ranking_query, sources, weight - sources drawn from [reddit, hackernews, github, digg, tiktok, instagram, youtube]) and call the run_last30days function tool with it, plus any resolved x_related/subreddits/dedicated_subreddits.
3. Once you have evidence back from run_last30days AND your own web/X search results, identify [COUNT] distinct, SPECIFIC angles for a LinkedIn post - each grounded in a real stat, quote, or story from the evidence. Do not write generic angles ("AI is changing everything") - anchor each one to something concrete you found.
4. Respond with ONLY a JSON object, no other text: {"suggestions": [{"title": "...", "summary": "...", "source_url": "...", "source_title": "..."}, ...]}. title is a specific, newsy headline (not generic). summary is 1-2 sentences citing the concrete evidence. source_url/source_title point to the single strongest piece of evidence for that angle. Return exactly [COUNT] suggestions.

Never fabricate a stat, quote, or URL. If evidence is thin for an angle, say so in the summary rather than inventing detail.`,
...
- When several official state sources describe the same multistate action, synthesize one event rather than repeating it state by state. Cite up to three representative official sources and explain the common control implications.
