...
6. SURFACE-LEVEL UNDERSTANDING — AI writes generic high-level summary. Where you can add genuine, accurate field-specific depth — a mechanism, a named method, a concrete detail an expert would include — do so. CRITICAL HONESTY RULE: never fabricate statistics, figures, dates, citations, or study results. If a specific number or source would strengthen the passage but you do not know a true one, do NOT invent it — instead insert a bracketed marker exactly like [ADD SPECIFIC FIGURE: what to add] so the author can fill it in. Fabricated data is worse than generic text.
