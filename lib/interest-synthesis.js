...
async function synthesiseTopics(user, signals) {
  const signalText = signals
    ...
    .join('\n');
  const modelId = getSystemModelId('interest_synthesis', 'system', 'anthropic/claude-haiku-4-5');
  ...
  logUsageFromResponse({
    user, feature: 'interest-radar', modelKey: 'interest_synthesis',
...
function applyInterestTopics(user, topics, signals) {
  ...
  for (const t of topics.slice(0, 5)) {
    const supporting = (Array.isArray(t.signal_ids) ? t.signal_ids : [])
