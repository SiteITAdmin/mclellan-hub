...
async function generateWikiPage({ question, answer, existingPageSummaries }) {
  ...
  const resp = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: modelId,
      messages: injectPromptAddition('wiki_page_writer', 'system', [{ role: 'user', content: prompt }]),
...
async function imageToWiki({ filename, buffer, mimetype, projectName }) {
  const started = Date.now();
  const modelId = getSystemModelId('wiki_image_vision', 'system', 'google/gemini-2.0-flash-001');
  ...
  const visionResp = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: modelId,
      messages: injectPromptAddition('wiki_image_vision', 'system', [{
        role: 'user',
        content: [
