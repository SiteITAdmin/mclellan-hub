...
function addManualLink({ from, to, relation = 'related' }) {
  ...
  // Idempotent write-back into each file's Related section (writable pages only)
  if (a.writable && !a.wikilinks.includes(normalizeLinkKey(b.slug))) injectBacklink(a.fullPath, a.raw, b.slug);
  if (b.writable && !b.wikilinks.includes(normalizeLinkKey(a.slug))) injectBacklink(b.fullPath, b.raw, a.slug);
...
function removeLinkCompletely(fromSlug, toSlug) {
  ...
  if (a && a.writable) stripWikilinksTo(a, toSlug, resolver);
  if (b && b.writable) stripWikilinksTo(b, fromSlug, resolver);
}
...
// ── CRM people sync ───────────────────────────────────────────────────────────
// Ensures every CRM contact has a vault note at People/<name>.md so the graph
// shows them as person nodes. Calls syncContactVaultProfile (from crm.js) for
// any contact whose file is missing. Returns the names it created.
function syncPeopleNotes(contacts) {
  ...
  for (const c of contacts || []) {
    ...
    try {
      const { syncContactVaultProfile } = require('./crm');
      const result = syncContactVaultProfile(c.user, c.id);
      if (result?.synced) created.push(c.name);
    } catch (_) {}
  }
  return created;
}
...
const SEARCH_FILLER = new Set([
  ...
  'how', 'i', 'in', 'is', 'it', 'me', 'most', 'of', 'on', 'please', 'recent',
...
function searchTerms(query) {
  ...
  const useful = words.filter(word => word.length >= 3 && !SEARCH_FILLER.has(word));
  return [...new Set(useful.length ? useful : words.filter(word => word.length >= 2))];
}

function searchExcerpt(content, terms, maxChars = 700) {
  ...
  const start = Math.max(0, focus - Math.floor(maxChars * 0.25));
...
function searchAll(query, { limit = 12 } = {}) {
  ...
  return pages
    .map(p => {
      ...
      return {
        ...p,
        excerpt: searchExcerpt(p.content, matchedTerms),
        _score: matchedTerms.length * 20 + titleHits + bodyHits + typeBoost,
      };
    })
    .filter(Boolean)
...
async function generateWikiPage({ question, answer, existingPageSummaries }) {
  ...
  const resp = await fetch('hub-model://v1/chat/completions', {
    method: 'POST',
    headers: openRouterHeaders(TASK_CODES.WIKI_QA_SAVE),
    body: JSON.stringify({
      model: modelId,
  ...
  logUsageFromResponse({
    user: 'system',
    feature: 'wiki-qa-save',
    modelKey: 'wiki_page_writer',
    fallbackModelId: modelId,
    data,
    taskCode: TASK_CODES.WIKI_QA_SAVE,
...
async function documentToWiki({ filename, markdown, projectName }) {
  ...
  const resp = await fetch('hub-model://v1/chat/completions', {
    method: 'POST',
    headers: openRouterHeaders(TASK_CODES.WIKI_DOC_PAGE),
    body: JSON.stringify({
      model: modelId,
  ...
  if (!resp.ok) throw new Error(`OpenRouter error ${resp.status}`);
  const data = await resp.json();
  logUsageFromResponse({ user: 'system', feature: 'wiki-doc-save', modelKey: 'wiki_page_writer', fallbackModelId: modelId, data, durationMs: Date.now() - started, taskCode: TASK_CODES.WIKI_DOC_PAGE });
  return JSON.parse(data.choices[0].message.content);
}
...
async function imageToWiki({ filename, buffer, mimetype, projectName }) {
  ...
  const visionResp = await fetch('hub-model://v1/chat/completions', {
    method: 'POST',
    headers: openRouterHeaders(TASK_CODES.WIKI_IMAGE_VISION),
    body: JSON.stringify({
      model: modelId,
      messages: [{
        role: 'user',
  ...
  logUsageFromResponse({ user: 'system', feature: 'wiki-image-vision', modelKey: 'wiki_image_vision', fallbackModelId: modelId, data: visionData, durationMs: Date.now() - started, taskCode: TASK_CODES.WIKI_IMAGE_VISION });
