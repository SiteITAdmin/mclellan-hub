...
async function documentToWiki({ filename, markdown, projectName }) {
  ...
  const resp = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: openRouterHeaders(TASK_CODES.WIKI_INGEST),
    body: JSON.stringify({
      model: modelId,
  ...
  if (!resp.ok) throw new Error(`OpenRouter error ${resp.status}`);
  const data = await resp.json();
  logUsageFromResponse({ user: 'system', feature: 'wiki-doc-save', modelKey: 'wiki_page_writer', fallbackModelId: modelId, data, durationMs: Date.now() - started, taskCode: TASK_CODES.WIKI_INGEST });
  return JSON.parse(data.choices[0].message.content);
}
...
async function imageToWiki({ filename, buffer, mimetype, projectName }) {
  ...
  logUsageFromResponse({ user: 'system', feature: 'wiki-image-vision', modelKey: 'wiki_image_vision', fallbackModelId: modelId, data: visionData, durationMs: Date.now() - started, taskCode: TASK_CODES.WIKI_INGEST });
