...
const { getSystemModelId, injectPromptAddition } = require('./settings');
...
async function research(topic, user, onStatus, sourceUrl = null) {
  ...
  const plan = await llmJson(getSystemModelId('linkedin_planner', user, 'deepseek/deepseek-v3.2'), injectPromptAddition('linkedin_planner', user, [
...
You will be provided with the specific "LinkedIn post topic" as input. All generated queries must directly relate to this topic and be framed for effective web searching. Ensure the tone of the queries is neutral and objective, suitable for research purposes. Avoid using placeholder text or overly generic phrases; each query must be actionable and designed to retrieve substantive information. Do not invent proper nouns unless they are explicitly provided as part of the topic to be researched.` },
  ...
  const synthesis = await llmText(getSystemModelId('linkedin_synthesiser', user, 'anthropic/claude-sonnet-4-6'), injectPromptAddition('linkedin_synthesiser', user, [
...
async function draftPost(topic, synthesis, user, onStatus) {
  ...
  const draft = await llmText(getSystemModelId('linkedin_drafter', user, 'deepseek/deepseek-v4-flash'), injectPromptAddition('linkedin_drafter', user, [
...
async function scorePost(draftText, user, onStatus) {
  ...
  const score = await llmJson(getSystemModelId('linkedin_scorer', user, 'anthropic/claude-sonnet-4-6'), injectPromptAddition('linkedin_scorer', user, [
...
async function generateCarousel(topic, synthesis, draft, scoring, user, onStatus) {
  ...
  const slides = await llmJson(getSystemModelId('linkedin_carousel', user, 'deepseek/deepseek-v4-flash'), injectPromptAddition('linkedin_carousel', user, [
...
async function reviewCarousel(slides, draft, scoring, user, onStatus) {
  ...
  try {
    const reviewed = await llmJson(getSystemModelId('linkedin_refiner', user, 'mistralai/mistral-medium-3'), injectPromptAddition('linkedin_refiner', user, [
...
async function refineDraft(draft, synthesis, scoring, user, onStatus) {
  ...
  const topFixes = (scoring.top_fixes || [])
    ...
    .join('\n') || 'none';
  const refined = await llmText(getSystemModelId('linkedin_refiner', user, 'mistralai/mistral-medium-3'), injectPromptAddition('linkedin_refiner', user, [
...
async function generateImage(topic, user, onStatus) {
  ...
  const imagePrompt = await llmText(getSystemModelId('linkedin_image', user, 'deepseek/deepseek-chat'), injectPromptAddition('linkedin_image', user, [
