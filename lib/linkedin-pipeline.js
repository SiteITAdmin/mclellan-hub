...
async function refineDraft(draft, scoring, onStatus) {
  ...
  const refined = await llmText('mistralai/mistral-medium-3', [
    { role: 'system', content: `You are a ruthless editor improving a LinkedIn TEASER post for Douglas McLellan, a senior technology and digital transformation leader in Ireland (M365, AI strategy, healthcare IT, operational leadership). Today's date is ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}.

RECENCY: Do not introduce any statistics, examples, or claims from your training data. Work only from the draft and research context provided. If any figure in the draft references 2024 or earlier as if current, remove or reframe it — flag that the research should supply a more recent figure instead. Never present stale data as current.
