...
const { chatText, chatJson } = require('./chat-completions');
...
// ── LLM helpers (subscription plane — no OpenRouter, no cross-provider fallback) ─
...
async function llmText(model, messages, temperature = 0.7, taskCode = TASK_CODES.CONTENT, modelKey = 'linkedin-pipeline') {
  void model; void temperature; void taskCode;
  const content = await chatText({
    feature: modelKey,
    messages,
    timeout: LLM_TIMEOUT_MS,
  });
  if (!content) throw new Error(`LLM (${modelKey}) returned empty content`);
  return content;
}

async function llmJson(model, messages, taskCode = TASK_CODES.CONTENT, modelKey = 'linkedin-pipeline', options = {}) {
  void model; void taskCode; void options;
  return chatJson({
    feature: modelKey,
    messages,
    label: modelKey,
...
async function runPipeline(user, topic, onStatus = () => {}, existingPostId = null, sourceUrl = null, spiciness = 'professional') {
  if (process.env.SUBSCRIPTION_AGENT_DISABLED === '1') throw new Error('subscription model plane disabled');
