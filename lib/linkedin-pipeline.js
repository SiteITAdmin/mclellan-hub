...
async function llmJson(model, messages, taskCode = TASK_CODES.CONTENT, modelKey = 'linkedin-pipeline') {
  const started = Date.now();
...
async function research(topic, user, onStatus, sourceUrl = null) {
  ...
  ], TASK_CODES.LINKEDIN_PLANNER, 'linkedin-planner');
...
async function scorePost(draftText, user, onStatus) {
  ...
  ], TASK_CODES.LINKEDIN_SCORER, 'linkedin-scorer');
...
async function generateCarousel(topic, synthesis, draft, scoring, user, onStatus, spiciness = 'professional') {
  ...
  ], TASK_CODES.LINKEDIN_CAROUSEL, 'linkedin-carousel');
...
async function reviewCarousel(slides, draft, scoring, user, onStatus) {
  ...
  try {
    const reviewed = await llmJson(getSystemModelId('linkedin_carousel_reviewer', user, 'mistralai/mistral-medium-3'), [
    ...
    ], TASK_CODES.LINKEDIN_CAROUSEL_REVIEWER, 'linkedin-carousel-reviewer');
...
async function runPipeline(user, topic, onStatus = () => {}, existingPostId = null, sourceUrl = null, spiciness = 'professional') {
  ...
  try {
    ...
    try {
      ...
      if (quality.quality_veto) persist({ status: 'needs_revision' });
    } catch (err) {
...
async function generateDisplayTitle(user, postId) {
  ...
  const result = await llmJson(getSystemModelId('linkedin_title', user, 'anthropic/claude-haiku-4-5'), [
    { role: 'system', content: titlePrompt },
    { role: 'user', content: `Original topic:\n${String(post.topic || '').slice(0, 2000)}\n\nPublished post text:\n${postText.slice(0, 6000)}` },
  ], TASK_CODES.CONTENT, 'linkedin-title');
...
module.exports = { runPipeline, resumePost, generateDisplayTitle };
