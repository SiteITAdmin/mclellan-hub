...
// ── Main pipeline ─────────────────────────────────────────────────────────────
async function runPipeline(user, topic, onStatus = () => {}, existingPostId = null) {
...
async function resumePost(postId, user, onStatus = () => {}) {
  ...
  let carouselReviewed = false;
  try {
    const { pdf: pdfBuffer, wasReviewed } = await generateCarousel(post.topic, post.research, draft, scoring, onStatus);
    carouselReviewed = wasReviewed;
    const { drive } = getGoogleClients(user);
    const dateStr = new Date().toISOString().slice(0, 10);
