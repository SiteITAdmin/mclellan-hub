...
async function reviewCarousel(slides, draft, scoring, user, onStatus) {
  ...
  try {
    const refinerTodayStr = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
    const refinerPrompt = getSystemPrompt('linkedin_refiner', user, PROMPTS.linkedin_refiner)
      .replace('[DATE]', refinerTodayStr);
