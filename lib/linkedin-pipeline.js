...
const CAROUSEL_REVIEW_PROMPT = `You are a carousel slide editor. You will receive a 4-slide LinkedIn carousel JSON object and a teaser post. Review the slides against the rubric and return an improved version of the same JSON structure.

You MUST return a JSON object with exactly these keys: slide1, slide2, slide3, slide4.
...
Do NOT return the teaser post. Do NOT add extra keys. Return only the carousel JSON.`;
...
async function reviewCarousel(slides, draft, scoring, user, onStatus) {
  ...
  try {
    const reviewed = await llmJson(getSystemModelId('linkedin_refiner', user, 'mistralai/mistral-medium-3'), [
      { role: 'system', content: CAROUSEL_REVIEW_PROMPT },
