...
const db = require('./db');
const { logUsageFromResponse, logOpenRouterUsage, usageFromResponse } = require('./openrouter-usage');
...
const { writeLinkedInQualityReview, qualityVetoBlocks } = require('./hub-quality-board');
...
async function llmJsonOnce(model, messages, taskCode, modelKey) {
  ...
  const data = await r.json();
  // Log AFTER the parse so a bad-shape response is recorded as
  // status='shape_failure', not a lookalike 'ok' — the model_governance capo
  // reads these statuses. This pipeline keeps its own transport because it
  // falls back to a DIFFERENT model on failure (see llmJson) and handles
  // reasoning-model content arrays that the shared requestModelObject
  // deliberately does not.
  const usage = usageFromResponse(data);
  const logAttempt = (status, errorMsg) => logOpenRouterUsage({
    user: 'system',
    feature: modelKey,
    modelKey,
  ...
  try {
    const parsed = parseJsonMessage(data, model, modelKey);
    logAttempt('ok', null);
    return parsed;
  } catch (err) {
    logAttempt('shape_failure', err.message);
    throw err;
  }
}
...
async function runPipeline(user, topic, onStatus = () => {}, existingPostId = null, sourceUrl = null, spiciness = 'professional') {
  ...
  try {
    ...
    try {
      ...
      if (qualityVetoBlocks(user, postId)) {
...
async function resumePost(postId, user, onStatus = () => {}, options = {}) {
  const hub = db.hub();
  const post = loadPost(hub, postId);
  if (!post) throw new Error(`Post ${postId} not found`);
  const originalStatus = options.originalStatus || post.status;
  ...
  // Honour a manual override: if Douglas has chosen to ship this post anyway,
  // the PDF should build too. qualityVetoBlocks returns false once overridden.
  if (qualityVetoBlocks(user, postId, { artifactOnlyOk: true })) {
    persist({ status: 'needs_revision' });
  ...
  const completedStatus = ['scheduled', 'published'].includes(originalStatus) ? originalStatus : 'draft';
  persist({ status: carouselUrl ? completedStatus : 'needs_revision' });
