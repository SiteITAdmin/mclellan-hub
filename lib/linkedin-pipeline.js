...
// ── Research ──────────────────────────────────────────────────────────────────
async function research(topic, onStatus) {
  onStatus('Planning search queries…');

  const plan = await llmJson('deepseek/deepseek-v3.2', [
    { role: 'system', content: `You are an AI assistant tasked with generating targeted web search queries. Today's date is ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}. Your objective is to produce five distinct and specific search queries designed to research a given LinkedIn post topic from multiple perspectives. These queries should enable comprehensive exploration of the subject matter, uncovering various facets, potential arguments, supporting data, and counterpoints.
...
You will be provided with the specific "LinkedIn post topic" as input. All generated queries must directly relate to this topic and be framed for effective web searching. Ensure the tone of the queries is neutral and objective, suitable for research purposes. Avoid using placeholder text or overly generic phrases; each query must be actionable and designed to retrieve substantive information. Do not invent proper nouns unless they are explicitly provided as part of the topic to be researched.` },
  ...
  const context = sources.slice(0, 20).map((s, i) => {
    ...
    return s.fullContent ? `${base}\n\nFull content:\n${s.fullContent}` : base;
  }).join('\n\n---\n\n');

  pipelineLog('sources', sources.slice(0, 20).map(s => `${s.title} — ${s.url}`));
  onStatus('Synthesising research…');
  const synthesis = await llmText('anthropic/claude-sonnet-4-6', [
    { role: 'system', content: `You are a research analyst briefing a senior technology leader (Douglas McLellan — M365, AI strategy, healthcare IT, Ireland/EU) who is writing LinkedIn content. Today's date is ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}.
...
// ── Draft ─────────────────────────────────────────────────────────────────────
async function draftPost(topic, synthesis, onStatus) {
  onStatus('Writing draft…');
  const draft = await llmText('deepseek/deepseek-v4-flash', [
    { role: 'system', content: `You are Douglas McLellan's LinkedIn ghostwriter. Douglas is a senior technology and digital transformation leader based in Ireland — M365, AI strategy, healthcare IT, operational leadership.
...
// ── Score ─────────────────────────────────────────────────────────────────────
async function scorePost(draftText, onStatus) {
  onStatus('Scoring against rubric…');
  const score = await llmJson('anthropic/claude-sonnet-4-6', [
...
async function generateCarousel(topic, synthesis, draft, scoring, onStatus) {
  ...
  const slides = await llmJson('deepseek/deepseek-v4-flash', [
    { role: 'system', content: `Generate rich content for a 4-slide LinkedIn carousel PDF for Douglas McLellan, senior technology and digital transformation leader, Ireland. This is the detailed companion to a short teaser post — give it real substance, analysis, and depth. Target 500+ words total.
...
Voice: direct, third-party practitioner view, no corporate fluff. Include EU/Ireland context where the research supports it. No em dashes (—) anywhere in the output.` },
  ...
  const { slides: reviewed, wasReviewed } = await reviewCarousel(slides, draft, scoring, onStatus);
...
// ── Refine ────────────────────────────────────────────────────────────────────
async function refineDraft(draft, scoring, onStatus) {
  onStatus('Refining draft with Mistral…');
  ...
  const topFixes = (scoring.top_fixes || [])
    ...
    .join('\n') || 'none';
  const refined = await llmText('mistralai/mistral-medium-3', [
    { role: 'system', content: `You are a ruthless editor improving a LinkedIn TEASER post for Douglas McLellan, a senior technology and digital transformation leader in Ireland (M365, AI strategy, healthcare IT, operational leadership).
...
Make targeted improvements based on the feedback below. Preserve the core insight.
...
- All specifics must come from the research. Do NOT invent statistics, timeframes, or outcomes not in the research.
- No em dashes (—). Restructure the sentence or use a full stop instead.
...
// ── Main pipeline ─────────────────────────────────────────────────────────────
async function runPipeline(user, topic, onStatus = () => {}) {
  if (!OR_KEY()) throw new Error('OPENROUTER_API_KEY not set');

  const { synthesis, sources } = await research(topic, onStatus);
  const draft   = await draftPost(topic, synthesis, onStatus);
  const scoring = await scorePost(draft, onStatus);

  // Refine with Mistral based on score feedback — graceful skip
  let refinedDraft = null;
  let teaserRefined = false;
  try {
    refinedDraft = await refineDraft(draft, scoring, onStatus);
    teaserRefined = true;
  } catch (err) {
  ...
  try {
    const { pdf: pdfBuffer, wasReviewed } = await generateCarousel(topic, synthesis, refinedDraft || draft, scoring, onStatus);
  ...
  let imageData = null;
  try { imageData = await generateImage(topic, onStatus); } catch (err) {
  ...
  try {
    ...
    await appendRow(sheets, sheetId, [
      ...
      refinedDraft || draft,                                 // Refined Draft
      scoring.overall_score || '',                           // Score
      scoring.recruiter_value || '',                         // Verdict
      ax.signal_to_noise || '',                              // Signal/Noise
      ax.demonstrated_expertise || '',                       // Expertise
      topFix1 ? `${topFix1.problem} → ${topFix1.fix}` : '', // Top Fix
      scoring.recruiter_perspective || '',                   // Recruiter View
      teaserRefined ? 'Yes' : 'No',                         // Teaser Refined
      carouselReviewed ? 'Yes' : 'No',                      // Carousel Reviewed
      'Draft',
  ...
  return { topic, draft, refinedDraft, score: scoring, sources, localImagePath, carouselUrl, driveUrl, sheetUrl };
}
