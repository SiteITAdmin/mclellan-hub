...
function normalizePolicy(input = {}) {
  ...
  return {
    ...
    topicPlan: {
      ...
      researchEnabled: plan.researchEnabled !== false && plan.researchEnabled !== 'false' && plan.researchEnabled !== 0 && plan.researchEnabled !== '0',
      researchRecur: validRecur(plan.researchRecur, base.topicPlan.researchRecur),
      showIntelFallback: plan.showIntelFallback === true || plan.showIntelFallback === 'true' || plan.showIntelFallback === 1 || plan.showIntelFallback === '1' || plan.showIntelFallback === 'on',
      useLast30Days: plan.useLast30Days !== false && plan.useLast30Days !== 'false' && plan.useLast30Days !== 0 && plan.useLast30Days !== '0',
      dayPrefs: normalizeDayPrefs(plan.dayPrefs),
    },
  };
}

// Overlay only the keys actually present in `partial` onto the stored policy,
// then normalize and save. This is what the split cadence/topic-plan pages
// use: a cadence save carries no dayPrefs and must never wipe them, and a
// topic-plan save carries no schedules and must never reset those.
function mergeContentCadencePolicy(user, partial = {}) {
  const current = getContentCadencePolicy(user);
  const merged = JSON.parse(JSON.stringify(current));
  for (const section of ['linkedin', 'newsletter', 'topicPlan']) {
  ...
  return setContentCadencePolicy(user, merged);
}
