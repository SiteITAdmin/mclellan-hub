...
async function autoExtractDocumentTasks(user, hub, report) {
  ...
  for (const doc of docs) {
    // Skip if any tasks already exist for this doc
    const existing = hub.prepare(
      "SELECT 1 FROM google_tasks WHERE user = ? AND source = 'document' AND source_id LIKE ? AND deleted_at IS NULL LIMIT 1"
    ).get(user, `doc:${doc.id}:%`);
...
module.exports = { runMycelium, buildConnectivityReport };
