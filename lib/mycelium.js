    ...
    if (daysAway <= 1) {
      ...
      if (!taskExists(hub, user, checkinId)) {
        const outcome = await projectTask(user, {
          ...
          source: 'mycelium',
          origin: 'mycelium:flight-checkin',
          sourceId: checkinId,
          projectSlug: null,
        }, {
          createTaskFn,
...
async function checkFlightCalendarConflicts(user, hub, report) {
  ...
  let conflicts = 0;
  for (const f of flights) {
    const dayEvents = events.filter(e => e.date === f.flight_date && !isTaskPlannerCalendarEvent(e));
...
async function autoExtractDocumentTasks(user, hub, report, {
  ...
  for (const doc of docs) {
    ...
    let extracted;
    try {
      // Tests inject fetchFn with an OpenAI-shaped stub; production uses chatJson.
      if (fetchFn && fetchFn !== require('./fetch')) {
        const resp = await fetchFn('hub-model://v1/chat/completions', {
          method: 'POST',
          body: JSON.stringify({
        ...
        extracted = await chatJson({
          feature: 'mycelium-doc-tasks',
          messages: [{ role: 'user', content: prompt }],
          user,
          label: 'Mycelium document task extraction',
        });
      }
    } catch (err) {
    ...
    for (const t of tasks) {
      ...
      try {
        const result = await createTaskFn(user, {
          ...
          source: 'document',
          origin: 'mycelium:document-tasks',
          sourceId,
...
async function connectMeetingsToPrepTasks(user, hub, report, {
  ...
  const upcoming = events.filter(e =>
    !isTaskPlannerCalendarEvent(e)
    && e.date > today && e.date <= in48h && e.attendees?.length > 0
  );
