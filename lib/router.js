...
async function streamOpenRouter(modelId, messages, onChunk, options = {}) {
  ...
  const feature = options.feature
    || (options.webSearch ? 'hub_chat_research' : 'hub_chat');
...
// ── Main route function ───────────────────────────────────────────────────────
// searchProvider: 'off' | 'brave' | 'exa' | 'on' | legacy 'openrouter' (= research on)
// Registry-led: default Sonnet (hub_chat); Grok (hub_chat_research) when research is on.
// Catalogue keys hub-luna/terra/opus force that runner without OpenRouter model ids.
async function routeMessage({
  ...
  let modelKey = model || defaultModelKey;
  if (!modelKey || modelKey === 'free' || modelKey === 'openrouter/free' || !models[modelKey]) {
    modelKey = models['hub-sonnet'] ? 'hub-sonnet' : defaultModelKey;
  }
  const def = models[modelKey]
    || models['hub-sonnet']
    || models[defaultModelKey]
    || { endpoint: 'subscription', id: 'claude/sonnet', tier: 'everyday', search: 'none', label: 'Hub Sonnet' };

  if (noSearch || searchProvider === 'off') searchProvider = 'off';
  else if (searchProvider === 'openrouter') searchProvider = 'on';

  const wantsResearch = searchProvider !== 'off';
  // Catalogue key → registry feature. Research toggle uses Grok unless multi-search.
  let chatFeature = 'hub_chat';
  if (modelKey === 'hub-luna') chatFeature = 'hub_chat_luna';
  else if (modelKey === 'hub-terra') chatFeature = 'hub_chat_terra';
  else if (modelKey === 'hub-opus') chatFeature = 'hub_chat_opus';
  else if (modelKey === 'hub-grok') chatFeature = 'hub_chat_research';
  else if (modelKey === 'hub-sonnet') chatFeature = 'hub_chat';
  if (wantsResearch && modelKey !== 'multi-search' && modelKey !== 'hub-luna' && modelKey !== 'hub-terra' && modelKey !== 'hub-opus') {
    chatFeature = 'hub_chat_research';
  }
  const resolvedTaskCode = taskCode ||
    (def.tier === 'image' ? TASK_CODES.IMAGE :
      def.endpoint === 'multi-search' || chatFeature === 'hub_chat_research' || def.tier === 'research' || def.tier === 'news-research'
  ...
  const webCapable = def.search !== 'none' || chatFeature === 'hub_chat_research';

  // Prefetch Exa/Brave when the user asked for search — never OpenRouter web plugin.
  ...
  if (def.endpoint === 'multi-search' || modelKey === 'multi-search') {
    ...
    searchUsed = true;
  } else if (def.endpoint === 'openrouter' || def.endpoint === 'subscription' || String(modelKey).startsWith('hub-') || !def.endpoint) {
    if (def.tier === 'image') {
      throw new Error('Image generation via OpenRouter is retired. Use an external image tool or a future local vision path.');
    }
    const useResearch = wantsResearch || chatFeature === 'hub_chat_research';
    console.log(`[router] subscription chat feature=${chatFeature} key=${modelKey} research=${useResearch}`);
    result = await streamOpenRouter(def.id || modelKey, finalMessages, onChunk, {
      webSearch: useResearch,
      searchDepth,
    ...
    // Save to disk so Hub surfaces get a real URL instead of a huge data blob
