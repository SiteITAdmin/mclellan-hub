...
async function streamOpenRouter(modelId, messages, onChunk, options = {}) {
  ...
  if (options.webSearch) {
    const plugin = { ...WEB_SEARCH_PLUGIN };
    if (options.searchDepth) plugin.search_context_size = options.searchDepth;
    body.plugins = [plugin];
    console.log(`[stream] ${modelId} sending with webSearch plugin`);
  }
