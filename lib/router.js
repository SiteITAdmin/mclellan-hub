...
const WEB_SEARCH_TOOL = {
  ...
  parameters: { engine: 'firecrawl', max_results: 5 },
};
...
async function streamOpenRouter(modelId, messages, onChunk, options = {}) {
  ...
  if (options.webSearch) {
    // OpenRouter web search plugin currently down (500s across all engines/models).
    // Disabled until OpenRouter restores the service.
    // body.plugins = [{ ...WEB_SEARCH_PLUGIN }];
    console.log(`[stream] ${modelId} webSearch requested but plugin disabled (OpenRouter outage)`);
  }

  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: openRouterHeaders(options.taskCode || TASK_CODES.CHAT),
...
async function generateOpenRouterImage(modelId, prompt) {
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: openRouterHeaders(TASK_CODES.IMAGE),
    body: JSON.stringify({
      model: modelId,
...
async function routeMessage({
  ...
  if (def.endpoint === 'multi-search') {
    ...
    const data = await fetchOpenRouterFull(def.id, finalMessages, resolvedTaskCode);
    const msg = data.choices?.[0]?.message || {};
    const content = msg.content || '';
