...
const DEFAULT_MODELS = {
  ...
  // Web search tool deliberately disabled: the random model selected may not support it,
  // causing empty-answer failures. Users who need search should pick a named model.
  'free':             { endpoint: 'openrouter', id: 'openrouter/free', tier: 'everyday', search: 'none' },
  ...
  'mimo-flash':       { endpoint: 'openrouter', id: 'meta-llama/llama-3.3-70b-instruct:free', tier: 'everyday', search: 'web-plugin' },
  'mistral-small':    { endpoint: 'openrouter', id: 'mistralai/mistral-small-2603', tier: 'everyday', search: 'web-plugin' },
  'grok-fast':        { endpoint: 'openrouter', id: 'x-ai/grok-4.3', tier: 'everyday', search: 'web-plugin' },
  // Project tier
  ...
  'flux-pro':         { endpoint: 'openrouter', id: 'black-forest-labs/flux-1.1-pro:standard', tier: 'image', search: 'none' },
};
...
async function streamOpenRouter(modelId, messages, onChunk, options = {}) {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (parsed.usage) {
          ...
          // OpenRouter provides the real billed cost (includes search fees, native tokens, etc.)
  ...
  if (sources.length > 0 && !substantiveContent) {
    const warning = '⚠️ *The model ran a web search but returned no answer — it may not support the web search plugin. Try a more capable model (Gemini 2.5 Pro, Sonar, DeepSeek V3), or turn web search Off and rephrase.*\n\n';
    onChunk(warning);
    full = warning;
  }
...
async function routeMessage({ model, messages, user, onChunk, noSearch = false, searchProvider = 'openrouter', searchDepth = 'medium' }) {
  ...
  if (webCapable && (searchProvider === 'brave' || searchProvider === 'exa')) {
    ...
    if (lastUser) {
      ...
      const { content, sources } = await searcher(lastUser.content);
...
module.exports = { routeMessage, MODELS, DEFAULT_MODELS, tagConversation, generateGoogleImage, exaSearch, braveSearch, WEB_SEARCH_TOOL };
