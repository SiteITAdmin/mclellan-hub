...
function publishedThinking(user) {
  const posts = db.hub().prepare(
    `SELECT id, topic, content_type, refined_draft, draft, post_url, published_at, created_at
  ...
  return posts.map(post => {
    ...
    return {
      ...
      url: post.post_url || `${baseUrl(user)}/knowledge/${user}/linkedin/${date}-${slugify(post.topic, post.id)}.md`,
    };
  });
}
