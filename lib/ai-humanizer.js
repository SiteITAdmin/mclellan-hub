...
const CHANGE_PATTERNS = new Set([
  'rule_of_three', 'burstiness', 'transitions', 'nuance', 'vocabulary', 'surface_depth',
]);
...
function scanTells(text) {
  ...
  // Rule-of-three: "a, b, and c" style triads.
  const ruleOfThree = (t.match(/\b[\w-]+,\s+[\w-]+,\s+(?:and|or)\s+[\w-]+\b/gi) || []).length;
...
async function humanizeText({ text, field = 'general professional writing', aggressiveness = 'balanced', user = 'system' }) {
  ...
  const inlineMarkers = parsed.humanized_text.match(/\[ADD SPECIFIC FIGURE:[^\]]*\]/gi) || [];
...
module.exports = { humanizeText, scanTells, HUMANIZER_FALLBACK, MAX_INPUT_CHARS };
