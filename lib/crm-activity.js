...
function peopleOverview(user) {
  ...
  `, user);
  // Meetings usually carry the user's own contact row as an attendee (calendar
  // sync never matches a third party to a contact). People met is the distinct
  // contact count across CRM meeting sources (transcript, manual, calendar,
  // crm-engine) minus the owner's own contact record. Planner task blocks
  // are occupancy cache, not meetings.
  const meNameLike = `${user.toLowerCase()}%`;
  const peopleMet = count(hub, `
    SELECT COUNT(*) AS n FROM (
      SELECT DISTINCT ma.contact_id
      FROM meeting_attendees ma JOIN meetings m ON m.id = ma.meeting_id
      JOIN contacts c ON c.id = ma.contact_id
      WHERE m.user = ? AND lower(c.name) NOT LIKE ?
        AND ${crmMeetingSql('m')}
    )
  `, user, meNameLike);
...
function buildCrmActivity(user) {
  ...
  const todayMeetings = content.meetings.length
    ? count(db.hub(), `
        SELECT COUNT(*) AS n FROM meetings WHERE user = ? AND meeting_date = ? AND ${crmMeetingSql('')}
      `, user, today)
    : 0;
