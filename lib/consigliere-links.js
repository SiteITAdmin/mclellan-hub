...
function answerLink(item, out, seen, user) {
  ...
  const defaultKind = ANSWERABLE_KINDS[sourceId];
  const requests = (item.clarification_requests || []).filter(r => r && typeof r === 'object');
  if ((!defaultKind && !requests.some(request => request.kind)) || !requests.length) return;
  try {
    const { clarificationKey } = require('./crm-clarifications');
    const openKeys = openQuestionKeys(user);
    const keys = requests
      .map(request => clarificationKey({ ...request, kind: request.kind || defaultKind }))
      .filter(Boolean);
