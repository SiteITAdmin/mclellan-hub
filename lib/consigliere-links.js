...
// One link to the answer box, anchored on the first question of the batch, so the
// brief lands Douglas where he can respond rather than where the flag came from.
function answerLink(item, out, seen) {
  ...
  try {
    ...
    const key = clarificationKey({ ...requests[0], kind });
    const label = requests.length > 1 ? `Answer these ${requests.length}` : 'Answer this';
    addLink(out, seen, label, `/crm/questions#q-${key}`, false);
