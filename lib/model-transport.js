...
const { inspectModelWork, promptFingerprint } = require('./work-loop-guard');
...
async function runModelText({
  ...
  const fingerprint = promptFingerprint(packet);
  const guard = inspectModelWork({
    feature: modelKey || feature,
    fingerprint,
    user,
  });
  if (!guard.allowed) {
    const blockedUntil = new Date(guard.blockedUntil * 1000).toISOString();
    const message = `Model work loop blocked for ${modelKey || feature}: identical prompt already succeeded ${guard.calls} times; blocked until ${blockedUntil}`;
    logModelUsage({
      user,
      feature,
    ...
    error.code = 'MODEL_WORK_LOOP_BLOCKED';
    throw error;
  }
