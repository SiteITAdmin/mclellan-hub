  ...
  if (sourceKind === 'messaging_message') {
    ...
    return hub.prepare(`
      SELECT id, platform, chat_id, chat_name, is_group, sender_id, sender_name, body,
             (
               'Messaging (' || COALESCE(platform,'whatsapp') || ') from ' ||
               COALESCE(NULLIF(sender_name,''), NULLIF(sender_id,''), 'unknown') ||
               char(10) ||
               'Chat: ' || COALESCE(NULLIF(chat_name,''), NULLIF(chat_id,''), CASE WHEN is_group = 1 THEN 'group' ELSE 'dm' END) ||
               CASE WHEN is_group = 1 THEN ' (group)' ELSE ' (dm)' END ||
               char(10) || char(10) ||
               COALESCE(body,'')
             ) AS text,
             received_at AS ts
...
function actionProjectionBlockReason(sourceKind, duplicateDecision) {
...
async function processSource(user, sourceKind, row, context) {
  ...
  if (decision.candidate_actions.length) {
    const blockReason = actionProjectionBlockReason(sourceKind, duplicateDecision);
    if (blockReason) {
