...
const ACTION_IDENTITY_VERSION = 2;
const MESSAGING_SETTLE_SECONDS = 5 * 60;
const CHAT_RESOLUTION_CONFIDENCE = 0.85;
...
const ACTION_MESSAGING_GUARD = `Runtime messaging rule:
- SAME-CHAT CONTEXT is neighbouring WhatsApp turns, not this source. A question in this source is not outstanding if a later turn already answers it, refuses it, or reports it done.
- Set actionability to fyi for a reply-to-this-question task when the answer is already in that window.
- A leftover new commitment in this source may still be a task.
- Do not quote neighbour text as evidence.`;
...
function isInsideMessagingSettleWindow(evidence, referenceNow = now()) {
  if (evidence?.source_kind !== 'messaging_message') return false;
  const ts = Number(evidence.ts || evidence.row?.received_at || 0);
  return ts > 0 && (Number(referenceNow) - ts) < MESSAGING_SETTLE_SECONDS;
}

function messagingPromptContext(user, evidence) {
  if (evidence?.source_kind !== 'messaging_message' || !evidence.row) return '';
  const neighbors = formatMessagingChatContext(user, evidence.row);
  const open = openChatSourcedActions(user, evidence.row.chat_id)
    .filter(item => String(item.message_id) !== String(evidence.source_id));
  const taskLines = open.length
    ? [
      'OPEN TASKS FROM THIS CHAT (created from earlier messages; resolve one of these only, never invent a TASK_ID)',
      ...open.map(formatOpenChatActionLine),
    ].join('\n')
    : '';
  return [neighbors, taskLines].filter(Boolean).join('\n\n');
}

function formatOpenChatActionLine(item) {
  const when = item.message_at
    ? new Date(Number(item.message_at) * 1000).toISOString()
    : '';
  const who = item.sender_name || 'unknown';
  const title = String(item.title || item.candidate_title || '').replace(/\s+/g, ' ').trim();
  const ask = String(item.evidence_text || item.source_body || '').replace(/\s+/g, ' ').trim().slice(0, 180);
  return `- TASK_ID:${item.task_id} OUTCOME_ID:${item.outcome_id} ${title} | from ${who} at ${when} | ask: ${ask}`;
}

function openChatSourcedActions(user, chatId) {
  if (!user || !chatId) return [];
  return db.hub().prepare(`
    SELECT t.id AS task_id, t.google_task_id, t.title, t.notes, t.status,
           o.id AS outcome_id, o.source_id AS message_id, o.action_key,
           o.evidence_text, o.candidate_title, o.confidence, o.created_at AS outcome_at,
           m.body AS source_body, m.sender_name, m.received_at AS message_at
      FROM crm_action_outcomes o
      JOIN messaging_messages m
        ON m.id = o.source_id AND m.user = o.user
      JOIN google_tasks t
        ON t.id = o.task_id AND t.user = o.user
     WHERE o.user = ?
       AND o.source_kind = 'messaging_message'
       AND o.disposition IN ('task_created', 'task_and_event_created')
       AND m.chat_id = ?
       AND t.status = 'needsAction'
       AND t.deleted_at IS NULL
     ORDER BY m.received_at ASC, o.created_at ASC
  `).all(user, chatId);
}

function chatResolutionActionKey(taskId) {
  return hash(`chat-resolve:${String(taskId || '').trim()}`);
}

function classifyChatResolution(parsed) {
  const decision = String(parsed?.decision || '').trim().toLowerCase();
  const confidence = Number(parsed?.confidence) || 0;
  if (decision === 'resolves' && confidence >= CHAT_RESOLUTION_CONFIDENCE) return 'apply';
  if (decision === 'resolves') return 'review';
  return 'ignore';
}
...
function candidateSources(user, limit, { referenceNow = now() } = {}) {
  ...
  const evidence = listSourceEvidence(user)
    ...
    // WhatsApp Q&A is turn-taking. Hold a fresh bubble so a reply can land in
    // the same-chat window before this source is interpreted in isolation.
    .filter(evidence => !isInsideMessagingSettleWindow(evidence, referenceNow));
...
async function triageSource(user, evidence, entities) {
  ...
  for (const chunk of evidence.chunks) {
    const chatContext = messagingPromptContext(user, evidence);
    const prompt = [
      getSystemPrompt('crm_source_triage', 'system', PROMPTS.crm_source_triage)
...
function existingKnowledgeForReview(user, decision, evidence) {
  ...
  return [
    ...
    messagingPromptContext(user, evidence),
  ].filter(Boolean).join('\n');
}
...
// Deterministic anti-DUPLICATE guard for the OPEN corpus, the mirror of
// closedTaskDuplicateMatch. The model is only shown a recent slice of open
// tasks in the projection prompt, so its duplicate_of only covers what it can
// see; identity (stableActionKey) is bound to the source, so the SAME action
// arriving from a second source — a forwarded briefing, a handover doc, a
// re-forwarded Work Brief, a later meeting — gets a fresh key and would create
// a second task. This closes that hole at the point of creation, regardless of
// source, so no per-subject deny-list is needed.
//
// Two layers, cheapest first, both over the ENTIRE live-open corpus:
//   1. Normalised-title exact match — synchronous, always available, and the
//      only layer that works before a just-created sibling has been embedded.
//   2. Semantic shortlist against open_task embeddings (same Ollama index the
//      closed guard uses for completed_task), then MODEL adjudication for the
//      precision call. The score alone cannot decide here: the live path
//      compares a bare candidate (title + evidence) against a stored task
//      vector (title + notes), and measured on production this corpus's true
//      duplicates land at ~0.52–0.62 while genuinely novel actions reach ~0.48
//      — overlapping bands, so any fixed cutoff would both miss real paraphrases
//      and suppress new work. The embedding is therefore used only for recall:
//      take the most-similar live-open candidates above a low floor and let the
//      crm_duplicate_review model decide "same concrete action?", keeping
//      different people / sub-steps / deliverables distinct (Alan vs Alec must
//      never merge). Below the floor the action is treated as novel with no
//      model call, so novel work stays cheap.
// Fail-open throughout: any embedding or model outage falls back to a weaker
// layer and finally to creating the task, because a wrongly-dropped task costs
// Douglas the thing he needed (effect-gate philosophy in CLAUDE.md).
const OPEN_TASK_SHORTLIST_FLOOR = 0.45;  // recall floor; below this = treat as novel, no model call
const OPEN_TASK_ADJUDICATE_MAX = 2;      // bound model calls to the most-similar live candidates

async function adjudicateOpenTaskDuplicate(user, action, task) {
  try {
    const modelId = getSystemModelId('crm_duplicate_review', 'system', DUPLICATE_FALLBACK);
    const parsed = await requestModelObject({
      modelId,
      messages: [{ role: 'user', content: [
        'A NEW candidate task was extracted from a source. An EXISTING open task is already on the list.',
        'Decide if the NEW task is the SAME concrete action as the EXISTING one — i.e. the NEW one would be a duplicate and should not be created.',
        'They are the SAME only if doing the EXISTING task fully accomplishes the NEW one.',
        'They are DIFFERENT if they involve different people, a different deliverable, or a different concrete step of the same project — even when the wording is similar.',
        '',
        `NEW: ${String(action.title || '').trim()}`,
        action.evidence ? `NEW evidence: ${String(action.evidence).slice(0, 300)}` : '',
        `EXISTING: ${String(task.title || '').trim()}`,
        '',
        'Return JSON {"same_action": true|false}.',
      ].filter(Boolean).join('\n') }],
      user,
      feature: 'crm-duplicate-review',
      modelKey: 'crm_duplicate_review',
      taskCode: TASK_CODES.KNOWLEDGE_SYNTHESIS,
      defaults: { same_action: false },
      label: 'open-task duplicate adjudication',
    });
    return parsed?.same_action === true;
  } catch (err) {
    // Fail-open: if the adjudicator is unavailable, do not suppress the task.
    console.warn('[crm-engine] open-task adjudication failed (task will be created):', err.message);
    return false;
  }
}

async function openTaskDuplicateMatch(user, action, {
  semanticSearchFn = null,
  adjudicateFn = adjudicateOpenTaskDuplicate,
} = {}) {
  const title = String(action?.title || '').trim();
  if (!title) return null;

  const open = db.hub().prepare(`
    SELECT id, title, project_slug FROM google_tasks
    WHERE user = ? AND deleted_at IS NULL AND status = 'needsAction' AND COALESCE(title, '') != ''
  `).all(user);
  if (!open.length) return null;
  const openById = new Map(open.map(t => [t.id, t]));

  // Layer 1: normalised-title exact match over the whole open corpus.
  const wantedTitle = normaliseTaskTitle(title);
  if (wantedTitle) {
    const hit = open.find(task => normaliseTaskTitle(task.title) === wantedTitle);
    if (hit) return { taskId: hit.id, matchedTitle: hit.title, method: 'title', score: 1 };
  }

  // Layer 2: semantic shortlist over open_task embeddings (verified still
  // live-open), then model adjudication of the most-similar candidates.
  try {
    const search = semanticSearchFn || require('./retrieval').semanticSearch;
    const hits = await search(user, `${title}\n${action.evidence || ''}`, 5, {
      sourceKinds: ['open_task'],
      minScore: OPEN_TASK_SHORTLIST_FLOOR,
    });
    let adjudicated = 0;
    for (const top of hits || []) {
      const task = openById.get(top.source_id); // skip stale vectors of since-closed tasks
      if (!task) continue;
      if (adjudicated >= OPEN_TASK_ADJUDICATE_MAX) break;
      adjudicated += 1;
      const same = await adjudicateFn(user, action, task);
      if (same) return { taskId: task.id, matchedTitle: task.title, method: 'semantic_adjudicated', score: top.score };
    }
  } catch (err) {
    console.warn('[crm-engine] open-task semantic dedup skipped:', err.message);
  }
  return null;
}

async function projectActions(user, chunk, candidates, { evidence = null } = {}) {
  ...
  const chatContext = evidence ? messagingPromptContext(user, evidence) : '';
  const messagingGuard = evidence?.source_kind === 'messaging_message' ? `\n\n${ACTION_MESSAGING_GUARD}` : '';
  ...
  const parsed = await requestModelObject({
    modelId,
    messages: [{ role: 'user', content: `${basePrompt}\n\n${ACTION_STATE_GUARD}\n\n${ACTION_EVENT_GUARD}\n\n${ACTION_EVIDENCE_GUARD}${messagingGuard}${chatContext ? `\n\n${chatContext}` : ''}` }],
    user,
...
function reusableResolutionDecision(user, evidence) {
  const row = currentSourceReceipt(user, evidence, 'crm_action_resolved');
  if (!row) return null;
  const status = String(row.status || '').toLowerCase();
  // Skip/error may be retried: newest-first can process the answer before the
  // earlier question has created its task in the same pass. A finished or
  // already-reviewed decision must not loop.
  if (status !== 'done' && status !== 'review') return null;
  return { row, decision: parseJson(row.payload, null) };
}

function emptyResolutionTotals() {
  return { resolved: 0, review: 0, errors: 0, skipped: 0, details: [] };
}

async function requestChatActionResolution(user, evidence, openActions) {
  const modelId = getSystemModelId('crm_action_resolution', 'system', ACTION_FALLBACK);
  const chatContext = formatMessagingChatContext(user, evidence.row) || 'SAME-CHAT CONTEXT\n(none)';
  const openBlock = openActions.length
    ? openActions.map(formatOpenChatActionLine).join('\n')
    : '(none)';
  const prompt = getSystemPrompt('crm_action_resolution', 'system', PROMPTS.crm_action_resolution)
    .replaceAll('[SOURCE_TEXT]', evidence.text)
    .replaceAll('[CHAT_CONTEXT]', chatContext)
    .replaceAll('[OPEN_CHAT_TASKS]', openBlock);
  const parsed = await requestModelObject({
    modelId,
    messages: [{ role: 'user', content: prompt }],
    user,
    feature: 'crm-action-resolution',
    modelKey: 'crm_action_resolution',
    taskCode: TASK_CODES.KNOWLEDGE_SYNTHESIS,
    defaults: { resolutions: [] },
    label: 'CRM chat action resolution response',
  });
  parsed.resolutions = Array.isArray(parsed.resolutions) ? parsed.resolutions : [];
  return { parsed, modelId };
}

function matchResolutionToOpenAction(resolution, openActions) {
  const taskId = String(resolution?.task_id || '').trim();
  if (!taskId) return null;
  return openActions.find(item => String(item.task_id) === taskId) || null;
}

async function applyChatActionResolutions(user, evidence, openActions, parsed, {
  completeTaskFn = null,
  assertSourceLeaseFn = null,
} = {}) {
  const totals = emptyResolutionTotals();
  const { completeTask } = completeTaskFn ? { completeTask: completeTaskFn } : require('./google-tasks');
  for (const resolution of parsed?.resolutions || []) {
    const target = matchResolutionToOpenAction(resolution, openActions);
    if (!target) continue;
    const actionKey = chatResolutionActionKey(target.task_id);
    const existing = getActionOutcome(user, evidence, actionKey);
    if (existing && OUTCOME_TERMINAL.has(existing.disposition)) {
      totals.skipped += 1;
      totals.details.push({ task_id: target.task_id, disposition: existing.disposition, reason: 'already_resolved' });
      continue;
    }
    const verdict = classifyChatResolution(resolution);
    const span = locateEvidenceSpan(evidence, resolution.evidence, evidence.chunks[0])
      || {
        start: 0,
        end: String(evidence.text || '').length,
        chunk_index: evidence.chunks[0]?.index || 0,
        chunk_id: evidence.chunks[0]?.chunk_id || null,
        text: String(resolution.evidence || evidence.text || '').slice(0, 400),
        exact: false,
      };
    const identity = {
      candidate: null,
    ...
    const extra = {
      ...
      target_outcome_id: target.outcome_id,
      target_source_id: target.message_id,
      decision: resolution.decision,
      reason: resolution.reason || '',
      prior_evidence: resolution.prior_evidence || null,
    };
    if (verdict === 'ignore') {
      totals.skipped += 1;
      totals.details.push({ task_id: target.task_id, disposition: 'ignored', reason: resolution.decision || 'unrelated' });
      continue;
    }
    if (verdict === 'review') {
      upsertActionOutcome(user, evidence, actionKey, {
        action, identity, disposition: 'review', reason: 'chat_resolution_uncertain',
        taskId: target.task_id,
        payload: outcomePayload(evidence, action, identity, extra),
      });
      totals.review += 1;
      totals.details.push({ task_id: target.task_id, disposition: 'review', reason: 'chat_resolution_uncertain' });
      continue;
    }
    const task = db.hub().prepare(`
      SELECT id, google_task_id, title, status, deleted_at
        FROM google_tasks WHERE user = ? AND id = ?
    `).get(user, target.task_id);
    if (!task || task.deleted_at) {
      totals.skipped += 1;
      totals.details.push({ task_id: target.task_id, disposition: 'skipped', reason: 'target_task_missing' });
      continue;
    }
    if (taskState(task) !== 'open') {
      upsertActionOutcome(user, evidence, actionKey, {
        action, identity, disposition: 'dismissed', reason: 'resolved_by_later_evidence_already_closed',
        taskId: task.id,
        payload: outcomePayload(evidence, action, identity, extra),
      });
      totals.skipped += 1;
      totals.details.push({ task_id: target.task_id, disposition: 'dismissed', reason: 'already_closed' });
      continue;
    }
    if (!task.google_task_id) {
      upsertActionOutcome(user, evidence, actionKey, {
        action, identity, disposition: 'review', reason: 'chat_resolution_missing_google_task_id',
        taskId: task.id,
        payload: outcomePayload(evidence, action, identity, extra),
      });
      totals.review += 1;
      totals.details.push({ task_id: target.task_id, disposition: 'review', reason: 'missing_google_task_id' });
      continue;
    }
    try {
      if (assertSourceLeaseFn) assertSourceLeaseFn();
      await completeTask(user, task.google_task_id, {
        origin: 'crm-knowledge-engine:action-resolution',
      });
      if (assertSourceLeaseFn) assertSourceLeaseFn();
    } catch (err) {
      ...
      if (assertSourceLeaseFn) assertSourceLeaseFn();
      upsertActionOutcome(user, evidence, actionKey, {
        action, identity, disposition: 'error', reason: `chat_resolution_complete_failed: ${err.message}`,
        taskId: task.id,
        payload: outcomePayload(evidence, action, identity, { ...extra, error: err.message }),
      });
      totals.errors += 1;
      totals.details.push({ task_id: target.task_id, disposition: 'error', reason: err.message });
      continue;
    }
    upsertActionOutcome(user, evidence, actionKey, {
      action, identity, disposition: 'dismissed', reason: 'resolved_by_later_evidence',
      taskId: task.id,
      payload: outcomePayload(evidence, action, identity, extra),
    });
    const prior = resolveSourceEvidence(user, 'messaging_message', target.message_id);
    writeReceipt(user, 'messaging_message', target.message_id, 'crm_action_resolved', {
      status: 'done',
      summary: `Open task "${task.title}" completed because later message ${evidence.source_id} answered it.`,
      payload: {
        resolved_task_id: task.id,
        resolving_source_id: evidence.source_id,
        resolving_revision: evidence.revision_hash,
        target_outcome_id: target.outcome_id,
      },
      modelKey: 'crm_action_resolution',
      sourceRevision: prior?.revision_hash || null,
    });
    totals.resolved += 1;
    totals.details.push({ task_id: target.task_id, disposition: 'dismissed', reason: 'resolved_by_later_evidence' });
  }
  return totals;
}

async function resolveMessagingChatActions(user, evidence, { dependencies = {} } = {}) {
  const totals = emptyResolutionTotals();
  if (evidence?.source_kind !== 'messaging_message') return { ...totals, skipped: 1 };
  const existing = reusableResolutionDecision(user, evidence);
  if (existing) return { ...totals, skipped: 1, reused: true };
  if (actionProjectionBlockReason(evidence.source_kind, null, evidence.row)) {
    writeReceipt(user, evidence.source_kind, evidence.source_id, 'crm_action_resolved', {
      status: 'skipped',
      summary: 'Historical or blocked messaging source is not eligible to resolve live tasks.',
      payload: { reason: 'blocked_source' },
      sourceRevision: evidence.revision_hash,
    });
    return { ...totals, skipped: 1 };
  }
  const openActions = openChatSourcedActions(user, evidence.row?.chat_id)
    .filter(item => String(item.message_id) !== String(evidence.source_id));
  if (!openActions.length) {
    writeReceipt(user, evidence.source_kind, evidence.source_id, 'crm_action_resolved', {
      status: 'skipped',
      summary: 'No open same-chat tasks to resolve.',
      payload: { open_tasks: 0 },
      sourceRevision: evidence.revision_hash,
    });
    return { ...totals, skipped: 1 };
  }
  const resolveFn = dependencies.resolveChatActionsFn || requestChatActionResolution;
  const assertSourceLeaseFn = dependencies.assertSourceLeaseFn || null;
  if (assertSourceLeaseFn) assertSourceLeaseFn();
  const result = await resolveFn(user, evidence, openActions);
  if (assertSourceLeaseFn) assertSourceLeaseFn();
  const applied = await applyChatActionResolutions(user, evidence, openActions, result.parsed, {
    completeTaskFn: dependencies.completeTaskFn || null,
    assertSourceLeaseFn,
  });
  const status = applied.errors ? 'error' : applied.review ? 'review' : 'done';
  writeReceipt(user, evidence.source_kind, evidence.source_id, 'crm_action_resolved', {
    status,
    summary: `${applied.resolved} chat task(s) resolved, ${applied.review} review, ${applied.errors} error.`,
    payload: { resolutions: result.parsed, applied, open_tasks: openActions.map(item => item.task_id) },
    modelKey: 'crm_action_resolution',
...
function normaliseEvidenceIdentity(value) {
  return String(value || '').normalize('NFKC').toLowerCase().replace(/\s+/g, ' ').trim();
}

function sameEvidenceMaterial(left, right) {
  const a = normaliseEvidenceIdentity(left);
  const b = normaliseEvidenceIdentity(right);
  if (!a || !b) return false;
  if (a === b) return true;
  // Old outcomes sometimes retained a whole source/chunk as their evidence
  // while the current projector retains the exact quotation.  Containment is
  // therefore a replay signal for one source, never a cross-source duplicate
  // decision.  A short generic phrase is too weak to act as an identity.
  const shorter = a.length <= b.length ? a : b;
  const longer = a.length <= b.length ? b : a;
  return shorter.length >= 24 && longer.includes(shorter);
}

// A knowledge revision may legitimately change when the Hub learns how a raw
// source links to people/projects.  That must never mint another task from an
// action already projected from the same raw evidence.  This is the durable
// compatibility guard for outcomes created before action identity v2 and for
// future source-representation changes.
function priorSameSourceProjection(user, evidence, identity) {
  const span = identity?.span;
  if (!evidence || span?.exact !== true || !String(span.text || '').trim()) return null;
  const rows = db.hub().prepare(`
    SELECT o.*, t.status AS task_status, t.deleted_at AS task_deleted_at
    FROM crm_action_outcomes o
    LEFT JOIN google_tasks t ON t.id = o.task_id AND t.user = o.user
    WHERE o.user = ? AND o.source_kind = ? AND o.source_id = ?
      AND o.action_key != ?
      AND o.disposition IN ('task_created','event_created','task_and_event_created','existing_open_task','fyi','dismissed')
    ORDER BY o.created_at ASC, o.id ASC
  `).all(user, evidence.source_kind, evidence.source_id, identity.action_key);
  const peers = new Set(Array.isArray(identity.peer_candidate_keys) ? identity.peer_candidate_keys : []);
  for (const row of rows) {
    if (!sameEvidenceMaterial(row.evidence_text, span.text)) continue;
    const payload = parseJson(row.payload);
    // Two separately identified asks may quote the same sentence.  During the
    // current v2 projection batch they are explicit peers and may each project
    // once.  A legacy outcome or an outcome from another source revision is
    // never granted that exception: the old evidence already had its chance.
    const currentV2Peer = row.source_revision === evidence.revision_hash
      && Number(payload.action_identity_version) === ACTION_IDENTITY_VERSION
      && peers.has(row.action_key);
    if (currentV2Peer) continue;
    return { row, payload };
  }
  return null;
}

function reusableActionProjectionForContent(user, evidence) {
  const rows = db.hub().prepare(`
    SELECT * FROM knowledge_receipts
    WHERE user = ? AND source_kind = ? AND source_id = ?
      AND stage = 'crm_action_projected' AND status IN ('done','review')
    ORDER BY created_at DESC, rowid DESC
  `).all(user, evidence.source_kind, evidence.source_id);
  for (const row of rows) {
    const payload = parseJson(row.payload);
    if (payload.action_revision
      && payload.action_revision === (evidence.action_revision_hash || evidence.revision_hash)) {
      return { row, payload, legacy: false };
    }
    // Captured messages are immutable at the raw body boundary. Before action
    // identity v2, their projection receipts did not store action_revision;
    // a completed legacy receipt is nevertheless authoritative that this exact
    // message already had its one automatic task-projection pass.
    if (!payload.action_revision && evidence.source_kind === 'messaging_message') {
      return { row, payload, legacy: true };
    }
  }
  return null;
}

function reuseProjectedActionContent(user, evidence, candidates, priorProjection) {
  const totals = emptyProjectionTotals();
  const peerCandidateKeys = candidates.map(candidate => candidate?.candidate_key).filter(Boolean);
  for (const candidate of candidates || []) {
    const action = {
      candidate_key: candidate.candidate_key,
      title: candidate.action || '',
      actionability: candidate.actionability || 'implied',
      confidence: Number(candidate.confidence) || 0,
    ...
    const identity = {
      ...
      peer_candidate_keys: peerCandidateKeys,
    };
    const existing = getActionOutcome(user, evidence, identity.action_key);
    if (existing && OUTCOME_TERMINAL.has(existing.disposition)) {
      tallyProjection(totals, { outcome: existing, disposition: existing.disposition, replayed: true }, action);
      continue;
    }
    const prior = priorSameSourceProjection(user, evidence, identity);
    const priorTask = prior?.row?.task_id ? {
      status: prior.row.task_status,
      deleted_at: prior.row.task_deleted_at,
    } : null;
    const priorState = priorTask ? taskState(priorTask) : null;
    const priorNeedsHuman = priorProjection.row.status === 'review' && !prior;
    const disposition = priorState === 'open'
      ? 'existing_open_task'
      : priorNeedsHuman ? 'review' : 'dismissed';
    const outcome = upsertActionOutcome(user, evidence, identity.action_key, {
      action,
      identity,
      disposition,
      reason: priorState === 'open'
        ? 'unchanged_action_content_existing_task'
        : priorNeedsHuman
          ? 'unchanged_action_content_review_requires_human'
          : 'unchanged_action_content_already_projected',
      taskId: prior?.row?.task_id || null,
      eventId: prior?.row?.event_id || null,
      payload: outcomePayload(evidence, action, identity, {
        unchanged_action_content: {
          prior_projection_receipt_id: priorProjection.row.id,
          prior_source_revision: priorProjection.payload.source_revision || null,
          prior_action_revision: priorProjection.payload.action_revision || null,
          legacy_message_projection: priorProjection.legacy,
          prior_outcome_id: prior?.row?.id || null,
          prior_disposition: prior?.row?.disposition || null,
        },
      }),
    });
    tallyProjection(totals, { outcome, disposition }, action);
  }
  return totals;
}
...
// When the projection says a named OTHER person is the doer, the task is still
// captured — but attributed to them so it lands in "assigned to others" and
// never in Douglas's planner (his choice, 12 Aug 2026). Douglas-owned or
// unattributed asks stay his, exactly as before.
const OWNER_IS_DOUGLAS_RE = /^(douglas|doug|mclellan|douglas mclellan|me|myself|self|hub|user|douglas mclellan \(me\))$/i;
function assigneeFromOwner(owner) {
  const name = String(owner || '').trim();
  if (!name) return null;
  if (OWNER_IS_DOUGLAS_RE.test(name)) return null;
...
function outcomePayload(evidence, action, identity, extra = {}) {
  return {
    ...
    action_revision: evidence.action_revision_hash || evidence.revision_hash,
...
async function projectOneAction(user, evidence, action, identity, entities, {
  ...
  assertSourceLeaseFn = null,
  approved = false,
  openTaskDuplicateMatchFn = openTaskDuplicateMatch,
} = {}) {
  ...
  if (!approved) {
    ...
    if (prior) {
      ...
      const priorState = priorTask ? taskState(priorTask) : null;
      const disposition = priorState === 'open' ? 'existing_open_task' : 'dismissed';
      return insertInitialActionOutcomeResult(user, evidence, actionKey, {
        action,
        identity,
        disposition,
        reason: priorState === 'open'
          ? 'same_source_evidence_existing_task'
          : 'same_source_evidence_already_projected',
        taskId: prior.row.task_id || null,
        eventId: prior.row.event_id || null,
        payload: outcomePayload(evidence, action, identity, {
          same_source_projection: {
            prior_outcome_id: prior.row.id,
            prior_action_key: prior.row.action_key,
            prior_source_revision: prior.row.source_revision,
            prior_disposition: prior.row.disposition,
  ...
  // An exact provider idempotency-key match belongs to this action's outbox
  // state machine, not the cross-source duplicate corpus. Resolve it first so
  // a stable task can advance pending calendar work or ambiguity review.
  ...
  if (!approved && !stableTask) {
    ...
    // Source-agnostic OPEN-corpus dedup: the same action arriving from a
    // different source (forwarded briefing/handover/Work Brief/meeting) links to
    // the task that already exists instead of creating a second one. An explicit
    // human approval is a decision to create anyway, so it bypasses this.
    const openDuplicate = await openTaskDuplicateMatchFn(user, action);
    if (openDuplicate) {
      if (existing) return preservedExistingOutcomeResult(existing);
      return insertInitialActionOutcomeResult(user, evidence, actionKey, {
        action, identity, disposition: 'existing_open_task', reason: 'duplicate_of_open_task', taskId: openDuplicate.taskId,
        payload: outcomePayload(evidence, action, identity, {
          verified_task_id: openDuplicate.taskId,
          duplicate_of_open_task: {
            task_id: openDuplicate.taskId,
            matched_title: openDuplicate.matchedTitle,
            method: openDuplicate.method,
            score: openDuplicate.score,
          },
        }),
      });
    }
  ...
  if (stableTask) {
    if (taskState(stableTask) !== 'open') {
      if (existing) return preservedExistingOutcomeResult(existing);
      return insertInitialActionOutcomeResult(user, evidence, actionKey, {
        action, identity, disposition: 'dismissed', reason: `stable_task_${taskState(stableTask)}_not_recreated`, taskId,
        payload: outcomePayload(evidence, action, identity, {
    ...
    try {
      const task = await createTaskFn(user, {
        ...
        projectSlug,
        assignee: assigneeFromOwner(action.owner),
      });
...
async function createProjectedTasks(user, sourceKind, sourceId, actions, entities, {
  ...
  for (const action of actions || []) {
    ...
    const identity = {
      ...actionIdentity(evidence, canonicalAction, candidates, chunk),
      peer_candidate_keys: peerCandidateKeys,
    };
...
async function projectCandidates(user, evidence, candidates, entities, dependencies = {}) {
  ...
  // Candidates whose projection batch threw in THIS call. Their fresh
  // action_projection_failed row is not a "prior outage" the reclaim-from-triage
  // recovery below may act on — reclaiming it in the same invocation would
  // re-project the same candidate while the projection provider is still down,
  // double-recording the error and firing task creation the outage should
  // have blocked. Recovery is a later call, once projection is healthy again.
  ...
  for (const [chunkId, group] of byChunk) {
    ...
    for (const batch of candidateBatches(group)) {
      try {
        assertSourceLease();
        const projected = await projectActionsFn(user, chunk, batch, { evidence });
        assertSourceLease();
        ...
        for (const candidate of batch) {
          failedThisRun.add(candidate.candidate_key);
  ...
  for (const candidate of candidates) {
    ...
    if (existing && isRetryableProjectionFailure(existing) && !failedThisRun.has(candidate.candidate_key)) {
...
async function processSourceClaimed(user, evidence, context, { retry = false, dependencies = {} } = {}) {
  ...
  if (!actionProjectionCovered(user, evidence, decision)) {
    if (decision.candidate_actions.length) {
      const priorProjection = reusableActionProjectionForContent(user, evidence);
      if (priorProjection) {
        projection = reuseProjectedActionContent(user, evidence, decision.candidate_actions, priorProjection);
      } else {
        assertSourceLease();
      ...
      writeReceipt(user, evidence.source_kind, evidence.source_id, 'crm_action_projected', {
        ...
        payload: {
          ...
          action_revision: evidence.action_revision_hash || evidence.revision_hash,
          ...(priorProjection ? {
            reused_action_projection: {
              receipt_id: priorProjection.row.id,
              source_revision: priorProjection.payload.source_revision || null,
              action_revision: priorProjection.payload.action_revision || null,
              legacy_message_projection: priorProjection.legacy,
            },
          } : {}),
        },
      ...
      writeReceipt(user, evidence.source_kind, evidence.source_id, 'crm_action_projected', {
        status: 'done', summary: 'No candidate actions in this source.', payload: {
          candidates: [],
          action_revision: evidence.action_revision_hash || evidence.revision_hash,
        }, ...receiptOptions,
      });
  ...
  let resolution = emptyResolutionTotals();
  if (evidence.source_kind === 'messaging_message') {
    try {
      assertSourceLease();
      resolution = await resolveMessagingChatActions(user, evidence, { dependencies });
      assertSourceLease();
    } catch (err) {
      if (isSourceProcessingLeaseLostError(err)) throw err;
      writeReceipt(user, evidence.source_kind, evidence.source_id, 'crm_action_resolved', {
        status: 'error', summary: err.message, payload: { error: err.message }, modelKey: 'crm_action_resolution', ...receiptOptions,
      });
      resolution = { ...emptyResolutionTotals(), errors: 1 };
    }
  }
  const foldResolution = result => ({
    ...result,
    errors: (result.errors || 0) + (resolution.errors || 0),
    reviews: (result.reviews || 0) + (resolution.review || 0),
    resolved: (result.resolved || 0) + (resolution.resolved || 0),
  });
  ...
  if (!decision.should_synthesise) {
    ...
    return foldResolution({ triaged: triageCount, synthesised: 0, skipped: 1, errors: projection.errors, reviews: projection.review });
  }
  ...
  if (duplicateReviewError) {
    ...
    return foldResolution({ triaged: triageCount, synthesised: 0, skipped: 0, errors: projection.errors + 1, reviews: projection.review });
  }
  ...
  if (duplicatePlan.mode === 'review') {
    ...
    return foldResolution({ triaged: triageCount, synthesised: 0, skipped: 0, errors: projection.errors, reviews: projection.review + 1 });
  }
  try {
    assertSourceLease();
    ...
    if (duplicatePlan.mode === 'synthesise_then_retire') {
      ...
      if (!replacements.length) {
        ...
        return foldResolution({ triaged: triageCount, synthesised: 1, skipped: 0, errors: projection.errors, reviews: projection.review + 1, atomsStored: result.stored, proposed: result.proposed });
      }
    ...
    return foldResolution({ triaged: triageCount, synthesised: 1, skipped: 0, errors: projection.errors, reviews: projection.review, atomsStored: result.stored, proposed: result.proposed });
  } catch (err) {
    if (isSourceProcessingLeaseLostError(err)) throw err;
    writeReceipt(user, evidence.source_kind, evidence.source_id, 'crm_knowledge_synthesised', {
...
async function runCrmKnowledgeEngine(user, { limit = 8, linkBudget = 12, dependencies = {} } = {}) {
  ...
  // Newest-first may process a reply before the earlier question in the same
  // pass. Re-enter those messaging sources so resolution can see tasks created
  // later in this run. Saved triage/projection receipts are reused.
  for (const evidence of sources) {
    if (evidence.source_kind !== 'messaging_message') continue;
    try {
      const extra = await processSource(user, evidence, context, { dependencies });
      totals.resolved = (totals.resolved || 0) + (extra.resolved || 0);
      totals.reviews = (totals.reviews || 0) + (extra.review || 0);
      totals.errors = (totals.errors || 0) + (extra.errors || 0);
    } catch (err) {
      if (isSourceProcessingLeaseLostError(err)) continue;
      totals.errors += 1;
    }
  }
  return totals;
...
async function runCrmKnowledgeSource(user, {
  ...
  if (isCanonicalEvidenceExcluded(evidence)) {
    return {
      ...empty,
      reason: 'canonical_source_excluded',
      exclusion_reason: canonicalEvidenceExclusionReason(evidence),
    };
  }
  if (!dependencies.requestModelObject && process.env.SUBSCRIPTION_AGENT_DISABLED === '1') {
...
module.exports = {
  ...
  _test: {
    ...
    MESSAGING_SETTLE_SECONDS,
    CHAT_RESOLUTION_CONFIDENCE,
    ACTION_REVIEW_FLOOR,
    ...
    priorSameSourceProjection,
    reusableActionProjectionForContent,
    reuseProjectedActionContent,
    ...
    openTaskDuplicateMatch,
    adjudicateOpenTaskDuplicate,
    taskHistoryForActionProjection,
    isInsideMessagingSettleWindow,
    messagingPromptContext,
    openChatSourcedActions,
    classifyChatResolution,
    chatResolutionActionKey,
    applyChatActionResolutions,
    resolveMessagingChatActions,
