...
//   2. Semantic match against open_task embeddings (same Ollama index the
//      closed guard uses for completed_task). This corpus is single-topic, so
//      embeddinggemma floors high even for unrelated tasks; a flat 0.90 like
//      the closed guard would both miss real 0.82–0.88 paraphrases and risk
//      suppressing genuine work. So: auto-dedup only above a very safe floor,
//      and hand the ambiguous mid band to a one-shot model adjudication that
//      keeps different people / sub-steps / deliverables distinct (Alan vs
//      Alec must never merge).
...
const OPEN_TASK_SEMANTIC_AUTO = 0.92;    // auto-dedup: unambiguous paraphrase
const OPEN_TASK_SEMANTIC_REVIEW = 0.82;  // shortlist floor for model adjudication
...
async function openTaskDuplicateMatch(user, action, {
  ...
  // Layer 2: semantic match over open_task embeddings, verified still live-open.
  try {
    const search = semanticSearchFn || require('./retrieval').semanticSearch;
    const hits = await search(user, `${title}\n${action.evidence || ''}`, 5, {
      sourceKinds: ['open_task'],
      minScore: OPEN_TASK_SEMANTIC_REVIEW,
    });
    for (const top of hits || []) {
      const task = openById.get(top.source_id); // skip stale vectors of since-closed tasks
      if (!task) continue;
      if (top.score >= OPEN_TASK_SEMANTIC_AUTO) {
        return { taskId: task.id, matchedTitle: task.title, method: 'semantic', score: top.score };
      }
      // Mid band: let the model decide, protecting distinct people/steps.
