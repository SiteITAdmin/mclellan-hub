...
// This is intentionally narrower than ordinary review recovery. Triage can
// identify the right candidate while citing a paraphrase; only a fresh,
// canonicalised projection with an exact source quote may replace that
// automatic evidence gate. The old review itself remains non-creatable.
function isRetryableProjectionEvidenceReview(outcome, user, evidence, identity, actionKey) {
  ...
  return outcome?.disposition === 'review'
    ...
    && exactEvidenceSpan(identity?.span)
    && isNonCreatablePayload(payload)
    && !outcome.task_id
    && !outcome.event_id
    && !outcome.resolved_at
...
async function projectOneAction(user, evidence, action, identity, entities, {
  ...
  if (stableTask) {
    ...
    if (existing && !OUTCOME_TERMINAL.has(existing.disposition)) {
      ...
      if (existing.disposition !== 'review' || !isAutomaticProjectionRecoveryReview(existing)) {
      ...
      const claim = claimPendingSideEffect(user, evidence, actionKey, {
        action, identity, phase: 'task', taskId, approved: false,
      });
      if (!claim.claimed) {
...
async function runCrmKnowledgeSource(user, {
  ...
  if (!process.env.OPENROUTER_API_KEY && !dependencies.requestModelObject) {
    return { ...empty, reason: 'OPENROUTER_API_KEY not set' };
  }
