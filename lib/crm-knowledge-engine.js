...
// Compact task history for projection. Completed/deleted/wrong stay — they are
// authoritative human decisions the prompt must not recreate — but open tasks
// keep fuller detail and closed tasks are title-only so every call does not
// re-pay ~17k tokens of notes history.
function taskHistoryForActionProjection(user) {
  const rows = db.hub().prepare(`
    ...
    ORDER BY
      CASE
        WHEN deleted_at IS NULL AND status NOT IN ('completed', 'wrong') THEN 0
        ELSE 1
      END ASC,
  ...
  return rows.map(task => {
    ...
    // Closed states: identity + title only. Notes and project are noise for
    // the anti-recreation guard and dominated the previous prompt cost.
    return `- [${state}] TASK_ID:${task.id} ${String(task.title || '').slice(0, 120)}`;
  }).join('\n');
}
