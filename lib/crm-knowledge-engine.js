...
- Each projected action must copy the supplied candidate_key when it corresponds to a supplied candidate.
...
// `sourceReceipt` is intentionally a simple latest-row helper for legacy
// callers.  Recovery must instead find the newest receipt for this exact raw
// revision: a source can change and later return to an earlier revision.
...
function actionCreationBlockReason(action, identity) {
  ...
  if (!exactEvidenceSpan(identity?.span)) return 'non_exact_evidence_requires_review';
  return null;
}
...
function candidateForProjectedAction(action, candidates) {
  const explicit = String(action?.candidate_key || action?._candidate_key || '').trim();
  if (explicit) {
    const found = candidates.find(candidate => candidate.candidate_key === explicit);
    if (found) return found;
  }
  // A model occasionally omits candidate_key despite the projection contract.
  // Prefer the candidate's normalised ask over its quotation in that case: one
  // sentence may contain several different asks, all with the same exact
  // evidence span.  Matching quotation first would collapse those asks onto
  // the first candidate and make the others look omitted.
  const projectedSemantics = candidateSemantics(action);
  if (projectedSemantics) {
  ...
  if (evidence) {
    const evidenceMatches = candidates.filter(candidate => {
    ...
    // An evidence-only fallback is safe only when it has one unambiguous
    // target.  A shared exact quote must stay reviewable rather than silently
    // receiving another candidate's side-effect identity.
...
function actionIdentity(evidence, action, candidates, chunk) {
  ...
  const span = candidateEvidenceSpan || locateEvidenceSpan(evidence, action?.evidence || action?.action, chunk)
    || candidateSpan(evidence, chunk, action);
  return {
    candidate,
    span,
    action_key: candidate?.candidate_key || stableActionKey(evidence, span)
      || malformedCandidateKey(evidence, chunk, action, `projection:${String(action?.candidate_key || action?._candidate_key || '')}`),
  };
}
...
// Claiming is deliberately not an upsert.  Two workers may both reach a new
// source at once; only the one whose INSERT wins may make the irreversible
// provider call.  Every other worker sees the same pending row and fails
// closed until a locally cached stable side effect can be reconciled.
function insertPendingActionOutcome(user, evidence, actionKey, {
  action = {},
  identity = null,
  disposition,
  reason,
  ...
  `).run(
    ...
    title, evidenceText, span?.start ?? null, span?.end ?? null, span?.chunk_index ?? null, span?.chunk_id ?? null,
    action?.actionability || identity?.candidate?.actionability || null, Number(action?.confidence ?? identity?.candidate?.confidence) || 0,
    disposition, reason || null, taskId, JSON.stringify(payload || {}), ts, ts,
  );
...
function claimPendingSideEffect(user, evidence, actionKey, {
  ...
  const payload = outcomePayload(evidence, action, identity, taskId ? { task_id: taskId } : {});
  if (insertPendingActionOutcome(user, evidence, actionKey, {
...
async function projectOneAction(user, evidence, action, identity, entities, {
  ...
  // Only an exact source quotation with a concrete title can reach an
  // irreversible provider call.  Approximate fallback spans remain reviewable
  // evidence, including when a user later tries to accept the review row.
  ...
  if (sourceTask) {
    ...
    return { outcome, disposition: 'existing_open_task' };
  }
  ...
  if (decision.disposition === 'fyi') {
    ...
    return { outcome, disposition: 'fyi' };
  }
  ...
  if (stableTask) {
    if (taskState(stableTask) !== 'open') {
      const outcome = upsertActionOutcome(user, evidence, actionKey, {
        ...
        payload: outcomePayload(evidence, action, identity, { stable_task_id: taskId }),
      });
      return { outcome, disposition: 'review' };
    }
  } else {
    ...
    try {
      ...
      const outcome = upsertActionOutcome(user, evidence, actionKey, {
        ...
        payload: outcomePayload(evidence, action, identity, { error: err.message }),
      });
      return { outcome, disposition: 'error' };
    }
  }
  ...
  if (!event) {
    ...
    const outcome = upsertActionOutcome(user, evidence, actionKey, {
      ...
      payload: outcomePayload(evidence, action, identity, { task_id: taskId }),
    });
  ...
  if (!stableEvent) {
    if (assertSourceLeaseFn) assertSourceLeaseFn();
    const claim = claimPendingSideEffect(user, evidence, actionKey, {
      action, identity, phase: 'event', taskId, approved,
    });
    if (!claim.claimed) {
    ...
    try {
      ...
      const outcome = upsertActionOutcome(user, evidence, actionKey, {
        ...
        payload: outcomePayload(evidence, action, identity, { task_id: taskId, error: err.message }),
      });
      return { outcome, disposition: 'error' };
    }
  }
  ...
  const outcome = upsertActionOutcome(user, evidence, actionKey, {
    ...
    taskId, eventId,
    payload: outcomePayload(evidence, action, identity, { task_id: taskId, event_id: eventId }),
  });
...
async function createProjectedTasks(user, sourceKind, sourceId, actions, entities, {
  ...
  for (const action of actions || []) {
    ...
    const identity = actionIdentity(evidence, action, candidates, chunk);
    const result = await projectOneAction(user, evidence, action, identity, entities, {
      ...dependencies,
      approved,
    });
    tallyProjection(totals, result, action);
  }
  return totals;
}
  ...
  for (const [chunkId, group] of byChunk) {
    ...
    for (const batch of candidateBatches(group)) {
      try {
        ...
        const result = await createProjectedTasks(user, evidence.source_kind, evidence.source_id, matchedActions, entities, {
          evidence,
          candidates: batch,
          chunkById,
          dependencies,
...
async function runCrmKnowledgeEngine(user, { limit = 8, linkBudget = 12, dependencies = {} } = {}) {
  if (!process.env.OPENROUTER_API_KEY && !dependencies.requestModelObject) {
    return { considered: 0, triaged: 0, synthesised: 0, skipped: 0, errors: 0, reviews: 0, reason: 'OPENROUTER_API_KEY not set' };
  }
