...
async function processSourceClaimed(user, evidence, context, { retry = false, dependencies = {} } = {}) {
  ...
  // Source-wide duplicate classification was a table-era bottleneck: one
  // uncertain or hallucinated atom target could suppress every unrelated
  // action and claim in a complete email. Canonical per-claim synthesis and
  // atom upsert already perform the correct evidence-scoped deduplication, so
  // the source-wide gate is deliberately bypassed.
  const duplicateDecision = {
    decision: 'new',
    confidence: 1,
    reason: 'source_wide_duplicate_gate_removed_per_claim_synthesis_authoritative',
  };
