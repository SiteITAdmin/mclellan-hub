...
function sourceRows(user, sourceKind, { untriagedOnly = false } = {}) {
  ...
  if (sourceKind === 'email_summary') {
    ...
    const subjectSql = HUB_REPORT_SUBJECT_PREFIXES.map(() => 'subject LIKE ?').join(' OR ');
    const subjectLikes = HUB_REPORT_SUBJECT_PREFIXES.map(p => `${p}%`);
    const selfAuthoredSql = hubAddress
      ? `lower(COALESCE(from_email,'')) = ? OR ${subjectSql}`
      : subjectSql;
...
async function createProjectedTasks(user, sourceKind, sourceId, actions, entities) {
  ...
  return {
    created, skipped, errors, details,
      ...
      try {
        ...
        if (projected) {
          ...
          const eventsSummary = created.events?.created
            ...
            : '';
          writeReceipt(user, sourceKind, row.id, 'crm_action_projected', {
            status: (created.errors || created.events?.errors) ? 'error' : actions.length ? 'done' : 'skipped',
            summary: actions.length
              ? `${actions.length} source-backed action candidate(s); ${created.created} task(s) created, ${created.skipped} skipped.${eventsSummary}`
