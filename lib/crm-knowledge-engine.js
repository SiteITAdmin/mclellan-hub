...
const CRM_JOURNEY_PURPOSE = 'Every eligible CRM source is triaged; valuable evidence becomes provenance-backed compiled knowledge; actionable commitments become tasks; incomplete or failed source journeys are repaired.';
const JOURNEY_STAGES = [
  'crm_source_triage',
  'crm_duplicate_reviewed',
  'crm_action_projected',
  'crm_knowledge_synthesised',
];
const JOURNEY_WINDOW_SECONDS = 30 * 86400;
const JOURNEY_STAGE_STALL_SECONDS = 30 * 60;
const JOURNEY_UNSTARTED_STALL_SECONDS = 24 * 3600;
...
function candidateSources(user, limit) {
  ...
  for (const kind of ENGINE_SOURCE_KINDS) {
    for (const row of sourceRows(user, kind)) {
      ...
      const priorTriage = sourceReceipt(user, kind, row.id, 'crm_source_triage');
      // An error receipt is evidence of an unfinished journey, not proof that
      // the source was handled. Replay only errors the existing engine can
      // plausibly repair; permanent HTTP/model-config failures stay visible for
      // the Consigliere and repair venue instead of burning calls forever.
      if (priorTriage && (priorTriage.status !== 'error' || !repairDisposition(priorTriage.summary).repairable)) continue;
...
function receiptsByStage(receipts = []) {
  const map = {};
...
function repairDisposition(message = '') {
  const text = String(message || '');
  if (/OpenRouter\s+(?:400|401|402|403|404|405|410|422)\b/i.test(text)) {
    return { repairable: false, disposition: 'config', explanation: 'Permanent OpenRouter response requires model, credentials, request, or credit correction.' };
  }
  if (/was not valid JSON/i.test(text)) {
    return { repairable: false, disposition: 'model_config', explanation: 'Exhausted malformed JSON is routed to model/config review by the repair venue.' };
  }
  return { repairable: true, disposition: 'retry', explanation: 'The existing CRM engine can safely replay this source-backed journey.' };
}

function issue(identity, stage, reason) {
  return { ...identity, stage, reason, ...repairDisposition(reason) };
}

function sourceJourneyIssue({ sourceKind, row, receipts = [], synthesisProcessed = false, nowTs = now() } = {}) {
  const byStage = receiptsByStage(receipts);
  const triage = byStage.crm_source_triage;
  const age = nowTs - Number(row?.ts || 0);
  const identity = { source_kind: sourceKind, source_id: String(row?.id || ''), row };

  if (!triage && synthesisProcessed) return null;
  if (!triage) {
    return row?.ts && age > JOURNEY_UNSTARTED_STALL_SECONDS
      ? issue(identity, 'crm_source_triage', 'Eligible source remained untriaged beyond the intake window.')
      : null;
  }
  if (triage.status === 'error') {
    return issue(identity, 'crm_source_triage', `Source triage failed: ${triage.summary || 'unknown error'}`);
  }

  const decision = (() => {
    try { return JSON.parse(triage.payload || '{}'); } catch (_) { return {}; }
  })();
  const triageAge = nowTs - Number(triage.created_at || 0);
  const duplicate = byStage.crm_duplicate_reviewed;
  const action = byStage.crm_action_projected;
  const synthesis = byStage.crm_knowledge_synthesised;

  if (duplicate?.status === 'error') {
    return issue(identity, 'crm_duplicate_reviewed', `Duplicate/supersession review failed: ${duplicate.summary || 'unknown error'}`);
  }
  if (action?.status === 'error') {
    return issue(identity, 'crm_action_projected', `Action projection failed: ${action.summary || 'unknown error'}`);
  }
  if (synthesis?.status === 'error') {
    return issue(identity, 'crm_knowledge_synthesised', `Knowledge synthesis failed: ${synthesis.summary || 'unknown error'}`);
  }

  const candidates = Array.isArray(decision.candidate_actions) ? decision.candidate_actions : [];
  if (candidates.length && !action && triageAge > JOURNEY_STAGE_STALL_SECONDS) {
    return issue(identity, 'crm_action_projected', `${candidates.length} action candidate(s) were identified but never projected.`);
  }
  if (decision.should_synthesise && !synthesis && !synthesisProcessed && triageAge > JOURNEY_STAGE_STALL_SECONDS) {
    return issue(identity, 'crm_knowledge_synthesised', 'Source was approved for synthesis but no compiled-knowledge outcome followed.');
  }
  return null;
}

function collectJourneyEvidence(user, { nowTs = now(), windowSeconds = JOURNEY_WINDOW_SECONDS } = {}) {
  const hub = db.hub();
  const cutoff = nowTs - windowSeconds;
  const sources = [];
  for (const kind of ENGINE_SOURCE_KINDS) {
    for (const row of sourceRows(user, kind)) {
      if (Number(row.ts || 0) >= cutoff) sources.push({ kind, row });
    }
  }

  let receiptRows = [];
  try {
    const kindPlaceholders = ENGINE_SOURCE_KINDS.map(() => '?').join(',');
    const stagePlaceholders = JOURNEY_STAGES.map(() => '?').join(',');
    receiptRows = hub.prepare(`
      SELECT * FROM knowledge_receipts
      WHERE user = ? AND source_kind IN (${kindPlaceholders})
        AND stage IN (${stagePlaceholders}) AND created_at >= ?
      ORDER BY created_at DESC, rowid DESC
    `).all(user, ...ENGINE_SOURCE_KINDS, ...JOURNEY_STAGES, cutoff);
  } catch (_) { receiptRows = []; }
  const receipts = new Map();
  for (const receipt of receiptRows) {
    const key = `${receipt.source_kind}:${receipt.source_id}`;
    if (!receipts.has(key)) receipts.set(key, []);
    receipts.get(key).push(receipt);
  }

  let processedRows = [];
  try {
    processedRows = hub.prepare('SELECT source_kind, source_id FROM synthesis_state WHERE user = ?').all(user);
  } catch (_) { processedRows = []; }
  const processed = new Set(processedRows.map(row => `${row.source_kind}:${row.source_id}`));
  return { sources, receipts, processed };
}

function auditCrmKnowledgeJourneys(user = 'douglas', options = {}) {
  const nowTs = options.nowTs || now();
  let evidence;
  try { evidence = collectJourneyEvidence(user, { ...options, nowTs }); }
  catch (err) { return { purpose: CRM_JOURNEY_PURPOSE, issues: [], repairableIssues: [], escalations: [], error: err.message }; }
  const issues = [];
  for (const source of evidence.sources) {
    const key = `${source.kind}:${source.row.id}`;
    const issue = sourceJourneyIssue({
      sourceKind: source.kind,
      row: source.row,
      receipts: evidence.receipts.get(key) || [],
      synthesisProcessed: evidence.processed.has(key),
      nowTs,
    });
    if (issue) issues.push(issue);
  }
  return {
    purpose: CRM_JOURNEY_PURPOSE,
    considered: evidence.sources.length,
    issues,
    repairableIssues: issues.filter(item => item.repairable),
    escalations: issues.filter(item => !item.repairable),
  };
}

async function repairCrmKnowledgeJourneys(user = 'douglas', { limit = 4, linkBudget = 12 } = {}) {
  if (!process.env.OPENROUTER_API_KEY) {
    return { purpose: CRM_JOURNEY_PURPOSE, attempted: 0, resolved: 0, unresolved: 0, reason: 'OPENROUTER_API_KEY not set' };
  }
  const audit = auditCrmKnowledgeJourneys(user);
  if (audit.error) throw new Error(`CRM journey audit failed: ${audit.error}`);
  const targets = audit.repairableIssues.slice(0, limit);
  if (!targets.length) return { purpose: CRM_JOURNEY_PURPOSE, attempted: 0, resolved: 0, unresolved: 0 };
  ...
  for (const target of targets) {
    writeReceipt(user, target.source_kind, target.source_id, 'agent:crm_journey_supervisor', {
      status: 'warn',
      summary: `CRM journey repair starting at ${target.stage}: ${target.reason}`,
      payload: { purpose: CRM_JOURNEY_PURPOSE, target: { stage: target.stage, reason: target.reason } },
    });
    const result = await processSource(user, target.source_kind, target.row, context);
    const latest = auditCrmKnowledgeJourneys(user).issues.find(issue =>
      issue.source_kind === target.source_kind && issue.source_id === target.source_id
    );
    const resolved = !latest;
    writeReceipt(user, target.source_kind, target.source_id, 'agent:crm_journey_supervisor', {
      status: resolved ? 'pass' : 'fail',
      summary: resolved
        ? `CRM journey repair completed from ${target.stage}.`
        : `CRM journey remains unmet after repair: ${latest.reason}`,
      payload: { purpose: CRM_JOURNEY_PURPOSE, before: { stage: target.stage, reason: target.reason }, result, after: latest || null },
    });
    outcomes.push({ source_kind: target.source_kind, source_id: target.source_id, resolved, before: target.reason, after: latest?.reason || null });
  }
  return {
    purpose: CRM_JOURNEY_PURPOSE,
...
module.exports = {
  ...
  auditCrmKnowledgeJourneys,
  repairCrmKnowledgeJourneys,
  sourceJourneyIssue,
  CRM_JOURNEY_PURPOSE,
  writeReceipt,
  sourceReceipt,
