...
// Claude Code CLI (Opus 4.8, via scripts/repair/claude-runner.mjs) against it with bounded instructions, drives
...
const REPAIR_AGENT_FALLBACK_MODEL = 'claude-opus-4-8';
...
// snapshotDir is a test seam only. Production always reads the real prod
// snapshot; a test passes a controlled dir so the default-off contract can be
// proven without depending on whatever snapshot happens to sit on the box the
// venue runs on (the Mac mini always has one).
function autoDeployEnabled({ snapshotDir = SNAPSHOT_DIR } = {}) {
  if (process.env[AUTODEPLOY_ENV] === '1') return true;
  try {
    const Database = require('better-sqlite3');
    const db = new Database(path.join(snapshotDir, 'hub.db'), { readonly: true, fileMustExist: true });
...
const ROLLUP_REMOTE_PATH = '/app/data/repair-rollup.json';

// The venue runs on the Mac mini against a snapshot and never writes to prod
// knowledge_receipts, so without this its activity (a PR opened, a fix
// deployed) would only ever reach Douglas via its own separate email — it
// would never show up in the daily VPS report. Pushed independently of
// whether a deploy happened, so PR-only results (the common case with
// auto-deploy off) still surface tomorrow. Best-effort: if the scp fails, the
// run's own email is still the record of what happened — this file existing
// is a nice-to-have for the daily report, not the source of truth.
function pushRollupToVps(entries) {
  const tmpFile = path.join(os.tmpdir(), `hub-repair-rollup-${Date.now()}.json`);
  fs.writeFileSync(tmpFile, JSON.stringify({ generated_at: new Date().toISOString(), entries }, null, 2));
  const res = spawnSync(
    'scp',
    ['-o', 'StrictHostKeyChecking=accept-new', tmpFile, `${DEPLOY_HOST}:${ROLLUP_REMOTE_PATH}`],
    { encoding: 'utf8', timeout: SSH_TIMEOUT_MS },
  );
  try { fs.unlinkSync(tmpFile); } catch (_) { /* scratch */ }
  return { ok: res.status === 0, error: res.status === 0 ? null : (res.stderr || '').slice(0, 500) };
}
...
function resolveRepairModel(modelId) {
  const normalized = String(modelId || '').trim();
  if (normalized === 'opus') return 'claude-opus-4-8';
  if (/^claude-opus/.test(normalized)) return normalized;
  throw new Error(`repair_agent must be an Opus model for the local Claude Code CLI (received ${modelId || 'empty'})`);
}
...
function createWorktree(reproducerId) {
  ...
  // the diff, and the repair agent never sees a byte of Douglas's life).
...
function checkScope(reproducer, files) {
  ...
  if (!files.length) return { ok: false, reason: 'the repair agent made no changes' };
...
// Run one Claude Code CLI session in a child process. The outer five-minute
// kill is the hard deadline. Claude Code CLI subscription execution has no
// per-turn dollar usage feed, so the session is bounded by that timeout; final
// usage cost is recorded after the session rather than treated as a real-time
// brake.
function runRepairSession({ worktree, reproducer, modelId, feedback }) {
  return new Promise((resolve) => {
    ...
    const runner = path.join(ROOT, 'scripts', 'repair', 'claude-runner.mjs');
...
function openPullRequest(reproducer, branch, verification, session) {
  ...
  fs.writeFileSync(bodyFile, [
    ...
    `- Execution: Claude Code CLI (Opus 4.8) subscription ($${session.costUsd || 0}, ${session.turns} turns, attempt ${session.attempt}/${MAX_REPAIR_ATTEMPTS})`,
    '',
    '🤖 Generated by the Hub self-repair venue (Claude Code CLI Opus 4.8 coding agent, human-gated)',
  ].join('\n'));
...
async function notifySuccess({ reproducer, branch, prUrl, verification, session, files }) {
  ...
  return sendRepairEmail({
    ...
    text: [
      ...
      gateLines,
      '',
      `Execution: Claude Code CLI (Opus 4.8) subscription ($${session.costUsd || 0}; attempt ${session.attempt}/${MAX_REPAIR_ATTEMPTS}, ${session.turns} turns, model ${session.modelId})`,
      '',
...
async function notifyFailure({ reproducer, reason, attempts, costUsd }) {
  return sendRepairEmail({
    ...
    text: [
      ...
      `Error class: ${reproducer.error_class}`,
      `Subsystem: ${reproducer.source.capo} (${reproducer.source.job_type})`,
      `Attempts: ${attempts}/${MAX_REPAIR_ATTEMPTS}, Claude Code CLI (Opus 4.8) subscription execution ($${costUsd})`,
      `Last failure: ${reason}`,
      '',
  ...
  if (health.ok) {
    const email = await sendRepairEmail({
      ...
      text: [
        ...
        '',
        `Execution: Claude Code CLI (Opus 4.8) subscription ($${session.costUsd || 0}; attempt ${attempt}/${MAX_REPAIR_ATTEMPTS}, model ${session.modelId})`,
        '',
...
async function repairOne(reproducer, { modelId = null } = {}) {
  ...
  const chosenModel = resolveRepairModel(modelId || getSnapshotModelId(REPAIR_AGENT_FEATURE, REPAIR_AGENT_FALLBACK_MODEL));
  ...
  try {
    ...
    for (let attempt = (reproducer.repair_attempts || 0) + 1; attempt <= MAX_REPAIR_ATTEMPTS; attempt++) {
      ...
      const session = await runRepairSession({ worktree, reproducer, modelId: chosenModel, feedback });
      totalCost += session.costUsd || 0;
      session.modelId = chosenModel;
      session.attempt = attempt;

      if (session.status !== 'done') {
        lastReason = `repair session ${session.status}: ${session.error || ''}`;
        feedback = null;
      ...
      if (!verification.ok) {
        ...
        writeRepairReceipt({
          ...
          payload: {
            ...
            execution: 'claude_cli_subscription',
