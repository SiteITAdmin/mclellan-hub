...
// reproducer, creates a throwaway git worktree, runs Pi (via
// scripts/repair/pi-runner.mjs) against it with bounded instructions, drives
...
const { refererForTaskCode, titleForTaskCode, TASK_CODES } = require('./openrouter-attribution');
...
const REPAIR_COST_CAP_USD = 2.0;
const MAX_FILES_CHANGED = 3;

const REPAIR_AGENT_FEATURE = 'repair_agent';
const REPAIR_AGENT_FALLBACK_MODEL = 'google/gemini-2.5-pro-preview';
...
// OpenRouter pricing for the chosen model, in USD per million tokens (pi's
// cost units). Failure returns null and the runner falls back to an
// expensive estimate so the cost brake stops early rather than never.
async function fetchModelPricing(modelId) {
  try {
    ...
    return {
      ...
      cacheRead: Number(model.pricing.input_cache_read || model.pricing.prompt) * 1e6,
      cacheWrite: Number(model.pricing.input_cache_write || 0) * 1e6,
      contextWindow: Number(model.context_length) || undefined,
    };
  } catch (_) {
    return null;
  }
...
function createWorktree(reproducerId) {
  ...
  // the diff, and Pi never sees a byte of Douglas's life).
...
function checkScope(reproducer, files) {
  ...
  if (!files.length) return { ok: false, reason: 'Pi made no changes' };
...
function buildSystemPrompt() {
  return [
    'You are the McLellan Hub self-repair agent. You fix ONE narrow, reproduced runtime error',
...
// Run one Pi session in a child process. The 5-minute timeout is enforced
// with a kill; the $2 cost cap is enforced inside the runner from the event
// stream (and reported back here).
function runPiSession({ worktree, reproducer, modelId, pricing, feedback }) {
  return new Promise((resolve) => {
    const jobFile = path.join(os.tmpdir(), `hub-repair-job-${reproducer.id}-${Date.now()}.json`);
    fs.writeFileSync(jobFile, JSON.stringify({
      worktree,
      modelId,
      costCapUsd: REPAIR_COST_CAP_USD,
      pricing,
      contextWindow: pricing?.contextWindow,
      // Attribution only — pi injects Authorization from the provider config.
      headers: {
        'HTTP-Referer': refererForTaskCode(TASK_CODES.REPAIR_AGENT),
        'X-OpenRouter-Title': titleForTaskCode(TASK_CODES.REPAIR_AGENT),
        'X-Title': titleForTaskCode(TASK_CODES.REPAIR_AGENT),
      },
      systemPrompt: buildSystemPrompt(),
      prompt: buildUserPrompt(reproducer, feedback),
    }, null, 2));

    const runner = path.join(ROOT, 'scripts', 'repair', 'pi-runner.mjs');
    ...
    child.on('close', () => {
      ...
      if (timedOut) {
        resolve({ status: 'timeout', costUsd: result?.costUsd || lastCost(events), turns: result?.turns || 0, events, error: `killed after ${REPAIR_TIMEOUT_MS}ms` });
      } else if (result) {
        resolve({ ...result, events });
      } else {
        resolve({ status: 'error', costUsd: lastCost(events), turns: 0, events, error: stderrBuf.slice(-1000) || 'runner produced no result' });
      }
    });
  });
}

function lastCost(events) {
  for (let i = events.length - 1; i >= 0; i--) {
    if (typeof events[i].costUsd === 'number') return events[i].costUsd;
  }
  return 0;
}
...
function openPullRequest(reproducer, branch, verification, session) {
  ...
  fs.writeFileSync(bodyFile, [
    ...
    `- Cost: $${session.costUsd} (${session.turns} turns, attempt ${session.attempt}/${MAX_REPAIR_ATTEMPTS})`,
    '',
    '🤖 Generated by the Hub self-repair venue (Pi coding agent, human-gated)',
  ].join('\n'));
...
async function notifySuccess({ reproducer, branch, prUrl, verification, session, files }) {
  ...
  return sendRepairEmail({
    ...
    text: [
      ...
      gateLines,
      '',
      `Cost: $${session.costUsd} (attempt ${session.attempt}/${MAX_REPAIR_ATTEMPTS}, model ${session.modelId})`,
      '',
...
async function notifyFailure({ reproducer, reason, attempts, costUsd }) {
  return sendRepairEmail({
    ...
    text: [
      ...
      `Error class: ${reproducer.error_class}`,
      `Subsystem: ${reproducer.source.capo} (${reproducer.source.job_type})`,
      `Attempts: ${attempts}/${MAX_REPAIR_ATTEMPTS}, total cost $${costUsd}`,
      `Last failure: ${reason}`,
      '',
...
async function repairOne(reproducer, { modelId = null } = {}) {
  ...
  const chosenModel = modelId || getSnapshotModelId(REPAIR_AGENT_FEATURE, REPAIR_AGENT_FALLBACK_MODEL);
  const pricing = await fetchModelPricing(chosenModel);
  ...
  try {
    ...
    for (let attempt = (reproducer.repair_attempts || 0) + 1; attempt <= MAX_REPAIR_ATTEMPTS; attempt++) {
      ...
      const session = await runPiSession({ worktree, reproducer, modelId: chosenModel, pricing, feedback });
      totalCost += session.costUsd || 0;
      session.modelId = chosenModel;
      session.attempt = attempt;

      if (session.status !== 'done') {
        lastReason = `Pi session ${session.status}: ${session.error || ''}`;
        feedback = null;
        // Timeout/cost-abort leaves the worktree half-edited: reset it.
