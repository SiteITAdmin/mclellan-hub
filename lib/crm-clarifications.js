...
function clarificationKey(item = {}) {
  switch (item.kind) {
    ...
    // Keyed on the page's byte revision, so a corrected transcription stays
    // answered; only a genuinely changed page asks again.
    case KINDS.NOTE_TRANSCRIPTION:
      return hash(['nt', item.file || '', item.revision || '']);
    default:
...
// The Drive ingest names files `<driveFileId>-<original name>`. The id is noise
// to a human checking a page, so show the name he gave the note.
function displayNoteName(name = '') {
  return String(name)
    // App captures are `boox-<uuid>-<title>`; match the uuid exactly.
    .replace(/^boox-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}-/i, '')
    // Drive ingest prefixes `<fileId>-`. Lazy and bounded: a Drive id contains
    // hyphens itself, so a greedy match eats the real name too
    // (…-Notebook-5.pdf became "5.pdf").
    .replace(/^[A-Za-z0-9_-]{25,50}?-/, '')
    .replace(/\.ocr\.json$/, '');
}

/**
 * Handwritten pages the hub has read but cannot trust. Apple Vision is reliable
 * on print and numbers and unreliable on cursive, so a transcription is never
 * knowledge on its own — it is a draft for Douglas to correct here, alongside a
 * link to the original page so he can check it against what he actually wrote.
 */
function noteTranscriptionQuestions() {
  let transcriptions = [];
  try {
    transcriptions = require('./boox-note-ocr').listTranscriptions({ limit: 60 });
  } catch (_) {
    return []; // vault absent (e.g. the VPS before a sync) — not a question
  }
  return transcriptions
    // A page he has already read and fixed is settled; only its bytes changing
    // (a re-export of the same note) can raise it again.
    .filter(record => !record.correctedAt)
    .map(record => ({
    kind: KINDS.NOTE_TRANSCRIPTION,
    file: record.file,
    revision: record.revision,
    document_name: displayNoteName(record.name),
    // Real link back to the page, so the transcription can be checked against
    // the handwriting rather than trusted blind.
    document_url: `/crm/notes/page?file=${encodeURIComponent(record.file)}`,
    question: `Is this transcription of "${displayNoteName(record.name)}" right?`,
    transcription: record.text || '',
    doubtful: (record.pages || []).flatMap(page => page.doubtful || []).slice(0, 20),
    line_count: record.lineCount || 0,
    low_confidence_lines: record.lowConfidenceLines || 0,
    doubt_ratio: record.doubtRatio ?? 1,
    page_count: record.pageCount || 0,
    truncated: Boolean(record.truncated),
    recognised_at: record.recognisedAt || null,
  }));
}
...
function groupedClarifications(user = 'douglas', { limit = 0 } = {}) {
  ...
  const notes = items.filter(item => item.kind === KINDS.NOTE_TRANSCRIPTION);
  const shownItems = [...take(attribution), ...take(notes), ...take(meeting), ...take(owners), ...take(aliases)];
  attachContext(user, shownItems);
  return {
    total: items.length,
    shown: shownItems.length,
    counts: { attribution_corrections: attribution.length, note_transcriptions: notes.length, meeting_questions: meeting.length, task_owners: owners.length, person_aliases: aliases.length },
    note_transcriptions: take(notes),
