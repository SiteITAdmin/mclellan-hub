...
handlers.email_process = async () => {
  const { processNewEmails, processSentEmails } = require('./email-processor');
  for (const user of BRIEFING_USERS()) {
    ...
    await processSentEmails(user).catch(err =>
      console.error(`[jobs] email_process_sent error (${user}):`, err.message));
  }
...
handlers.flight_refresh = async (payload) => {
  ...
  try {
    ...
    // Push notable status changes to Hermes — lazy require to avoid circular dep with reminders
    const statusChanged = status !== flight.status;
    const newDep = actualDep && !flight.actual_dep;
    const newArr = actualArr && !flight.actual_arr;
    if (statusChanged || newDep || newArr) {
      let chatMsg = null;
      if (newArr) chatMsg = `✅ *${flightNumber}* landed — ${actualArr}${scheduledArr ? ` (sched ${scheduledArr})` : ''}`;
      else if (newDep) chatMsg = `✈ *${flightNumber}* departed — ${actualDep}${scheduledDep ? ` (sched ${scheduledDep})` : ''}`;
      else if (statusChanged) chatMsg = `✈ *${flightNumber}* ${flightDate}: ${status}`;
      if (chatMsg) {
        const { deliverChat } = require('./reminders');
        deliverChat(user, chatMsg).catch(e => console.warn('[jobs] flight chat push:', e.message));
      }
    }
  } catch (err) {
