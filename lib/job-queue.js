...
// Outlook (Microsoft 365) work email ingestion — re-enqueues in 15 minutes.
// Quietly idles until an admin connects a Microsoft account at /admin/microsoft.
handlers.outlook_email_process = async () => {
  const { isOutlookSyncEnabled, processOutlookMail } = require('./outlook-processor');
  for (const user of BRIEFING_USERS()) {
    if (!isOutlookSyncEnabled(user)) continue;
    await processOutlookMail(user).catch(err =>
      console.error(`[jobs] outlook_email_process error (${user}):`, err.message));
  }
  scheduleJob('outlook_email_process', {}, now() + 15 * 60, 'self');
};

// Outlook calendar sync — every 30 minutes so same-day meeting changes reach
// the work brief and meeting prep, unlike the once-daily Google calendar sync.
handlers.outlook_calendar_sync = async () => {
  const { isOutlookSyncEnabled, syncOutlookCalendar } = require('./outlook-processor');
  for (const user of BRIEFING_USERS()) {
    if (!isOutlookSyncEnabled(user)) continue;
    await syncOutlookCalendar(user).catch(err =>
      console.error(`[jobs] outlook_calendar_sync error (${user}):`, err.message));
  }
  scheduleJob('outlook_calendar_sync', {}, now() + 30 * 60, 'self');
};
...
// slower safety net.
handlers.crm_knowledge_engine = async () => {
...
// Cross-entity synthesis — nightly at 04:15 Dublin. Reads all active atoms
// across every entity and writes back insight atoms (patterns, workflows,
// connections, gaps). subject_kind = 'insight'.
handlers.cross_entity_synthesis = async () => {
  ...
  scheduleJob('cross_entity_synthesis', {}, epochAtNextDublin(4, 15), 'self');
};
...
// Interest radar — daily at 05:45 Dublin, before the work brief goes out, so
// "on your radar" reflects the latest meetings and calendar.
...
function seedJobs() {
  ...
  if (!hasPending('outlook_email_process'))
    scheduleJob('outlook_email_process', {}, now() + 60, 'seed');

  if (!hasPending('outlook_calendar_sync'))
    scheduleJob('outlook_calendar_sync', {}, now() + 90, 'seed');

  if (!hasPending('google_tasks_sync'))
    scheduleJob('google_tasks_sync', {}, now() + 30, 'seed');

  if (!hasPending('crm_knowledge_engine'))
    scheduleJob('crm_knowledge_engine', {}, now() + 105, 'seed');
  ...
  if (!hasPending('cross_entity_synthesis'))
    scheduleJob('cross_entity_synthesis', {}, now() + 420, 'seed');
