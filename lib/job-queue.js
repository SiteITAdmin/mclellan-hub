...
// Purpose-aware CRM recovery. The Capo/remediation loop dispatches this only
// when source evidence shows a journey stopped after triage, duplicate review,
// action projection, or synthesis. The existing CRM engine performs the work.
handlers.crm_knowledge_repair = async (payload = {}) => {
  const { repairCrmKnowledgeJourneys } = require('./crm-knowledge-engine');
  const user = payload.user || 'douglas';
  const result = await repairCrmKnowledgeJourneys(user, { limit: payload.limit || 4 });
  console.log(`[jobs] crm_knowledge_repair ${user}:`, result);
};

// Purpose-aware LinkedIn recovery. This reuses runPipeline/resumePost through
// the journey supervisor; it never publishes externally.
handlers.linkedin_journey_repair = async (payload = {}) => {
  const { repairLinkedInJourneys } = require('./linkedin-journey');
  const user = payload.user || 'douglas';
  const result = await repairLinkedInJourneys(user, {
    limit: payload.limit || 3,
    onStatus: message => console.log(`[linkedin-journey] ${message}`),
  });
  console.log(`[jobs] linkedin_journey_repair ${user}:`, result);
};
