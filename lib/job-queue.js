...
// Briefing schedules — check every hour, generate+send any due schedules
handlers.briefing_schedule_run = async () => {
  ...
  scheduleJob('briefing_schedule_run', {}, now() + 60 * 60, 'self');
};

// URL watchlist — poll due feeds, re-enqueue in 15 minutes
handlers.watchlist_poll = async () => {
  const { processAllDueFeeds } = require('./watchlist');
  for (const user of BRIEFING_USERS()) {
    await processAllDueFeeds(user).catch(err =>
      console.error(`[jobs] watchlist_poll error (${user}):`, err.message));
  }
