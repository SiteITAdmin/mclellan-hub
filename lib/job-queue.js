...
const { extractFlightState, actualsMatchSchedule } = require('./flight-status');
...
handlers.google_tasks_sync = async () => {
  const { syncTasks, promoteDueStartTasks } = require('./google-tasks');
  for (const user of BRIEFING_USERS()) {
    try {
    ...
    // Promote start-dated tasks off the Later list once they come within the
    // ~2-week window — the mechanism that puts them back in the working list.
    try {
      const promo = await promoteDueStartTasks(user);
      if (promo?.promoted) console.log(`[jobs] google_tasks_sync ${user}: promoted ${promo.promoted} start-dated task(s)`);
    } catch (err) {
      console.error(`[jobs] start-date promotion error (${user}):`, err.message);
    }
  }
...
// content on each pass, so no write-path edits are needed. Douglas has reserved
// the Mac only from 23:00–02:30 Dublin for this historical re-index; no
// backfill batch starts outside that window.
...
// size is therefore the throughput lever. Do not start after 01:45: that leaves
// the measured pass budget to finish before the 02:30 window closes.
const EMBED_BACKFILL_WINDOW_START = 23 * 60;
const EMBED_BACKFILL_LAST_START = 1 * 60 + 45;
const HISTORIC_COMMUNICATION_SOURCE_KINDS = Object.freeze([
  'email_summary',
  'messaging_message',
]);

function dublinMinutes(at = new Date()) {
  try {
...
function isEmbedBackfillStartWindow(at = new Date()) {
  const minutes = dublinMinutes(at);
  return minutes >= EMBED_BACKFILL_WINDOW_START || minutes < EMBED_BACKFILL_LAST_START;
}

function nextEmbedBackfillRun(at = new Date(), {
  requeueNow = false,
  epochAtNextDublinFn = null,
} = {}) {
  if (requeueNow && isEmbedBackfillStartWindow(at)) return Math.floor(at.getTime() / 1000) + 60;
  const { epochAtNextDublin } = require('./reminders');
  return (epochAtNextDublinFn || epochAtNextDublin)(23, 0);
}
...
handlers.embed_backfill = async () => {
  ...
  if (!isEmbedBackfillStartWindow()) {
    scheduleJob('embed_backfill', {}, nextEmbedBackfillRun(), 'self');
    return;
  }
  ...
  for (const user of BRIEFING_USERS()) {
    try {
      const res = await backfillEmbeddings(user, {
        limit,
        sourceKinds: HISTORIC_COMMUNICATION_SOURCE_KINDS,
        compiledSources: [],
      });
  ...
  if (!alreadyQueued) {
    scheduleJob('embed_backfill', {}, nextEmbedBackfillRun(new Date(), { requeueNow: anyRemaining }), 'self');
  }
};
...
// Runs nightly ~03:00 Dublin. The dedicated crm_knowledge_engine scanner owns
// change-driven backlog recovery; this compatibility pass must not create a
// second rapid retry chain over the same sources.
handlers.synthesis_run = async () => {
  ...
  for (const user of BRIEFING_USERS()) {
    ...
    // Collapse paraphrase duplicates in the review queue so a fact Douglas
    // already approved or rejected is not re-asked in a slightly different
    // wording. Fail-open: any embedding/model outage leaves the queue untouched.
    try {
      const { dedupProposedAtoms } = require('./atom-dedup');
      const dd = await dedupProposedAtoms(user);
      if (dd.collapsed) console.log(`[jobs] atom dedup ${user}:`, dd);
    } catch (err) {
      console.error(`[jobs] atom dedup error (${user}):`, err.message);
    }
  }
  const { epochAtNextDublin } = require('./reminders');
  scheduleJob('synthesis_run', {}, epochAtNextDublin(3, 0), 'self');
};

handlers.work_loop_guard = async () => {
  try {
    const { runWorkLoopGuard } = require('./work-loop-guard');
    const result = runWorkLoopGuard();
    if (result.actions.length) console.warn('[jobs] work_loop_guard contained:', result.actions);
  } finally {
    const pending = db.hub().prepare(
      "SELECT 1 FROM system_jobs WHERE type = 'work_loop_guard' AND status = 'pending' LIMIT 1",
    ).get();
    if (!pending) scheduleJob('work_loop_guard', {}, now() + 15 * 60, 'self');
  }
};
...
// Whole-source attribution reconciliation scans frequently but invokes Terra
// only for a new/changed meeting revision or changed downstream link state.
// One source per run keeps the Mac subscription worker available to ingest.
handlers.attribution_reconciliation = async () => {
  const { runAttributionReconciliation } = require('./attribution-reconciliation');
  let remaining = 0;
  let errors = 0;
  for (const user of BRIEFING_USERS()) {
    try {
      const res = await runAttributionReconciliation(user, { limit: 1 });
      remaining += res.remaining || 0;
      errors += res.errors || 0;
      if (res.processed || res.applied || res.reviews || res.errors) {
        console.log(`[jobs] attribution_reconciliation ${user}:`, res);
      }
    } catch (err) {
      errors += 1;
      console.error(`[jobs] attribution_reconciliation error (${user}):`, err.message);
    }
  }
  scheduleJob('attribution_reconciliation', {}, now() + (remaining && !errors ? 5 * 60 : 20 * 60), 'self');
};
...
// Boox reference planner — rebuilt and pushed to Drive nightly at 04:50 Dublin,
// after the synthesis jobs have refreshed compiled project knowledge, so the
// tablet wakes up holding the same picture the Hub has. Read-only artifact: it
// creates no tasks and writes no knowledge.
handlers.boox_planner_publish = async () => {
  const { runBooxPlannerPublish } = require('./boox-planner');
  const { reshufflePlannerTasks } = require('./task-calendar-planner');
  const { epochAtNextDublin } = require('./reminders');
  if (process.env.BOOX_PLANNER_PUBLISH !== '0') {
    // A day's page is written once and never rebuilt, so whatever is stranded
    // in the past at 04:50 would be printed stranded and stay that way. Rescue
    // those blocks first (the same self-anchoring reshuffle the button runs);
    // the planner module itself stays read-only. A reshuffle failure must not
    // cost Douglas the PDF, so it is logged and the build continues.
    try {
      const shuffled = await reshufflePlannerTasks('douglas', {});
      console.log(`[jobs] boox_planner_publish reshuffle: moved ${shuffled.moved.length}, late ${shuffled.lateCount}, cleared ${shuffled.removedCompleted.length}, failed ${shuffled.failed.length}`);
      if (shuffled.failed.length) {
        console.error('[jobs] boox_planner_publish reshuffle could not move:', shuffled.failed.map(move => `${move.taskId}:${move.error}`).join(', '));
      }
    } catch (err) {
      console.error('[jobs] boox_planner_publish reshuffle error:', err.message);
    }
    try {
      const state = await runBooxPlannerPublish('douglas');
      console.log(`[jobs] boox_planner_publish: ${state.folderPath}/${state.name} (${state.bytes} bytes, ${state.days} days)`);
    } catch (err) {
      console.error('[jobs] boox_planner_publish error:', err.message);
    }
  }
  scheduleJob('boox_planner_publish', {}, epochAtNextDublin(4, 50), 'self');
};
...
handlers.flight_backfill = async () => {
  ...
  for (const f of flights) {
    ...
    try {
      ...
      const actualsOk = actualsMatchSchedule(f.scheduled_dep, actualDep, f.scheduled_arr, actualArr);
      if (!actualsOk) {
        console.warn(`[jobs] backfill skipped implausible actuals for ${f.flight_number} ${f.flight_date}: scheduled ${f.scheduled_dep}→${f.scheduled_arr} vs live ${actualDep}→${actualArr}`);
      }
      ...
      hub.prepare(`
        ...
        WHERE id = ?
      `).run(
        actualsOk ? status : f.status,
        actualsOk ? actualDep : '', actualsOk ? actualDep : '',
        actualsOk ? actualArr : '', actualsOk ? actualArr : '',
        scheduledDep, scheduledDep, scheduledArr, scheduledArr,
        f.id
      );
      if (actualsOk && (actualDep || actualArr)) {
...
handlers.flight_refresh = async (payload) => {
  ...
  try {
    ...
    const actualsOk = actualsMatchSchedule(flight.scheduled_dep, actualDep, flight.scheduled_arr, actualArr);
    if (!actualsOk) {
      console.warn(`[jobs] flight_refresh skipped implausible actuals for ${flightNumber} ${flightDate}: scheduled ${flight.scheduled_dep}→${flight.scheduled_arr} vs live ${actualDep}→${actualArr}`);
    }
    resolved = actualsOk && state.resolved;
    ...
    hub.prepare(`
      ...
      WHERE id = ?
    `).run(
      actualsOk ? status : flight.status,
      actualsOk ? actualDep : '', actualsOk ? actualDep : '',
      actualsOk ? actualArr : '', actualsOk ? actualArr : '',
      scheduledDep, scheduledDep, scheduledArr, scheduledArr,
      flightId
    ...
    if (actualsOk && actualDep) console.log(`[jobs] ${flightNumber} ${flightDate}: departed ${actualDep}`);
    if (actualsOk && actualArr) console.log(`[jobs] ${flightNumber} ${flightDate}: arrived ${actualArr}, status=${status}`);
...
function seedJobs() {
  ...
  if (!hasPending('work_loop_guard'))
    scheduleJob('work_loop_guard', {}, now() + 120, 'seed');
  ...
  if (!hasPending('attribution_reconciliation'))
    scheduleJob('attribution_reconciliation', {}, now() + 390, 'seed');
  ...
  if (!hasPending('boox_planner_publish')) {
    const { epochAtNextDublin } = require('./reminders');
    scheduleJob('boox_planner_publish', {}, epochAtNextDublin(4, 50), 'seed');
  }
...
module.exports = {
  ...
  _test: {
    handlers,
    dublinMinutes,
    isEmbedBackfillStartWindow,
    nextEmbedBackfillRun,
