...
handlers.flight_backfill = async () => {
  if (!process.env.AERODATABOX_KEY) return;
  const hub = db.hub();
  ...
  const flights = hub.prepare(`
    SELECT * FROM flights
    WHERE (actual_dep = '' OR actual_arr = '')
      AND flight_date >= ?
      AND flight_number != ''
  ...
  for (const f of flights) {
    ...
    try {
      ...
      const actualDep   = hhmm(rec.departure?.revisedTime  || rec.departure?.runwayTime);
      const actualArr   = hhmm(rec.arrival?.revisedTime    || rec.arrival?.runwayTime);
  ...
  // Re-enqueue for 23:00 tonight so this runs daily
  const tomorrow23 = new Date();
  tomorrow23.setDate(tomorrow23.getDate() + 1);
  tomorrow23.setHours(23, 0, 0, 0);
  scheduleJob('flight_backfill', {}, Math.floor(tomorrow23.getTime() / 1000), 'self');
};
...
function seedJobs() {
  ...
  // Immediately backfill any flights missing actual times (runs once per boot)
