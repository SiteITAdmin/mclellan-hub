...
const FEATURE_TASK_CODES = Object.freeze({
  ...
  'newsletter_digest_briefing': TASK_CODES.NEWSLETTER_BRIEFING,
...
function openRouterHeaders(taskCode, {
  ...
  // OpenRouter credentials are never emitted. These headers only route local
  // hub-model:// transport and preserve task-code attribution in request_logs.
  void apiKey; void baseUrl;
  const code = normalizeTaskCode(taskCode);
  const title = titleForTaskCode(code);
  return {
    'Content-Type': contentType,
    'X-Hub-Task-Code': code,
    'X-Hub-Feature': feature || code,
    // Historical field names retained so older log readers stay compatible.
    // No Authorization / OpenRouter credentials are ever set.
    'HTTP-Referer': `https://hub-model.local/${code.toLowerCase()}`,
