...
const TRIAGE_FALLBACK_MODEL = 'google/gemini-2.5-flash';

// Model slots are configured in the PROD admin UI and travel to the Mac mini
// inside the snapshot — read them from there, never from the local dev DB.
// Key layout must match lib/settings.js exactly, hence the shared prefix.
...
async function adviseTriage(reproducer, verdict, { snapshotDir = SNAPSHOT_DIR } = {}) {
  ...
  const prompt = [
    'You are the Hub self-repair triage reviewer. A deterministic classifier marked a runtime',
  ...
  try {
    const resp = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      method: 'POST',
      headers: openRouterHeaders(TASK_CODES.REPAIR_TRIAGE),
      body: JSON.stringify({
        model: modelId,
    ...
    logUsageFromResponse({
      user: 'system',
      feature: TRIAGE_FEATURE,
      modelKey: 'repair-triage',
      fallbackModelId: modelId,
      data,
      taskCode: TASK_CODES.REPAIR_TRIAGE,
      durationMs: Date.now() - started,
    });
    const parsed = parseModelObject(
      data.choices?.[0]?.message?.content,
      { decision: 'confirm', reason: '' },
      'repair triage reviewer',
    );
