...
function createPlannerDayPdf(snapshot, date) {
  ...
  const tasks = events.filter(event => event.isTask);
  return new Promise((resolve, reject) => {
    ...
    do {
      ...
      scheduleY += 18;
      taskY += 18;
      if (!events.length) doc.fillColor('#706d80').font('Helvetica').fontSize(10).text('Nothing scheduled today.', margin, scheduleY);
      ...
      while (eventIndex < events.length) {
        const height = scheduleEventHeight(doc, events[eventIndex], scheduleWidth) + 7;
        if (scheduleY + height > bottom - 28 && scheduleY > margin + 102) break;
        drawScheduleEvent(doc, events[eventIndex], margin, scheduleY, scheduleWidth);
        scheduleY += height;
        eventIndex += 1;
      }
      ...
      page += 1;
    } while (eventIndex < events.length || taskIndex < tasks.length || !page);
    doc.end();
  });
}

module.exports = { createPlannerDayPdf, dayEvents };
