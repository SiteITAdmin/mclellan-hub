...
function recordEffect(user, {
  ...
  try {
    ...
    `).run(
      ...
      EFFECT_STAGE, outcome === 'created' || outcome === 'completed' ? 'done' : outcome === 'refused' ? 'skipped' : 'error',
...
function effectHealth(user, { sinceSeconds = 86400, database = null } = {}) {
  ...
  for (const row of rows) {
    ...
    const entry = byOrigin.get(key)
      || { origin: key, created: 0, completed: 0, refused: 0, failed: 0, declared: Boolean(payload.origin_declared), titles: [] };
    if (payload.outcome === 'created') {
      entry.created += 1;
      if (entry.titles.length < 5) entry.titles.push(payload.title);
    } else if (payload.outcome === 'completed') entry.completed += 1;
  ...
  }), { created: 0, completed: 0, refused: 0, failed: 0 });
