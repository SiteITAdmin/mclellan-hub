...
// Which table owns each embeddable source. Only kinds listed here are ever
// pruned, so an unmapped kind is left alone rather than guessed at.
const SOURCE_KIND_TABLES = Object.freeze({
  atom: 'knowledge_atoms',
  email_summary: 'email_summaries',
  meeting_intake: 'meeting_intakes',
  document: 'documents',
  crm_fact: 'crm_facts',
  messaging_message: 'messaging_messages',
});

// Embeddings outlive their source when something deletes the source without
// telling the index. The big producer is nightly synthesis: it deletes every
// insight/thread atom and reinserts them with fresh uuid()s, so each run strands
// the previous run's vectors — 336 had accumulated by 4 Aug 2026, growing every
// night. Because ids are random rather than derived from content, a re-created
// atom never reclaims the old id and nothing can ever reconnect them.
//
// They are inert (a stale-model vector cannot match a current-model query) but
// they are dead weight and they drag the coverage figure in the daily report
// down, which makes a healthy corpus look broken. Sweeping by source existence
// covers every deletion path at once, including ones added later.
function pruneOrphanedEmbeddings({ dryRun = false } = {}) {
  const hub = db.hub();
  const removed = {};
  for (const [kind, table] of Object.entries(SOURCE_KIND_TABLES)) {
    ...
    const sql = `
      SELECT COUNT(*) AS n FROM embeddings
       WHERE source_kind = ?
         AND source_id NOT IN (SELECT id FROM "${table}")`;
    const n = hub.prepare(sql).get(kind)?.n || 0;
    if (!n) continue;
    removed[kind] = n;
    if (!dryRun) {
      hub.prepare(`
        DELETE FROM embeddings
         WHERE source_kind = ?
           AND source_id NOT IN (SELECT id FROM "${table}")`).run(kind);
    }
  }
  return removed;
}
...
async function indexSource(user, sourceKind, sourceId, text = null) {
  ...
  // Nothing is removed until replacements exist. This previously deleted the
  // source's chunks and only then called the embedder, so any transient failure
  // — a crashed Ollama runner, an unset LOCAL_EMBED_URL — left the source with
  // no vectors at all and threw, and the rows were only restored if some later
  // pass happened to succeed. One diagnostic backfill run against an unset embed
  // URL emptied ~2,900 sources that way on 3 Aug 2026, silently and invisibly.
...
async function backfillEmbeddings(user, {
  limit = 50,
  listSourceEvidenceFn = listSourceEvidence,
  isIndexedFn = isIndexed,
  indexSourceFn = indexSource,
  compiledSources = BACKFILL_COMPILED_SOURCES,
  sourceKinds = null,
} = {}) {
  const hub = compiledSources.length ? db.hub() : null;
  const model = embedModelId();
  let attempted = 0, processed = 0, indexedChunks = 0, remaining = 0;
  let skippedIncomplete = 0, retryableFailures = 0;
  ...
  const sourceEvidence = sourceKinds
    ? sourceKinds.flatMap(kind => listSourceEvidenceFn(user, kind))
    : listSourceEvidenceFn(user);
  for (const evidence of sourceEvidence) {
    if (!evidence.complete || !evidence.chunks.length) {
      skippedIncomplete++;
      continue;
    }
    if (isIndexedFn(evidence.source_kind, evidence.source_id, model, evidence.revision_hash)) continue;
    if (attempted >= limit) { remaining++; continue; }
    attempted++;
    try {
      indexedChunks += await indexSourceFn(user, evidence.source_kind, evidence.source_id, evidence);
      processed++;
    } catch (err) {
      retryableFailures++;
      remaining++;
  ...
  for (const src of compiledSources) {
    const rows = hub.prepare(src.sql).all(user);
    for (const row of rows) {
      if (!row.text || !row.text.trim()) continue;
      if (isIndexedFn(src.kind, row.id, model)) continue;
      if (attempted >= limit) { remaining++; continue; }
      attempted++;
      try {
        indexedChunks += await indexSourceFn(user, src.kind, row.id, row.text);
        processed++;
      } catch (err) {
        retryableFailures++;
        remaining++;
  ...
  return { attempted, processed, indexedChunks, remaining, skippedIncomplete, retryableFailures };
}

module.exports = {
  embed, embedOne, chunkText, cosine,
  indexSource, removeSource, isIndexed, markUnembeddable,
  semanticSearch, backfillEmbeddings, pruneOrphanedEmbeddings,
