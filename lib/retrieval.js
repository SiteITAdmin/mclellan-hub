...
function embedModelId() {
  return process.env.LOCAL_EMBED_MODEL
    || getSystemModelId('embeddings', 'system', EMBED_FALLBACK);
}
...
async function embed(texts, { taskCode = TASK_CODES.EMBEDDINGS } = {}) {
  ...
  const url = base.replace(/\/$/, '') + (base.includes('/api/') ? '' : '/api/embeddings');
  // Ollama-compatible: one text at a time when batch endpoint is absent.
  const vectors = [];
  for (const text of texts) {
    const resp = await fetch(url.includes('embeddings') ? url : `${base.replace(/\/$/, '')}/api/embeddings`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: process.env.LOCAL_EMBED_MODEL || 'nomic-embed-text', prompt: text, input: text }),
    });
    if (!resp.ok) {
    ...
    const v = data.embedding || data.data?.[0]?.embedding;
    if (!v) throw new Error('Local embeddings response missing vector');
    vectors.push(v);
  }
  logModelUsage({
    user: 'system', feature: 'embeddings', modelKey: 'embeddings',
    modelId: model, endpoint: 'local', durationMs: Date.now() - started, taskCode,
  });
  return vectors;
}
