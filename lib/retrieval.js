...
// A .docx converted to markdown inlines its images as `data:image/png;base64,…`.
// In the two documents that would not embed, 92% of the "text" was base64: one
// unbroken 6,649-character token per image. That traps llama.cpp inside Ollama
// (SIGTRAP, seen by the caller as `do embedding request: …: EOF`), and even when
// it survives, encoded bytes carry no meaning worth embedding — they only dilute
// the vector for the prose around them.
function stripEncodedBinary(value) {
  return String(value || '')
    .replace(/!?\[[^\]]*\]\(\s*data:[^)]*\)/gi, ' ')
    .replace(/data:[a-z0-9/+.*-]+;base64,[A-Za-z0-9+/=\s]{200,}/gi, ' ')
    .replace(/\S{500,}/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
async function indexSource(user, sourceKind, sourceId, text = null) {
  ...
  const chunks = (evidence?.chunks || chunkText(text))
    .map(chunk => {
      const clean = stripEncodedBinary(typeof chunk === 'string' ? chunk : chunk?.text);
      if (!clean) return null;
      return typeof chunk === 'string' ? clean : { ...chunk, text: clean };
    })
    .filter(Boolean);
