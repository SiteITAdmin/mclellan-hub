...
function isIndexed(sourceKind, sourceId, model = null) {
  const row = db.hub().prepare(
    'SELECT model FROM embeddings WHERE source_kind = ? AND source_id = ? LIMIT 1'
...
const BACKFILL_SOURCES = [
  ...
  { kind: 'completed_task', sql: "SELECT id, (COALESCE(title,'') || '. ' || COALESCE(notes,'')) AS text FROM google_tasks WHERE user = ? AND status = 'completed'" },
