...
const { postToGoogleChatSpace, buildFireCard } = require('./google-chat');
...
async function deliverChat(user, text, card) {
  const space = db.hub().prepare(
    "SELECT value FROM crm_context WHERE user = ? AND key = '_hermes_space'"
  ).get(user)?.value;
  if (space) {
    const ok = await postToGoogleChatSpace(space, text, card);
...
async function fireReminder(reminderId) {
  ...
  await deliverChat(r.user, fireMessage(r), buildFireCard(r));
