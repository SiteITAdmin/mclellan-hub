...
const SYNC_FOLDERS_KEY = '_outlook_sync_folders';
...
// ── Folder scoping ────────────────────────────────────────────────────────────
// Douglas gets cc'd on server alerts and helpdesk tickets that are not for
// him. Sync is therefore scoped to designated folders — Outlook rules (or
// manual filing) decide what the Hub sees. Default is the whole Inbox until
// folders are configured at /admin/microsoft.

function getSyncFolders(user) {
  const row = db.hub().prepare(
    'SELECT value FROM crm_context WHERE user = ? AND key = ?'
  ).get(user, SYNC_FOLDERS_KEY);
  const names = String(row?.value || '').split(',').map(s => s.trim()).filter(Boolean);
  return names.length ? names : ['Inbox'];
}

function setSyncFolders(user, names) {
  const cleaned = (Array.isArray(names) ? names : String(names || '').split(','))
    .map(s => String(s).trim())
    .filter(Boolean)
    .slice(0, 30);
  const hub = db.hub();
  if (!cleaned.length) {
    hub.prepare('DELETE FROM crm_context WHERE user = ? AND key = ?').run(user, SYNC_FOLDERS_KEY);
    return ['Inbox'];
  }
  hub.prepare(`
    INSERT INTO crm_context (id, user, key, value) VALUES (?, ?, ?, ?)
    ON CONFLICT(user, key) DO UPDATE SET value = excluded.value
  `).run(uuid(), user, SYNC_FOLDERS_KEY, cleaned.join(','));
  return cleaned;
}

async function listMailFolders(fetchAll) {
  // Top level + one level of children covers Inbox subfolders, where mail
  // rules usually file things. Graph v1.0 has no wellKnownName property, so
  // 'Inbox' is special-cased by its well-known id in resolveSyncFolders.
  const top = await fetchAll(
    '/me/mailFolders?$top=200&$expand=childFolders($top=200)',
    { limit: 400 }
  );
  const flat = [];
  for (const f of top) {
    flat.push({ id: f.id, displayName: f.displayName || '' });
    for (const c of f.childFolders || []) {
      flat.push({ id: c.id, displayName: c.displayName || '', parent: f.displayName || '' });
    }
  }
  return flat;
}

async function resolveSyncFolders(fetchAll, names) {
  const matched = [];
  const unresolved = [];
  for (const name of names) {
    if (name.toLowerCase() === 'inbox') {
      matched.push({ id: 'inbox', displayName: 'Inbox' });
    } else {
      unresolved.push(name);
    }
  }
  if (!unresolved.length) return { matched, missing: [] };

  const folders = await listMailFolders(fetchAll);
  const missing = [];
  for (const name of unresolved) {
    const hits = folders.filter(f => f.displayName.toLowerCase() === name.toLowerCase());
...
async function processOutlookMail(user = 'douglas', options = {}) {
  ...
  const folderNames = getSyncFolders(user);
  const { matched: folders, missing: missingFolders } = await resolveSyncFolders(fetchAll, folderNames);
  if (missingFolders.length) {
    console.warn(`[outlook] ${user}: configured sync folder(s) not found in mailbox: ${missingFolders.join(', ')}`);
  }
  ...
  for (const folder of folders) {
    const rows = await fetchAll(
      `/me/mailFolders/${encodeURIComponent(folder.id)}/messages?$filter=receivedDateTime ge ${sinceIso}`
  ...
  return {
    ...
    folders: folders.map(f => f.displayName),
    missingFolders,
  };
}
