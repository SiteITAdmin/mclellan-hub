...
const { resolveContactIdentityFromContacts } = require('./contact-identity');
...
function applyResolution(extraction, name, resolution, { overwrite = false } = {}) {
  const target = normalizeName(name);
  const meeting = extraction.meeting || {};
  let changed = 0;
  let corrected = 0;
  const apply = item => {
    ...
    if (item.matched_contact && !overwrite) return false;
    if (normalizeName(item.matched_contact) === normalizeName(resolution.contactName)) return false;
    if (item.matched_contact) corrected += 1;
    item.matched_contact = resolution.contactName;
    changed += 1;
    return true;
  };
  for (const a of meeting.attendees || []) {
    if (normalizeName(a.name) === target) apply(a);
  }
  for (const u of extraction.crm_updates || []) {
    if (normalizeName(u.subject) === target) apply(u);
  }
  for (const act of extraction.action_register || []) {
    if (normalizeName(act.owner) === target && apply(act)) {
...
async function resolveMeetingEntities({
  ...
  // Pass 1: deterministic identity wins before model output. It fills missing
  // links and corrects a contradictory stored model link when the source name
  // itself uniquely resolves by canonical name/alias/first name.
  const meeting = extraction.meeting || {};
  const seedNames = new Set();
  for (const a of meeting.attendees || []) if (a.name) seedNames.add(String(a.name).trim());
  for (const u of extraction.crm_updates || []) if (u.subject) seedNames.add(String(u.subject).trim());
  for (const act of extraction.action_register || []) if (act.owner) seedNames.add(String(act.owner).trim());
  for (const name of seedNames) {
    if (isPlaceholderPersonName(name)) continue;
    const identity = resolveContactIdentityFromContacts(contacts, { names: [name] });
    if (!identity.contact) continue;
    const applied = applyResolution(extraction, name, { contactName: identity.contact.name }, { overwrite: true });
    if (!applied.changed) continue;
    resolved.push({
      name,
      contact: identity.contact.name,
      method: applied.corrected ? 'deterministic_correction' : 'deterministic',
  ...
  if (useModel) {
    ...
    if (names.length) {
      ...
      for (const name of names) {
        ...
        if (!contact) continue;
        applyResolution(extraction, name, hit);
