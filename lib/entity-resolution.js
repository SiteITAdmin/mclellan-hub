...
// A bare first name ("Nick") is safe to persist as a durable alias only when it
// is unique across the CRM. When two people could be that name (Nick Chin vs
// Duncan Sackfield aka Nick), the model may still resolve it correctly for one
// meeting from context, but freezing it as a global alias would mislink every
// future bare "Nick". Multi-token names ("Alec Kangley") are distinctive enough
// to learn. Existing/human aliases are never governed by this — only auto-learn.
function isAmbiguousBareFirstName(name, contacts, targetContactId) {
  const clean = normalizeName(name);
  if (!clean || clean.includes(' ')) return false; // has a surname → distinctive
  return contacts.some(contact => {
    if (contact.id === targetContactId) return false;
    return contactTerms(contact).some(term => firstToken(term) === clean);
  });
}
  ...
  if (useModel) {
    ...
    if (names.length) {
      ...
      for (const name of names) {
        ...
        // free exact match — but never freeze an ambiguous bare first name.
        const canLearn = !isAmbiguousBareFirstName(name, contacts, contact.id);
        const didLearn = canLearn && learnAlias(user, contact, name, {
          source: 'auto_resolver', confidence: hit.confidence, reason: hit.reason, sourceKind, sourceId,
        });
        if (didLearn) learned += 1;
        resolved.push({ name, contact: contact.name, method: canLearn ? 'model' : 'model_context_only', confidence: hit.confidence, reason: hit.reason });
        resolvedNames.push(name);
      }
    }
  }
