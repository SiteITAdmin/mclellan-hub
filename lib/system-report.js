...
function moduleHealthChecks() {
  ...
  // Outlook (Microsoft 365) — only meaningful once an account is connected
  try {
    const { isOutlookSyncEnabled } = require('./outlook-processor');
    if (isOutlookSyncEnabled('douglas')) {
      for (const jobType of ['outlook_email_process', 'outlook_calendar_sync']) {
        const pending = hub.prepare(
          "SELECT 1 FROM system_jobs WHERE type=? AND status IN ('pending','running') LIMIT 1"
        ).get(jobType);
        if (!pending) warnings.push(`CRITICAL: ${jobType} has no pending job — Outlook sync has stopped`);
      }
      // A successful mail run always updates the last-check timestamp, so a
      // stale one means every run in the last day failed (e.g. expired token).
      const lastCheck = hub.prepare(
        "SELECT value FROM crm_context WHERE user = 'douglas' AND key = '_outlook_last_check_ts'"
      ).get();
      if (lastCheck && parseInt(lastCheck.value, 10) < now - 86400) {
        warnings.push('Outlook: no successful mail sync in over 24h — token may have expired; reconnect at /admin/microsoft');
      }
    }
  } catch (err) {
    warnings.push(`Outlook health check failed: ${err.message}`);
  }
...
async function sendSystemReport() {
  ...
  try {
    const artifact = saveHtmlArtifact({
      user: 'douglas',
      title: `Hub Daily Report — ${dateStr}`,
      subtitle: 'Daily health, activity, costs, and failure visibility for McLellan Hub.',
      markdown,
    });
    html = artifact.html;
