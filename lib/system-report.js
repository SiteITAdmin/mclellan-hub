...
async function sendSystemReport() {
  ...
  if (cats.errors.length) {
    sections.push(reportSection(`ERRORS & WARNINGS (${cats.errors.length})`, cats.errors.slice(0, 30)));
  }

  if (cats.agentmail.length) {
    sections.push(reportSection(`AGENTMAIL (${cats.agentmail.length} log lines)`, cats.agentmail.slice(0, 30)));
  }

  if (cats.email.length) {
    const shown = cats.email.length > 30 ? cats.email.slice(0, 30).concat([`and ${cats.email.length - 30} more`]) : cats.email;
    sections.push(reportSection(`EMAIL PROCESSING (${cats.email.length} log lines)`, shown));
  }

  if (cats.crm.length) {
    const shown = cats.crm.length > 20 ? cats.crm.slice(0, 20).concat([`and ${cats.crm.length - 20} more`]) : cats.crm;
    sections.push(reportSection(`CRM (${cats.crm.length} log lines)`, shown));
  }

  if (cats.newsletter.length) {
    const shown = cats.newsletter.length > 15 ? cats.newsletter.slice(0, 15).concat([`and ${cats.newsletter.length - 15} more`]) : cats.newsletter;
    sections.push(reportSection(`NEWSLETTER / RSS (${cats.newsletter.length} log lines)`, shown));
  }

  if (cats.tasks.length) {
    sections.push(reportSection(`TASKS (${cats.tasks.length} log lines)`, cats.tasks.slice(0, 30)));
  }
