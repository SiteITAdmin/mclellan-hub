...
function knowledgeSection(user = 'douglas') {
  ...
  const lines = [
    `Atoms: ${counts.active} active  |  ${counts.proposed} proposed  |  ${counts.stale} stale (out of view, never deleted — searchable in Ask the Hub)`,
  ];
  if (lint) {
    const when = new Date(lint.ranAt * 1000).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' });
    lines.push(`Last lint (${when}): ${lint.decayed} decayed, ${lint.staled} newly stale, ${lint.contradictions} contradiction(s), ${lint.proposed} awaiting review`);
    if (lint.newlyStale && lint.newlyStale.length) {
      lines.push('Newly out of view (reconfirm or retire at /admin/knowledge):');
...
Tasks created       : ${stats.tasks.created || 0}  |  Google tasks imported: ${stats.tasks.synced || 0}  |  Completed today: ${stats.tasks.completed || 0}
