...
async function sendSystemReport() {
  ...
  try {
    ...
    const remediation = remediationSummary('douglas');
    ...
    // Same "NEEDS YOU" facts the INGEST/EFFECTS/MODEL USAGE sections below
    ...
    sections.push(reportSection('YOUR BRIEF', [markdown]));
  } catch (err) {
  ...
  try {
    sections.push(ingestSection('douglas'));
  } catch (err) {
    console.warn('[system-report] ingest section failed:', err.message);
  }

  try {
    const effects = effectsSection('douglas');
    if (effects) sections.push(effects);
  } catch (err) {
    console.warn('[system-report] effects section failed:', err.message);
  }
  ...
  const usage = modelUsageSection(since24h);
  if (usage) sections.push(usage);

  // Failures only, deduplicated, and capped. The five per-module log dumps this
  // replaced carried up to 115 lines of raw journald into the email every
  // morning; a log is not a decision, and anything that needs Douglas is already
  // in the brief or in module health.
  if (cats.errors.length) {
    const distinct = compactRepeatedLines(cats.errors, 12);
    sections.push(reportSection(
      `ERRORS (${cats.errors.length} log line(s), ${distinct.length} distinct)`,
      distinct,
    ));
  }
