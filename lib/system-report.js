...
async function sendSystemReport() {
  ...
  try {
    ...
    // Douglas, and the SELF-HEALING section records what the agents did.
    try {
    ...
    if (remediation.counts.fixed || remediation.counts.dispatched || remediation.counts.escalated) {
      sections.push(reportSection('SELF-HEALING', [
        `Agents fixed ${remediation.counts.fixed}, dispatched ${remediation.counts.dispatched} retry(ies), escalated ${remediation.counts.escalated} to you.`,
        ...remediation.fixed.map(s => `✓ ${s}`),
        ...remediation.dispatched.map(s => `→ ${s}`),
        ...remediation.escalated.map(s => `⚠ ${s}`),
      ]));
    }
  } catch (err) {
