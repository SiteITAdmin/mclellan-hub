...
function effectsSection(user = 'douglas') {
  ...
  if (!totals.created && !totals.refused && !totals.failed) return null;
  const lines = [
    `External effects: ${fmt(totals.created)} created, ${fmt(totals.refused)} refused as duplicates, `
      + `${fmt(totals.failed)} failed`,
  ];
  for (const entry of origins) {
    if (!entry.created && !entry.failed) continue;
    lines.push(
...
// email — the full breakdown is one click away at /token-burn. This is just
...
function usageWatchSection(since24h) {
  ...
  if (!failing.length) return reportSection('USAGE WATCH', ['Usage normal — full breakdown: /token-burn']);
  return reportSection('USAGE WATCH', [
    ...failing.map(line => `NEEDS YOU — ${line}`),
    'Full breakdown: /token-burn',
  ]);
}
