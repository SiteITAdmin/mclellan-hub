...
// Apply a label and remove from INBOX (archive)
...
async function fetchNewEmails(user) {
  ...
  for (const msg of messages) {
    ...
    try {
      ...
      // Extract body text, capped at 2000 chars for the model call
      const bodyText = decodeBodyPart(full.data.payload).slice(0, 2000);
...
module.exports = { fetchNewEmails, moveToLabel, getOrCreateLabel, sendEmail };
