...
async function extractWorkIntelligence(email, contacts, projects) {
...
Return only JSON:
{
  ...
  "people": [
    {"name":"full name exactly stated","fact":"specific fact or work item directly evidenced"}
  ]
}
...
function storePeopleFacts(hub, user, email, people, source = 'agentmail') {
  const sourceLabel = source === 'gmail' ? 'Gmail' : 'AgentMail';
  for (const item of people || []) {
    if (!item?.name || !item?.fact) continue;
    const contact = resolveContact(hub, user, item.name);
    ...
    hub.prepare(`
      ...
      VALUES (?, ?, ?, ?, 'active', ?)
    `).run(uuid(), user, contact.id, item.fact, source);
    try {
  ...
  for (const listedMessage of listed) {
    ...
    try {
      ...
      const work = hasContent && !newsletter
        ? await extractWorkIntelligence(email, contacts, projects)
        : { is_work_content: false, summary: '', project_slug: null, people: [] };
      const validProject = projects.some(project => project.slug === work.project_slug)
        ? work.project_slug
        : projects.some(project => project.slug === classification.project_slug)
          ? classification.project_slug
          : (work.is_work_content && projects.some(p => p.slug === 'beacon'))
            ? 'beacon'
            : null;
      ...
      if (work.is_work_content) storePeopleFacts(hub, user, email, work.people);
