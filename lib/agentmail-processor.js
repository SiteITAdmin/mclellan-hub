...
- Only include someone in people if there is a direct, specific fact about them — not just a passive mention.${wrongFacts.length ? `
...
async function processAgentMail(user = 'douglas') {
  ...
  for (const listedMessage of listed) {
    ...
    try {
      ...
      const resolvedContactIds = work.is_work_content
