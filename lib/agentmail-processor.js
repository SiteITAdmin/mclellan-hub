  ...
  for (const listedMessage of listed) {
    ...
    try {
      ...
      `).run(
        ...
        // Raw body kept verbatim (capped) so the CRM engine reads the whole forwarded
        // thread and every explicit ask inside it, not just the one-line summary.
        email.bodyText ? String(email.bodyText).slice(0, 100000) : null
      );
