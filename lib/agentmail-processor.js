...
async function processAgentMail(user = 'douglas') {
  ...
  for (const listedMessage of listed) {
    ...
    try {
      ...
      // Create Google Task when the email requires action
      if (work.is_work_content && work.fact_type === 'action' && work.summary) {
        const actionPerson = (work.people || []).find(p => p.role === 'action_owner' || p.role === 'subject');
        const taskTitle = actionPerson
          ? `Reply to ${actionPerson.name} re: ${email.subject}`
