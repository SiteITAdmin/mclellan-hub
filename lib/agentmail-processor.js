...
Return only JSON:
{
  ...
  "action_task": "short imperative task title if Douglas himself needs to take an action (reply, decide, follow up, review, arrange) — null if no clear personal action required",
  "people": [
    {
...
- action_task: set when Douglas must personally reply, decide, arrange, or follow up — high bar, not for passive reading or FYI content.${wrongFacts.length ? `
...
function storePeopleFacts(hub, user, email, people, source = 'agentmail', factType = 'fact') {
  ...
  for (const { item, contact } of resolved) {
    ...
    hub.prepare(`
      ...
      VALUES (?, ?, ?, ?, 'active', ?, ?, ?)
    `).run(factId, user, contact.id, item.fact, source, JSON.stringify(linkedContacts), itemType);
...
async function processAgentMail(user = 'douglas') {
  ...
  for (const listedMessage of listed) {
    ...
    try {
      ...
      const trustedWork = isWorkSenderDomain(email.fromEmail);
      const newsletter = !trustedWork && isNewsletter(email);
      ...
      const resolvedContactIds = shouldExtract
        ? storePeopleFacts(hub, user, email, work.people, 'agentmail', work.fact_type)
        : [];
      ...
      if (shouldExtract) {
        // Primary: explicit action_task field from the model (Douglas needs to act personally)
        if (work.action_task) {
          createTask(user, {
            title: work.action_task,
            notes: `From: ${email.fromName || email.fromEmail} — ${email.subject}\n${work.summary || ''}`.trim(),
            source: 'agentmail',
            sourceId: `agentmail:action:${externalId}`,
            projectSlug: validProject || null,
          }).catch(err => console.warn('[tasks] agentmail task create failed:', err.message));
        }
        ...
        for (const person of actionOwners) {
          const personContact = resolvedContactIds.length
            ...
            : null;
          createTask(user, {
            title: `Follow up (${person.name}): ${email.subject}`,
            notes: `${person.fact}\n\nFrom: ${email.fromName || email.fromEmail} — ${email.subject}`,
            source: 'agentmail',
            sourceId: `agentmail:owner:${externalId}:${person.name.slice(0, 40)}`,
