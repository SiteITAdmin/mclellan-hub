...
function captureRawAgentMail(hub, user, externalId, full, email) {
  ...
  const identity = resolveContactIdentity(user, {
    names: [email.fromName],
    addresses: [email.fromEmail],
  }, { hub });
  hub.prepare(`
  ...
  const admission = admitCapturedEmail(hub, user, `agentmail:${externalId}`, 'agentmail');
...
async function processAgentMail(user = 'douglas') {
  ...
  const contacts = hub.prepare('SELECT id, name, email, phone, aliases FROM contacts WHERE user = ? ORDER BY name').all(user);
  ...
  for (const listedMessage of listed) {
    ...
    try {
      ...
      const capture = captureRawAgentMail(hub, user, externalId, materialized.full, email);
      if (materialized.attachmentReview) {
        markRawAgentMail(hub, user, externalId, 'attachment_unextracted');
        recordProcessingFailure(
          'agentmail',
      ...
      const senderContact = capture.identity.contact;
      ...
      const classification = (hasContent
        ? await classifyEmail(email, contacts, projects, emailLabels, user, 'agentmail', capture.identity)
