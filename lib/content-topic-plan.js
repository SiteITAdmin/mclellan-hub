...
const TOPIC_ALIASES = Object.freeze({
  ...
  'Industry Analysis': ['Business & Finance', 'Industry', 'Market', 'Vendor Strategy', 'Sector Trends'],
...
function rowMatchesAnyTopic(row, topics = Object.keys(TOPIC_ALIASES)) {
  ...
  return topics.some(topic =>
    topicTerms(topic).some(term => text.includes(term))
  );
}
...
function topicMatches(row, wanted) {
  ...
  return topicTerms(wanted).some(term =>
    haystack.includes(term) || (category && (term === category || term.includes(category)))
  );
}
