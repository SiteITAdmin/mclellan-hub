...
function buildTokenBurnAudit(user = 'douglas', summary = buildTokenBurnSummary(user), nowMs = Date.now()) {
  ...
  const invalidRows = importedRows.filter(row => {
    const values = ['codex_tokens', 'claude_code_tokens', 'antigravity_tokens', 'grok_build_tokens', 'api_tokens']
  ...
  checks.push(check(
    ...
    // The refresh runs from the Mac mini (scripts/update-token-burn-data.sh via
    // com.mclellan.token-burn-refresh) and pushes to the VPS — there is no import
    // button in the Hub, so the check has to name the real repair or the daily
    // brief will invent one.
    latestAge == null
      ? 'No latest imported date — run the token-burn refresh job on the Mac mini'
      : `latest=${summary.lastDate}; age=${latestAge} day(s) — run the token-burn refresh job on the Mac mini`,
  ...
  checks.push(check(
    ...
    liveManagementFresh
      ? `authoritative management API fetched at ${summary.openRouterLive.fetched_at}`
      : exportAge == null ? 'No OpenRouter export timestamp and no fresh live management API summary' : `export_last_at=${summary.openRouter?.export_last_at}; age=${exportAge} day(s)`,
    {
      ...
      legacy_export_role: 'historical baseline only; not a freshness requirement when management API is current',
