...
async function runCrossEntitySynthesis(user) {
  ...
  // Change-driven, retrieval-bounded: never dump the full atom table.
  ...
  if (require('./subscription-agent-jobs').enabled()) {
    const queued = require('./subscription-agent-jobs').enqueue({
      ...
      payload: {
        user,
        // Send only ids + labels for apply-side validation, not the full corpus.
        atomIds: atoms.map(a => a.id),
        systemPrompt,
        userPrompt: atomText,
        mode: 'apply',
        maxEvidenceRecords: atoms.length,
      },
    });
...
function completeRemoteCrossEntitySynthesis(payload, output) {
  ...
  if (!atoms.length && Array.isArray(payload?.atomIds) && payload.atomIds.length) {
    ...
    ).all(user, ...payload.atomIds.slice(0, 80));
  }
  ...
  const result = replaceCrossEntityInsights(user, insights);
  return { atomsRead: atoms.length, insights: result.written, execution: 'subscription_remote', runner: 'claude', model: 'sonnet', effort: 'high' };
}
