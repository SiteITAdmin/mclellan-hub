...
// The synthesis prompt asks for at most 15 short, structured insights. A 4k
// completion ceiling leaves generous room for that contract while preventing a
// reasoning model from turning a nightly knowledge refresh into a long report.
const CROSS_ENTITY_MAX_TOKENS = 4000;
const MAX_CROSS_ENTITY_INSIGHTS = 15;
...
connection must cite atoms from at least two different entities. Return at most
15 concise insights. This is a bounded synthesis result, not a prose report.`;
...
function normaliseCrossEntityInsights(rawInsights, atoms) {
  ...
  for (const raw of rawInsights.slice(0, MAX_CROSS_ENTITY_INSIGHTS)) {
...
async function runCrossEntitySynthesis(user) {
  ...
  const resp = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body:    JSON.stringify({
      model:           modelId,
      messages: [
        { role: 'system', content: `${getSystemPrompt('cross_entity_synthesis', 'system', PROMPTS.cross_entity_synthesis)}${CROSS_ENTITY_EVIDENCE_CONTRACT}` },
