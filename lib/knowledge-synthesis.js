...
function replaceCrossEntityInsights(user, insights) {
  const hub = db.hub();
  const ts = now();
  ...
  hub.transaction(() => {
    hub.prepare(`DELETE FROM knowledge_atoms WHERE user = ? AND subject_kind = ?`)
      .run(user, INSIGHT_KIND);
...
async function runCrossEntitySynthesis(user) {
  ...
  const atoms = hub.prepare(`
    SELECT id, subject_kind, subject_label, predicate, value, confidence
  ...
  // Production VPS: prepare evidence here, then let the authenticated Mac
  // pull-worker run Codex Luna. No raw database access is granted to the Mini.
  if (require('./subscription-agent-jobs').enabled()) {
    const queued = require('./subscription-agent-jobs').enqueue({
      ...
      dedupeKey: user,
      payload: { user, atoms, systemPrompt, userPrompt: atomText },
    });
    return { atomsRead: atoms.length, insights: 0, execution: 'subscription_remote', ...queued };
  }

  try {
    const local = await runSubscriptionText({ feature: 'cross_entity_synthesis', systemPrompt, userPrompt: atomText, timeoutMs: 300000 });
    if (local) {
      const parsed = parseJsonObject(local.text);
      if (!parsed || !Array.isArray(parsed.insights)) throw new Error('local Luna response was not valid insight JSON');
      const insights = normaliseCrossEntityInsights(parsed.insights, atoms);
      if (!insights.length) throw new Error('local Luna response had no sufficiently evidenced insights');
      const result = replaceCrossEntityInsights(user, insights);
      console.log(`[knowledge-synthesis] ${atoms.length} atoms -> ${result.written} subscription insights (${local.runner}/${local.model}/${local.effort})`);
      return { atomsRead: atoms.length, insights: result.written, execution: 'subscription', runner: local.runner, model: local.model, effort: local.effort };
    }
  } catch (err) {
    console.warn(`[knowledge-synthesis] subscription runner failed; using OpenRouter fallback: ${err.message}`);
  }
  ...
  const resp = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body:    JSON.stringify({
      ...
      temperature:     0.3,
      max_tokens:      CROSS_ENTITY_MAX_TOKENS,
    }),
  });
  ...
  if (choice.finish_reason === 'length') {
    console.warn(`[knowledge-synthesis] retained prior insights — model hit ${CROSS_ENTITY_MAX_TOKENS}-token cap`);
    return { atomsRead: atoms.length, insights: 0, retained: true, reason: 'length_capped' };
  }

  const parsed = parseJsonObject(choice.message?.content);
  if (!parsed || !Array.isArray(parsed.insights)) {
    console.warn('[knowledge-synthesis] retained prior insights — invalid model JSON');
    return { atomsRead: atoms.length, insights: 0, retained: true, reason: 'invalid_model_output' };
  }

  const insights = normaliseCrossEntityInsights(parsed.insights, atoms);
  if (!insights.length) {
    console.warn('[knowledge-synthesis] retained prior insights — candidate had no sufficiently evidenced insights');
    return { atomsRead: atoms.length, insights: 0, retained: true, reason: 'no_valid_insights' };
  }

  const result = replaceCrossEntityInsights(user, insights);
  console.log(`[knowledge-synthesis] ${atoms.length} atoms → ${result.written} source-backed insights (max_tokens=${CROSS_ENTITY_MAX_TOKENS})`);
  return { atomsRead: atoms.length, insights: result.written };
}
...
function completeRemoteCrossEntitySynthesis(payload, output) {
  ...
  if (!user || !parsed || !Array.isArray(parsed.insights)) throw new Error('remote Luna response was not valid insight JSON');
  const insights = normaliseCrossEntityInsights(parsed.insights, atoms);
  if (!insights.length) {
    ...
    return { atomsRead: atoms.length, insights: 0, retained: true, reason: 'no_valid_insights', execution: 'subscription_remote', runner: 'codex', model: 'gpt-5.6-luna', effort: 'medium' };
  }
  const result = replaceCrossEntityInsights(user, insights);
  return { atomsRead: atoms.length, insights: result.written, execution: 'subscription_remote', runner: 'codex', model: 'gpt-5.6-luna', effort: 'medium' };
}
