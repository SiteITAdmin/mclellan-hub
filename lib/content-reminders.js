...
function seedContentReminders(user) {
  const { createReminder, nextRecurOccurrence, syncRecurringReminder } = require('./reminders');
  const policy = getContentCadencePolicy(user);
  const seeds = [
    { key: `content-linkedin:${user}`, title: 'LinkedIn posting cadence', recur: policy.linkedin.recur, enabled: policy.linkedin.enabled },
    { key: `content-nl-midweek:${user}`, title: 'Newsletter topic health', recur: policy.newsletter.recur, enabled: policy.newsletter.enabled },
  ];
  for (const seed of seeds) {
    if (!seed.enabled) {
      syncRecurringReminder(user, seed.key, { enabled: false });
      continue;
    }
    const existing = syncRecurringReminder(user, seed.key, {
      enabled: true,
      title: seed.title,
      recur: seed.recur,
