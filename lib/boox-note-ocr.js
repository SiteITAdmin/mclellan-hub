 ...
 * actually writes, so a page he fixes today makes tomorrow's page easier to
 * read — without a model, a training run, or a second store. It is only a hint
 * list: a wrong entry can make Vision prefer a real word over a garbled one,
 * never invent text that is not on the page.
 */
