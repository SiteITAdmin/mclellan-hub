...
function rasterise(file, maxPages) {
  ...
  try {
    ...
    // "Command failed: pdftoppm …" on its own says nothing useful when this
    // shows up in a nightly log; carry poppler's own reason.
    const detail = String(err.stderr || '').trim() || err.message;
    throw new Error(`pdftoppm failed on ${path.basename(file)}: ${detail.slice(0, 300)}`);
  }
...
/**
 * The words to prime Vision with: the CRM vocabulary published by the VPS, plus
 * the words Douglas used when he corrected earlier pages.
 *
 * The second half is the learning loop: every correction is evidence of what he
 * actually writes, so a page he fixes feeds the next read — no model, no
 * training run, no second store. It is only a hint list: a wrong entry can make
 * Vision prefer a real word over a garbled one, never invent text that is not on
 * the page.
 *
 * MEASURED LIMIT (29 Aug 2026): priming changed nothing on Douglas's cursive.
 * "Meeting with Fergal - Copilot" still read as "Machy with fangel, - Coplet"
 * with Fergal and Copilot both in the list (verified 400 words reaching
 * VNRecognizeTextRequest.customWords, so this is a limitation, not a bug).
 * customWords biases the lexicon stage, which only helps when the character-level
 * read is already close ("Purv1ew" → "Purview"); his cursive collapses before
 * that stage, so the right candidate is never in the running. Keep it — it is
 * free, bounded, and does help print, lists and domain terms — but do not expect
 * it to rescue cursive. The lever that would is semantic reconstruction from what
 * the Hub already knows (his calendar said "Meet Fergal" that morning).
 */
function buildCustomWords({ maxWords = 500 } = {}) {
  const words = new Map();
  const add = (value) => {
  ...
  // Published by the VPS from the real CRM (the Mac's hub.db is a stale copy).
  try {
    const published = JSON.parse(fs.readFileSync(path.join(notesDir(), 'vocabulary.json'), 'utf8'));
    (published.words || []).forEach(add);
  } catch (_) { /* not published yet — corrections alone still help */ }

  // Learned from his own corrections, newest first so recent language wins the
  // cap. Only corrected text: the raw reading is what we are trying to improve.
  for (const record of listTranscriptions()) {
    if (record.correctedText) add(record.correctedText);
    if (words.size >= maxWords) break;
  }

  return [...words.values()].slice(0, maxWords);
}

/** Write the vocabulary somewhere the Swift helper can read it. */
function writeCustomWordsFile(words) {
  if (!words.length) return null;
  const file = path.join(os.tmpdir(), `hub-ocr-words-${process.pid}.json`);
  fs.writeFileSync(file, JSON.stringify(words));
  return file;
}

function recognise(bin, image, wordsFile) {
  const args = wordsFile ? [image, wordsFile] : [image];
  const raw = execFileSync(bin, args, { encoding: 'utf8', maxBuffer: 8 * 1024 * 1024 });
...
function transcribeFile(file, { maxPages = DEFAULT_MAX_PAGES } = {}) {
  ...
  const words = buildCustomWords();
  const wordsFile = writeCustomWordsFile(words);
  ...
  const pages = images.map((image, index) => {
    const lines = recognise(bin, image, wordsFile);
    return {
      page: index + 1,
  ...
  const record = {
    ...
    engine: 'apple-vision',
    // How much prior the reader had — useful when comparing an early reading
    // against a later one taken after corrections had accumulated.
  ...
  if (wordsFile) { try { fs.unlinkSync(wordsFile); } catch (_) { /* temp file */ } }
  return record;
}
