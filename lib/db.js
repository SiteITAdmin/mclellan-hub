...
function hub() {
  if (!_hub) {
    ...
    // Preserve the full email body as raw evidence. The CRM knowledge engine
    // must read the whole message, not a lossy one-line summary, or explicit
    // multi-item asks get flattened and dropped before they can become tasks.
    addCol('ALTER TABLE inbound_email_records ADD COLUMN body_text TEXT');
    addCol('ALTER TABLE email_summaries ADD COLUMN body_text TEXT');
    ...
    // OpenRouter retired (2026-08): disable openrouter endpoint catalogue rows.
    // Historical request_logs with endpoint='openrouter' are preserved for audit.
    try {
      _hub.exec(`UPDATE model_config
                 SET enabled = 0,
                     label = CASE
                       WHEN label LIKE '%[retired]%' THEN label
                       ELSE label || ' [retired — OpenRouter]'
                     END
                 WHERE endpoint = 'openrouter' AND enabled = 1`);
    } catch (_) {}
    // Subscription CLI catalogue entries for the admin model picker.
    _hub.prepare(`
      INSERT OR IGNORE INTO model_config
        (key, label, endpoint, model_id, tier, search, enabled, display_order, category)
      VALUES
        ('hub-luna', 'Codex Luna (subscription)', 'subscription', 'codex/gpt-5.6-luna',
         'everyday', 'none', 1, -5, 'Subscription'),
        ('hub-terra', 'Codex Terra (subscription)', 'subscription', 'codex/gpt-5.6-terra',
         'everyday', 'none', 1, -4, 'Subscription'),
        ('hub-sonnet', 'Claude Sonnet (subscription)', 'subscription', 'claude/sonnet',
         'everyday', 'none', 1, -3, 'Subscription'),
        ('hub-opus', 'Claude Opus (subscription)', 'subscription', 'claude/opus',
         'superior', 'none', 1, -2, 'Subscription'),
        ('hub-grok', 'Grok CLI (subscription)', 'subscription', 'grok/grok',
         'research', 'native', 1, -1, 'Subscription')
    `).run();
    // Retire the free OpenRouter catalogue key if present.
    try {
      _hub.exec(`UPDATE model_config SET enabled = 0, endpoint = 'subscription',
                 label = 'Free (retired — was OpenRouter)', model_id = 'retired/openrouter-free'
                 WHERE key = 'free'`);
    } catch (_) {}
    // Disable any remaining OpenRouter / openrouter-embeddings catalogue rows
    // (e.g. gemini embedding leftovers). Local Ollama is the embeddings path.
    try {
      _hub.exec(`UPDATE model_config
                 SET enabled = 0,
                     label = CASE
                     ...
                     END
                 WHERE enabled = 1
                   AND (endpoint LIKE 'openrouter%' OR model_id LIKE 'openrouter/%'
                        OR key LIKE '%embedding%' AND endpoint != 'subscription'
                        AND endpoint != 'local')`);
    } catch (_) {}
    // Prefer local Ollama embeddings slot metadata for admin display.
    try {
      _hub.prepare(`
        INSERT OR IGNORE INTO model_config
          (key, label, endpoint, model_id, tier, search, enabled, display_order, category)
        VALUES
          ('local-qwen3-embedding', 'Local Ollama qwen3-embedding', 'local',
           'ollama/qwen3-embedding', 'everyday', 'none', 1, -6, 'Local specialist')
      `).run();
    } catch (_) {}
    ...
    addLogCol('ALTER TABLE request_logs ADD COLUMN prompt_fingerprint TEXT');
    try {
      _hub.exec(`CREATE INDEX IF NOT EXISTS idx_rlogs_prompt_loop
                   ON request_logs(model_key, prompt_fingerprint, ts DESC)
    ...
    // Raw provider headers and ingestion state are kept separately from the
    // compatibility summary fields.  A row is captured before classifier work
    // and remains retryable until that work completes.
    addCol('ALTER TABLE email_summaries ADD COLUMN raw_metadata TEXT NOT NULL DEFAULT \'{}\'');
    addCol('ALTER TABLE email_summaries ADD COLUMN ingestion_status TEXT NOT NULL DEFAULT \'processed\'');
    ...
    _hub.exec(`
      ...
      CREATE TRIGGER IF NOT EXISTS contacts_reject_company_name_insert
      ...
      BEGIN
        SELECT RAISE(ABORT, 'Company names cannot be created as contacts');
      END;

      CREATE TRIGGER IF NOT EXISTS contacts_reject_company_name_update
      BEFORE UPDATE OF user, name ON contacts
      WHEN EXISTS (
        SELECT 1 FROM companies
        WHERE user = NEW.user
          AND lower(trim(name)) = lower(trim(NEW.name))
      ...
      BEGIN
        SELECT RAISE(ABORT, 'Company names cannot be used for contacts');
      END;
    ...
    CREATE INDEX IF NOT EXISTS idx_inbound_email_records_user_source
      ...
      -- WhatsApp / Apple messaging capture. Raw evidence only —
      ...
      -- Sealed historical message archives are faithful, immutable source
      -- evidence. They are reportable by original timestamp and release into
      -- the normal CRM knowledge path in deliberately bounded daily batches.
      CREATE TABLE IF NOT EXISTS messaging_archive_buckets (
        id TEXT PRIMARY KEY,
        user TEXT NOT NULL,
        ...
        sealed_at INTEGER,
        last_release_day TEXT,
        created_at INTEGER NOT NULL DEFAULT (unixepoch()),
        UNIQUE(user, project_id, name)
      );
      CREATE INDEX IF NOT EXISTS idx_messaging_archive_buckets_project
        ON messaging_archive_buckets(user, project_id);

      CREATE TABLE IF NOT EXISTS messaging_archive_messages (
        id TEXT PRIMARY KEY,
        ...
        created_at INTEGER NOT NULL DEFAULT (unixepoch()),
        FOREIGN KEY(bucket_id) REFERENCES messaging_archive_buckets(id)
      );
      CREATE INDEX IF NOT EXISTS idx_messaging_archive_messages_bucket_ts
        ON messaging_archive_messages(bucket_id, received_at DESC);
      CREATE INDEX IF NOT EXISTS idx_messaging_archive_messages_release
        ON messaging_archive_messages(bucket_id, released_at, received_at);

      CREATE TRIGGER IF NOT EXISTS trg_messaging_archive_no_insert_after_seal
      BEFORE INSERT ON messaging_archive_messages
      WHEN EXISTS (
        SELECT 1 FROM messaging_archive_buckets b
        WHERE b.id = NEW.bucket_id AND b.sealed_at IS NOT NULL
      )
      BEGIN
        SELECT RAISE(ABORT, 'messaging archive bucket is sealed');
      END;

      CREATE TRIGGER IF NOT EXISTS trg_messaging_archive_no_unseal
      BEFORE UPDATE OF sealed_at ON messaging_archive_buckets
      WHEN OLD.sealed_at IS NOT NULL AND NEW.sealed_at IS NOT OLD.sealed_at
      BEGIN
        SELECT RAISE(ABORT, 'messaging archive bucket cannot be unsealed');
      END;
    ...
    addCol('ALTER TABLE meetings ADD COLUMN source_id TEXT');
    _hub.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_meetings_source ON meetings(user, source, source_id);
    `);
    _hub.exec(`
    ...
    _hub.exec(`
      ...
      CREATE TABLE IF NOT EXISTS subscription_agent_jobs (
        id TEXT PRIMARY KEY,
      ...
      CREATE INDEX IF NOT EXISTS idx_subscription_agent_jobs_status
        ON subscription_agent_jobs(status, created_at);
      CREATE UNIQUE INDEX IF NOT EXISTS idx_subscription_agent_jobs_open
        ON subscription_agent_jobs(feature, dedupe_key)
    ...
    _hub.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_rlogs_prompt_loop
        ON request_logs(model_key, prompt_fingerprint, ts DESC)
        WHERE prompt_fingerprint IS NOT NULL;
    `);
    ...
    try {
      _hub.exec(`
        UPDATE system_jobs
           SET status = 'failed',
               error = 'superseded duplicate work-loop guard job',
               ran_at = unixepoch()
         WHERE type = 'work_loop_guard'
           AND status = 'pending'
           AND id NOT IN (
             SELECT id FROM system_jobs
              WHERE type = 'work_loop_guard' AND status = 'pending'
              ORDER BY run_at ASC, created_at ASC, id ASC LIMIT 1
           );
        CREATE UNIQUE INDEX IF NOT EXISTS idx_system_jobs_one_work_loop_guard_pending
          ON system_jobs(type)
          WHERE type = 'work_loop_guard' AND status = 'pending';
      `);
    } catch (err) {
      console.warn('[db] work-loop guard queue constraint skipped:', err.message);
    }
    // Cross-entity synthesis is a singleton refresh. Preserve an audit trail
    // for old duplicate rows by failing them rather than silently executing
    // the same graph-wide LLM job repeatedly, then enforce one pending run.
    try {
      _hub.exec(`
        UPDATE system_jobs
           SET status = 'failed',
               error = 'superseded duplicate cross-entity synthesis job',
               ran_at = unixepoch()
         WHERE type = 'cross_entity_synthesis'
           AND status = 'pending'
           AND id NOT IN (
             SELECT id
               FROM system_jobs
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS idx_system_jobs_one_cross_entity_pending
          ON system_jobs(type)
          WHERE type = 'cross_entity_synthesis' AND status = 'pending';
      `);
    } catch (err) {
      console.warn('[db] cross-entity synthesis queue guard skipped:', err.message);
    }
    // Suggestions are a non-urgent nightly batch. Preserve duplicate rows as
    // failed audit records, then allow only one pending evening run.
    try {
      _hub.exec(`
        UPDATE system_jobs
           SET status = 'failed',
               error = 'superseded duplicate suggestion batch job',
               ran_at = unixepoch()
         WHERE type = 'suggestion_run'
           AND status = 'pending'
           AND id NOT IN (
             SELECT id
               FROM system_jobs
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS idx_system_jobs_one_suggestion_pending
          ON system_jobs(type)
          WHERE type = 'suggestion_run' AND status = 'pending';
      `);
    } catch (err) {
      console.warn('[db] suggestion queue guard skipped:', err.message);
    }
    ...
    `);
    // Every deliberate suggestion decision is implicit relevance feedback.
    // Older databases only stored explained Wrong decisions, so add the
    // outcome fields idempotently and preserve existing Accept/Dismiss history.
    addCol('ALTER TABLE suggestion_feedback ADD COLUMN outcome TEXT');
    addCol('ALTER TABLE suggestion_feedback ADD COLUMN quality_score INTEGER');
    _hub.prepare(`
      UPDATE suggestion_feedback
      SET outcome = COALESCE(outcome, 'wrong'),
          quality_score = COALESCE(quality_score, CASE COALESCE(outcome, 'wrong')
            WHEN 'accepted' THEN 100
            WHEN 'not_this_time' THEN 60
            WHEN 'dismissed' THEN 25
            ELSE 0
          END)
      WHERE outcome IS NULL OR quality_score IS NULL
    `).run();
    _hub.prepare(`
      ...
      SELECT 'historical-suggestion-outcome:' || s.id,
             s.user, s.id, s.short_code, s.domain, s.title, s.body, s.evidence,
             s.status,
             CASE s.status
               WHEN 'accepted' THEN 100
               WHEN 'not_this_time' THEN 60
             ...
             '', s.created_at
      FROM suggestions s
      WHERE s.status IN ('accepted', 'not_this_time', 'dismissed')
        AND NOT EXISTS (
    ...
    `);
    // These raw/compiled-evidence columns are intentionally additive: old
    // installations retain their data and are reprocessed under the current
    // evidence revision when a body was never captured.
    addCol('ALTER TABLE email_summaries ADD COLUMN body_text TEXT');
    addCol('ALTER TABLE inbound_email_records ADD COLUMN body_text TEXT');
    addCol('ALTER TABLE email_summaries ADD COLUMN raw_metadata TEXT NOT NULL DEFAULT \'{}\'');
    addCol('ALTER TABLE email_summaries ADD COLUMN ingestion_status TEXT NOT NULL DEFAULT \'processed\'');
    addCol('ALTER TABLE embeddings ADD COLUMN source_revision TEXT');
    ...
    // Douglas's answers to the quality boards' open questions. This is raw
    // input the system has never seen — the boards can only ask, and until this
    // existed there was no screen to answer on, so the same question was
    // re-asked every night forever. The answer is not the knowledge: it is
    // projected into knowledge_atoms (derived_by='manual'), and this table is
    // the provenance for that atom plus the record that lets the boards stop
    // asking. question_key is a stable hash of the flagged item.
    _hub.exec(`
      CREATE TABLE IF NOT EXISTS crm_clarification_answers (
        id TEXT PRIMARY KEY,
        user TEXT NOT NULL,
      ...
      CREATE UNIQUE INDEX IF NOT EXISTS idx_clarification_answers_key
        ON crm_clarification_answers(user, question_key);
    `);
    ...
    // Full-source engine lease. This is deliberately separate from the action
    // outbox: it prevents concurrent workers from running the nondeterministic
    // triage/review/synthesis pipeline twice, while crm_action_outcomes still
    // owns fail-closed provider-call recovery.
    _hub.exec(`
      CREATE TABLE IF NOT EXISTS crm_source_processing_leases (
        user TEXT NOT NULL,
        source_kind TEXT NOT NULL,
        source_id TEXT NOT NULL,
        ...
        last_error TEXT,
        PRIMARY KEY (user, source_kind, source_id, source_revision, pipeline_version)
      );
      CREATE INDEX IF NOT EXISTS idx_crm_source_processing_leases_expiry
        ON crm_source_processing_leases(expires_at);
    `);

    // Action outcomes are compiled semantic decisions, not a second task
    // table.  Their unique key is the source revision + pipeline version +
    // exact source-derived action key, so a replay can reconcile a pending
    // external side effect without deriving identity from model wording.
    _hub.exec(`
      CREATE TABLE IF NOT EXISTS crm_action_outcomes (
        ...
        updated_at INTEGER NOT NULL DEFAULT (unixepoch()),
        resolved_at INTEGER,
        UNIQUE(user, source_kind, source_id, source_revision, pipeline_version, action_key)
      );
      CREATE INDEX IF NOT EXISTS idx_crm_action_outcomes_queue
        ON crm_action_outcomes(user, disposition, updated_at DESC);
      CREATE INDEX IF NOT EXISTS idx_crm_action_outcomes_source
        ON crm_action_outcomes(user, source_kind, source_id, source_revision, pipeline_version);
    `);
    ...
    `);
    addCol('ALTER TABLE synthesis_state ADD COLUMN source_revision TEXT');
    addCol('ALTER TABLE synthesis_state ADD COLUMN pipeline_version TEXT');
    ...
    _hub.prepare(`INSERT OR IGNORE INTO nakai_reg_monitor_sites (id, name, url, browser, cadence) VALUES (?, ?, ?, ?, ?)`).run('nrs_cbi_markets', 'Central Bank of Ireland Markets Update', 'https://www.centralbank.ie/regulation/markets-update', 0, 'daily');
    _hub.prepare(`INSERT OR IGNORE INTO nakai_reg_monitor_sites (id, name, url, browser, cadence) VALUES (?, ?, ?, ?, ?)`).run('nrs_fca',  'FCA',                       'https://www.fca.org.uk/news',                         0, 'daily');
    ...
    // FCA's browser-rendered route has repeatedly returned zero links in
    // production even though the server-rendered HTML contains the releases.
    _hub.prepare(`UPDATE nakai_reg_monitor_sites SET browser = 0 WHERE id = 'nrs_fca'`).run();

    // Nakai's briefing is EU/UK/Irish. Keep already-ingested US evidence for
    // provenance, but remove the US source configuration from this monitor.
    // The source catalogue remains in us-regulatory-sources.js for a future,
    // separate US briefing.
    _hub.prepare(`
      DELETE FROM nakai_reg_monitor_sites
    ...
    // FCA email/RSS alerts provide a second evidence path alongside the direct
    // server-rendered news-page monitor.
