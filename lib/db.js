...
// Keyword search across ALL documents regardless of project.
// Returns top `limit` rows with project name for context injection.
function searchDocuments(query, limit = 5) {
  const terms = query.toLowerCase().split(/\s+/).filter(t => t.length >= 2).slice(0, 6);
  if (!terms.length) return [];
  const conditions = terms.map(() =>
    '(instr(lower(d.filename),?)>0 OR instr(lower(d.markdown),?)>0)'
  ).join(' AND ');
  const params = terms.flatMap(t => [t, t]);
  return hub().prepare(
    `SELECT d.id, d.filename, d.markdown, p.name AS project_name, p.slug AS project_slug
     FROM documents d
...
module.exports = { hub, portfolio, searchDocuments };
