...
function upsertAtom(user, atom) {
  ...
  // Retired atoms are included so a rejected fact stays rejected: a re-extracted
  // exact/substring copy merges into the retired atom (kept retired by the status
  // CASE below) instead of resurfacing as a fresh proposal. Paraphrases that share
  // no substring are caught by the semantic pass in lib/atom-dedup.js, not here.
  if (!existing) {
    const MIN_FUZZY = 20;
  ...
  if (existing) {
    ...
    // A re-confirmation only adds provenance and nudges confidence. A human
    // decision is sticky: an 'active' (approved) atom is never demoted back to
    // 'proposed' by a later re-extraction, and a 'retired' (rejected) atom is
    // never resurrected. Only a still-'proposed' atom can be promoted to active.
    hub.prepare(`
      UPDATE knowledge_atoms
         SET source_refs = ?, confidence = ?, last_confirmed = ?, updated_at = ?,
             subject_id = COALESCE(subject_id, ?),
             subject_kind = COALESCE(subject_kind, ?),
             status = CASE WHEN status IN ('active', 'retired') THEN status ELSE ? END
       WHERE id = ?
