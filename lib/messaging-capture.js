...
function buildEvidenceText(row) {
  ...
  return [
    ...
    routing.contact_id ? `Resolved contact id: ${routing.contact_id}` : '',
...
function routingMetadata(row = {}) {
  ...
  return {
    ...
    contact_id: String(payload.contact_id || route.contact_id || payload.identity_resolution?.contact_id || '').trim(),
    identity_status: String(payload.identity_resolution?.status || '').trim(),
    identity_matched_by: Array.isArray(payload.identity_resolution?.matched_by)
      ? payload.identity_resolution.matched_by.map(String)
      : [],
...
function messageIdentityHints(payload = {}) {
  const suppliedRoute = payload?.route && typeof payload.route === 'object' ? payload.route : {};
  const raw = payload?.raw && typeof payload.raw === 'object' ? payload.raw : {};
  const direction = String(raw.direction || payload.direction || '').trim().toLowerCase();
  const isGroup = Boolean(payload.is_group || payload.isGroup);
  const names = [payload.contact_name, suppliedRoute.contact_name];
  const addresses = [];
  if (direction === 'outbound' && !isGroup) {
    names.push(payload.chat_name || payload.chatName);
    addresses.push(raw.chat_identifier, ...(Array.isArray(raw.participant_handles) ? raw.participant_handles : []));
  } else {
    names.push(payload.sender_name || payload.senderName || payload.pushName);
    addresses.push(payload.sender_id || payload.senderId || payload.user_id);
  }
...
function captureMessagingMessage(user, payload = {}) {
  ...
  const identity = resolveContactIdentity(user, messageIdentityHints(payload), { hub });
  const storedPayload = { ...payload };
  storedPayload.identity_resolution = {
  ...
  if (identity.contact) {
    storedPayload.contact_id = identity.contact.id;
    storedPayload.contact_name = identity.contact.name;
  }
  ...
  const row = {
    ...
    raw_json: JSON.stringify(storedPayload),
    status: 'received',
    created_at: now(),
  };
  ...
  return { id: row.id, created: true, duplicate: false, row, admission, identity };
}
