...
async function parseIntent(user, text, source = 'dchat') {
  ...
  const feature = taskCode === TASK_CODES.HERMES_CRM_CAPTURE ? 'hermes-crm-capture' : 'crm-intent';
  const { chatJson } = require('./chat-completions');
  return chatJson({
    feature,
    messages: [{ role: 'user', content: prompt }],
    user,
    label: 'CRM parser response',
  });
}
...
function buildBriefingText(user, calendarEvents = []) {
  ...
  const briefingCalendarEvents = (calendarEvents || []).filter(event => !isTaskPlannerCalendarEvent(event));
  ...
  if (!facts.length && !briefingCalendarEvents.length && !storedToday.length && !reminderLines.length) return null;
  ...
  for (const [name, contactFacts] of Object.entries(byContact)) {
    ...
    const meetings = briefingCalendarEvents.filter(ev => contactMatchesCalendarEvent(name, aliases, ev));
  ...
  try {
    ...
    if (openTasks.length) {
      ...
      const effectiveDue = t => t.effective_due || (t.due ? String(t.due).slice(0, 10) : null);
      const byUrgency = (a, b) => (rank[a.priority] ?? 3) - (rank[b.priority] ?? 3) || String(effectiveDue(a) || '9999').localeCompare(String(effectiveDue(b) || '9999'));
      const fmtTask = t => {
        const bits = [
          t.priority === 'high' ? '‼️' : null,
          t.title,
          t.due ? `_(due ${new Date(t.due).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })})_` : null,
          t.dependency_deferred && t.dep_not_before ? `_(not before ${String(t.dep_not_before).replace('T', ' ')})_` : null,
          t.effort_minutes ? `_[${t.effort_minutes >= 60 ? (t.effort_minutes / 60) + 'h' : t.effort_minutes + 'm'}]_` : null,
        ].filter(Boolean).join(' ');
        return `• ${bits}`;
      };
      const overdue = openTasks.filter(t => t.is_overdue).sort(byUrgency);
      const dueToday = openTasks.filter(t => !t.is_overdue && effectiveDue(t) === todayIso).sort(byUrgency);
      const thisWeek = openTasks.filter(t => !t.is_overdue && effectiveDue(t) > todayIso && effectiveDue(t) <= weekIso).sort(byUrgency);
      const later = openTasks.filter(t => !t.is_overdue && (!effectiveDue(t) || effectiveDue(t) > weekIso)).sort(byUrgency);
...
async function fetchCalendarEvents(user, { daysBack = 0, daysForward = 0 } = {}) {
  try {
    ...
    const live = allItems.filter(ev => ev.status !== 'cancelled' && ev.summary && (ev.start?.dateTime || ev.start?.date));
    try {
      const { promoteFreeDayBanners } = require('./task-calendar-planner');
      await promoteFreeDayBanners(calendar, live);
    } catch (err) {
      console.warn(`[crm] free-day banner promote failed for ${user}:`, err.message);
    }
    return live
      .map(ev => {
        ...
        return {
          ...
          hubSource: ev.extendedProperties?.private?.hubSource || null,
          hubTaskId: ev.extendedProperties?.private?.hubTaskId || null,
...
// Upsert a single meetings row keyed on (user, calendar_event_id) — shared by
// the read-side calendar sync below and by the CRM action-projection write
// path (lib/google-calendar.js) so both funnel through one dedup-safe write.
// `source_id` is only ever set by the write path; a sync pass never clobbers
// it (COALESCE keeps whatever was there).
function upsertMeetingRow(user, {
  title, date, time, durationMins, location, notes,
  companyId = null, calendarEventId, source = 'calendar', sourceId = null,
}) {
  const hub = db.hub();
  const existing = hub.prepare(
    'SELECT id FROM meetings WHERE user = ? AND calendar_event_id = ?'
  ).get(user, calendarEventId);
  const meetingId = existing?.id || uuid();
  hub.prepare(`
    INSERT INTO meetings
      (id, user, title, meeting_date, meeting_time, duration_mins, location, notes,
       company_id, calendar_event_id, source, source_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(user, calendar_event_id) DO UPDATE SET
      ...
      source_id = COALESCE(meetings.source_id, excluded.source_id)
  `).run(
    meetingId, user, title, date, time, durationMins, location, notes,
    companyId, calendarEventId, source, sourceId
  );
  return meetingId;
}

function linkMeetingAttendee(meetingId, contactId) {
  db.hub().prepare(
    'INSERT OR IGNORE INTO meeting_attendees (meeting_id, contact_id) VALUES (?, ?)'
  ).run(meetingId, contactId);
}

function adoptTaskPlannerMeetingFromCalendarEvent(user, event) {
  const hub = db.hub();
  const existing = hub.prepare(
    'SELECT id, source, source_id FROM meetings WHERE user = ? AND calendar_event_id = ?'
  ).get(user, event.id);
  if (!existing || existing.source === TASK_PLANNER_SOURCE) return existing || null;
  const taskId = event.hubTaskId || existing.source_id || null;
  hub.prepare(
    'UPDATE meetings SET source = ?, source_id = COALESCE(?, source_id) WHERE id = ? AND user = ?'
  ).run(TASK_PLANNER_SOURCE, taskId, existing.id, user);
  hub.prepare('DELETE FROM meeting_attendees WHERE meeting_id = ?').run(existing.id);
  return existing;
}

function importCalendarEventsAsMeetings(user, events) {
  const hub = db.hub();
  ...
  const transaction = hub.transaction(() => {
    for (const event of events) {
      if (isTaskPlannerCalendarEvent(event)) {
        adoptTaskPlannerMeetingFromCalendarEvent(user, event);
        continue;
      }
      ...
      const meetingId = upsertMeetingRow(user, {
        title: event.summary, date: event.date, time: event.time,
        durationMins: event.durationMins, location: event.location, notes: event.notes,
        companyId: company?.id || null, calendarEventId: event.id, source: 'calendar',
      });
      imported += 1;
      ...
      for (const attendee of event.attendeeDetails || []) {
        ...
        if (!contact) continue;
        linkMeetingAttendee(meetingId, contact.id);
        attendeesMatched++;
      }
    }
  });
  transaction();
  return { imported, attendeesMatched };
}

function listCrmMeetings(user) {
  return db.hub().prepare(`
    SELECT m.*, co.name AS company_name, COUNT(ma.contact_id) AS attendee_count
    FROM meetings m
    LEFT JOIN companies co ON co.id = m.company_id
    LEFT JOIN meeting_attendees ma ON ma.meeting_id = m.id
    WHERE m.user = ? AND ${crmMeetingSql('m')}
    GROUP BY m.id
...
async function syncCalendarMeetings(user, options = {}) {
  ...
  const result = importCalendarEventsAsMeetings(user, events);
  try {
    require('./crm-nudges').recomputeLastContacted(user);
  } catch (err) {
    console.warn('[crm] last-contacted recompute failed:', err.message);
  }
  return { events: result.imported, attendeesMatched: result.attendeesMatched };
}
...
module.exports = {
  ...
  importCalendarEventsAsMeetings,
  listCrmMeetings,
  upsertMeetingRow,
  linkMeetingAttendee,
  listContacts,
