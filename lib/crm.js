...
function taskCodeForCrmSource(source) {
  return ['google-chat', 'hermes'].includes(String(source || '').toLowerCase())
...
async function fetchCalendarEvents(user, { daysBack = 0, daysForward = 0 } = {}) {
  try {
    ...
    if (allDay.length && process.env.CRM_CALENDAR_DEBUG === '1') {
      console.log(`[crm] skipping ${allDay.length} all-day event(s):`, allDay.map(e => e.summary).join(', '));
    }
    return allItems
...
function listContacts(user, query = '') {
  ...
  return contacts
    ...
    .sort((a, b) => b._nameMatch - a._nameMatch || a.name.localeCompare(b.name));
}
