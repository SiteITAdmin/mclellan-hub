...
function buildBriefingText(user, calendarEvents = []) {
  ...
  const meetingNames = new Set(
    calendarEvents.flatMap(ev => ev.attendees || []).map(n => n.toLowerCase())
  );
  ...
  const sorted = Object.keys(byContact).sort((a, b) => {
    const aM = meetingNames.has(a.toLowerCase()) ? 0 : 1;
    const bM = meetingNames.has(b.toLowerCase()) ? 0 : 1;
  ...
  if (sorted.length) {
    ...
    for (const name of sorted) {
      const hasMeeting = meetingNames.has(name.toLowerCase());
      lines.push(`\n*${name}*${hasMeeting ? ' 📅' : ''}`);
