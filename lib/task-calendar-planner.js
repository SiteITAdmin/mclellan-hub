...
const { getCachedTasks } = require('./google-tasks');
...
async function getPlannerSnapshot(user, { startDate, endDate }, options = {}) {
  ...
  return {
    ...
    tasks,
    unscheduledTasks,
    scheduledCount: tasks.length - unscheduledTasks.length,
  };
}
...
function minutesFromTime(value) {
  ...
  if (!match) throw new Error('Work hours must be HH:MM');
  const result = Number(match[1]) * 60 + Number(match[2]);
  if (Number(match[1]) > 23 || Number(match[2]) > 59) throw new Error('Work hours are not valid');
  return result;
}
...
function computeAutoPlan({ tasks, events, startDate, endDate, workStart = '09:00', workEnd = '17:30', includeWeekends = false, now = new Date() }) {
  const range = validateRange(startDate, endDate);
  const startMinute = minutesFromTime(workStart);
  const endMinute = minutesFromTime(workEnd);
  if (endMinute - startMinute < MIN_DURATION_MINUTES) throw new Error('Work day must be at least 15 minutes');
  ...
  for (const task of ordered) {
    ...
    for (let index = 0; index < range.span; index += 1) {
      ...
      let earliest = startMinute;
      if (date < nowDate) continue;
      if (date === nowDate) earliest = Math.max(earliest, Math.ceil(nowMinute / 15) * 15);
      const intervals = eventBusyIntervals(plannedEvents, date, startMinute, endMinute);
      const slot = firstFreeSlot(intervals, earliest, endMinute, duration);
    ...
    if (!placed) unplaced.push({ taskId: task.id, title: task.title, reason: 'No free slot before its due date in this range' });
  }
...
async function autoPlanTasks(user, options = {}, dependencies = {}) {
  ...
  const plan = computeAutoPlan({
    ...
    ...range,
    workStart: options.workStart || '09:00',
    workEnd: options.workEnd || '17:30',
