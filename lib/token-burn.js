...
const db = require('./db');
const { titleForTaskCode } = require('./openrouter-attribution');
...
const DEPLOY_OPENROUTER_LIVE_PATH = path.join(__dirname, '..', 'token-burn-dashboard', 'deploy-data', 'openrouter-live.summary.json');
const LOCAL_OPENROUTER_LIVE_PATH = path.join(__dirname, '..', 'token-burn-dashboard', 'data', 'openrouter-live.summary.json');
...
function readOpenRouterSummary() {
  const summary = readJsonFile(
    ...
    { rows: 0, tokens: 0, cost_usd: 0, export_first_at: null, export_last_at: null, by_model: [], by_provider: [], by_app: [] },
  );
...
function readOpenRouterLiveSummary() {
  const summary = readJsonFile(
    [DEPLOY_OPENROUTER_LIVE_PATH, LOCAL_OPENROUTER_LIVE_PATH],
    { requests: 0, tokens: 0, cost_usd: 0, cache_hit_rate: 0, fetched_at: null, time_range: null, by_model: [], by_provider: [], by_app: [] },
  );
  for (const key of ['by_model', 'by_provider', 'by_app']) {
    summary[key] = Array.isArray(summary[key]) ? summary[key] : [];
  }
  return summary;
}
...
function exactTotal(row) {
  return Number(row.codex_tokens || 0) +
    ...
    Number(row.grok_build_tokens || 0) +
...
function buildTokenBurnSummary(user) {
  ...
  const openRouterLive = readOpenRouterLiveSummary();
  ...
  const api = importedRows.reduce((sum, row) => sum + Number(row.api_tokens || 0), 0);
  const openRouterExportAgeDays = daysSinceDate(openRouter.export_last_at);
  ...
  return {
    ...
    antigravity: importedRows.reduce((sum, row) => sum + Number(row.antigravity_tokens || 0), 0),
    antigravityEstimated: importedRows.some(row => row.antigravity_estimated),
    grokBuild: importedRows.reduce((sum, row) => sum + Number(row.grok_build_tokens || 0), 0),
    api,
    nonOpenRouterApi: Math.max(0, api - Number(openRouter.tokens || 0)),
    openRouterExportAgeDays,
    openRouterExportStale: openRouterExportAgeDays != null && openRouterExportAgeDays > 3,
...
function buildTokenBurnPage(user) {
  ...
  const accountTasks = (summary.openRouterLive.by_app || []).map(row => ({
    ...
    task_type: String(row.label || '').includes(' User: ') ? 'user' :
      String(row.label || '').includes(' Auto: ') ? 'automatic' :
      String(row.label || '').startsWith('McLellan Hub') ? 'user' : 'external',
    source: 'openrouter',
  }));
