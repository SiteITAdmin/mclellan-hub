 ...
 *
 * Resolution order: capture-time identity → exact/alias name match
 * (deterministic, free) → LLM linker
 ...
 * the subscription model plane; both models are admin-controllable slots.
 */
...
function resolveByName(label, entities, context = {}) {
  ...
  const contextual = exact.filter(entity =>
    (context.projectId && entity.kind === 'project' && entity.id === context.projectId)
    || (context.contactId && entity.kind === 'contact' && entity.id === context.contactId)
    || (context.companyId && entity.kind === 'company' && entity.id === context.companyId)
  );
...
function sourceContext(user, sourceKind, row) {
  ...
  if (sourceKind === 'messaging_message') {
    ...
    if (routing.contact_id) {
      const contact = hub.prepare('SELECT id FROM contacts WHERE user = ? AND id = ?').get(user, routing.contact_id);
      if (contact) context.contactId = contact.id;
    }
    const routedSubject = routing.subject_contact_name || routing.contact_name;
    if (!context.contactId && routedSubject) {
      const contacts = hub.prepare('SELECT id, name, aliases FROM contacts WHERE user = ?').all(user);
      ...
      if (matches.length === 1) context.contactId = matches[0].id;
    }
