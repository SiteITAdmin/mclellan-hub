...
async function linkSubject(draft, entities) {
  const candidates = entities
    .map(e => `${e.id} — ${e.label}${e.aliases.length ? ` (aka ${e.aliases.join(', ')})` : ''}`)
    .join('\n') || '(none)';
...
async function synthesiseSource(user, sourceKind, sourceId, text, entities, budget) {
  ...
  for (const d of drafts) {
    ...
    if (ent) {
      ...
      budget.n--;
      try {
        const link = await linkSubject(d, entities);
...
async function runSynthesis(user, { limit = 15, linkBudget = 20 } = {}) {
  ...
  for (const src of SYNTH_SOURCES) {
    ...
    for (const row of rows) {
      ...
      try {
        const r = await synthesiseSource(user, src.kind, row.id, text, entities, budget);
