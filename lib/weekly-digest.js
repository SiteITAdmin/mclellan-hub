...
async function llm(prompt, user = 'system') {
  ...
  const modelId = process.env.DIGEST_MODEL || 'deepseek/deepseek-v3.2';
...
async function buildWeeklyDigest(user) {
  ...
  const linkedinRows = hub.prepare(`
    SELECT topic, status, content_type FROM linkedin_posts
    WHERE user = ? AND created_at > ?
    ORDER BY created_at DESC
  `).all(user, cutoff);

  if (linkedinRows.length) {
    const liLines = linkedinRows.map(r => `${r.status}: "${r.topic}" (${r.content_type})`).join('\n');
