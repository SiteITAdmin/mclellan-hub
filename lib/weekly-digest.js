...
async function llm(prompt, user = 'system') {
  ...
  const resp = await fetch(OR_URL, {
    method: 'POST',
    headers: openRouterHeaders(TASK_CODES.WEEKLY_DIGEST),
    body: JSON.stringify({
      model: modelId,
  ...
  logUsageFromResponse({
    ...
    data,
    taskCode: TASK_CODES.WEEKLY_DIGEST,
...
async function sendWeeklyDigest(user) {
  ...
  try {
    ...
    // Prefer the Hermes two-way space so the weekly digest benefits from command replies
    ...
    const card = {
      cardsV2: [{
        ...
        card: {
          header: { title: '📋 Weekly Digest', subtitle: new Date().toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' }) },
          sections: [{
            widgets: [{
              buttonList: {
                buttons: [
                  { text: 'Flights', onClick: { action: { function: 'flights' } } },
                  { text: 'Regs', onClick: { action: { function: 'regs' } } },
                  { text: 'Reminders', onClick: { action: { function: 'reminders' } } },
                  { text: 'Briefing →', onClick: { openLink: { url: `${hubUrl}/crm` } } },
                ],
              },
            }],
          }],
    ...
    const sent = await deliverChat(user, text, card);
    if (sent) {
