...
async function llm(prompt, user = 'system') {
  ...
  const modelId = getSystemModelId('weekly_digest', 'system',
    process.env.DIGEST_MODEL || 'google/gemini-2.5-pro-preview');
  const resp = await fetch(OR_URL, {
    ...
    body: JSON.stringify({
      model: modelId,
      messages: [
        { role: 'system', content: getSystemPrompt('weekly_digest', 'system', PROMPTS.weekly_digest) },
...
async function buildWeeklyDigest(user) {
  ...
  if (chatRows.length) {
    ...
    const synthesis = await llm(`Section: Chats & Projects\n\nItems:\n${chatLines}`, user);
  ...
  if (emailRows.length) {
    ...
    const synthesis = await llm(`Section: Emails\n\nItems:\n${emailLines}`, user);
  ...
  if (factRows.length) {
    ...
    const synthesis = await llm(`Section: People & CRM\n\nItems:\n${factLines}`, user);
