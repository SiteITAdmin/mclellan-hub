...
async function rssFetch(url) {
  // 1. Try the URL itself (handles Feedburner, direct RSS links, etc.)
  // 2. Try appending /feed/ (WordPress pattern)
  // 3. Sniff the HTML for <link rel="alternate" type="application/rss+xml">
  const candidates = [url];

  // Also probe /feed/ variants unless the URL already looks like a feed
