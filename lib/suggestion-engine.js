 ...
 * call the subscription model plane, INSERT OR IGNORE
...
function writeSuggestionReceipt(user, {
  sourceKind = 'opportunity_signal',
  sourceId,
  stage,
  status = 'review',
  summary = '',
  ...
  try {
    ...
    console.warn('[suggestions] knowledge receipt write failed:', err.message);
    return null;
  }
}

function opportunityReceiptTargets(signalIds, signals, fallbackSourceId) {
  const ids = [...new Set((signalIds || []).filter(Boolean))];
  if (ids.length) return ids.map(id => ({ sourceKind: 'opportunity_signal', sourceId: id }));
  // A malformed candidate has no valid signal id to anchor a receipt to.  Keep
  // it observable as a candidate/run receipt rather than silently dropping it.
  return [{ sourceKind: 'suggestion_opportunity_run', sourceId: fallbackSourceId || uuid() }];
}

function recordOpportunitySuppression(user, candidate, {
  signals = [],
  signalIds = [],
  stage = 'suggestion_admission',
  status = 'review',
  reason = 'candidate withheld',
  runId = null,
  extra = {},
} = {}) {
  const targetSignals = signals.filter(signal => !signalIds.length || signalIds.includes(signal.id));
  const payload = {
    candidate,
    signalIds,
  ...
  const receipts = [];
  for (const target of opportunityReceiptTargets(signalIds, signals, runId)) {
    const id = writeSuggestionReceipt(user, {
      sourceKind: target.sourceKind,
      sourceId: target.sourceId,
      stage,
      status,
      summary: `Opportunity suggestion withheld: ${reason}`,
      payload,
    });
    if (id) receipts.push(id);
  }
  return receipts;
}
...
async function acceptSuggestionRow(user, s, {
  ...
  try {
    ...
    task = await create(user, {
      ...
      notes: s.body,
      source: 'suggestion',
      origin: 'suggestion-engine:accepted',
      sourceId: s.id,
...
async function runTravelSuggester(user) {
  ...
  // Reconcile explicit calendar flight details before asking the travel
  // suggester what is unbooked. This prevents a calendar booking from being
  // reported as missing merely because its original confirmation email is old.
  try {
    const { discoverCalendarFlights } = require('./flight-calendar-import');
    await discoverCalendarFlights(user, hub, [], calendarEvents);
  } catch (err) {
    console.warn('[suggestions] calendar flight import failed:', err.message);
  }
...
function opportunitySignalIds(value) {
  let evidence = value;
  if (typeof value === 'string') {
    try { evidence = JSON.parse(value || '{}'); } catch (_) { evidence = {}; }
  } else if (value?.evidence !== undefined) {
    try { evidence = JSON.parse(value.evidence || '{}'); } catch (_) { evidence = {}; }
  }
  return [...new Set((evidence?.signals || []).map(signal => signal?.id).filter(Boolean))].sort();
}

function priorOpportunitySignalIds(user) {
  const rows = db.hub().prepare(`
    SELECT evidence FROM suggestions
    WHERE user = ? AND domain = 'opportunity'
  `).all(user);
  return new Set(rows.flatMap(row => opportunitySignalIds(row)));
}

function unseenOpportunitySignals(user, signals) {
  const seen = priorOpportunitySignalIds(user);
  return (signals || []).filter(signal => !seen.has(signal.id));
}

function retireRepeatedOpportunitySuggestions(user) {
  const hub = db.hub();
  const rows = hub.prepare(`
    SELECT id, evidence, status, short_code FROM suggestions
    WHERE user = ? AND domain = 'opportunity'
    ORDER BY COALESCE(short_code, 0), created_at, id
  `).all(user);
  const seen = new Set();
  let retired = 0;
  for (const row of rows) {
    const ids = opportunitySignalIds(row);
    const repeated = ids.some(id => seen.has(id));
    if (repeated && row.status === 'open') {
      retired += hub.prepare("UPDATE suggestions SET status = 'retired' WHERE id = ? AND status = 'open'")
        .run(row.id).changes;
    }
...
function suggestionTaskHistory(user) {
  return db.hub().prepare(`
    SELECT id, title, notes, status, deleted_at, completed_at, created_at
    FROM google_tasks
    WHERE user = ?
    ORDER BY
      CASE WHEN status = 'needsAction' AND deleted_at IS NULL THEN 0 ELSE 1 END,
      COALESCE(completed_at, deleted_at, created_at) DESC
    LIMIT 120
  `).all(user).map(task => ({
    id: task.id,
    state: task.deleted_at ? (task.status === 'wrong' ? 'wrong' : 'deleted')
...
function suggestionDecisionHistory(user) {
  const hub = db.hub();
  const suggestions = hub.prepare(`
    SELECT id, title, body, status, evidence
    FROM suggestions
    WHERE user = ? AND domain IN ('opportunity', 'travel', 'contact')
      AND status != 'open'
    ORDER BY created_at DESC
  ...
  `).all(user).map(row => ({
    ...
    opportunity_signal_ids: opportunitySignalIds(row),
  }));
  return { tasks: suggestionTaskHistory(user), suggestions };
}

function filterOpportunityDuplicateDecisions(proposed, decisions) {
  const byIndex = new Map((decisions || []).map(decision => [Number(decision.candidate_index), decision]));
  return (proposed || []).filter((_, index) => {
    const decision = byIndex.get(index);
    return decision?.decision === 'keep' && Number(decision.confidence) >= 0.6;
  });
}

async function reviewOpportunityDuplicates(user, proposed, { review = llmJson } = {}) {
  if (!proposed.length) return [];
  const history = suggestionDecisionHistory(user);
  if (!history.tasks.length && !history.suggestions.length) return proposed;
  const prompt = `${getSystemPrompt('suggestion_duplicate_review', 'system', PROMPTS.suggestion_duplicate_review)}

CANDIDATES:
${JSON.stringify(proposed.map((candidate, candidate_index) => ({ candidate_index, ...candidate })), null, 2).slice(0, 12000)}

AUTHORITATIVE TASK HISTORY:
${JSON.stringify(history.tasks, null, 2).slice(0, 30000)}

PRIOR SUGGESTION DECISIONS:
${JSON.stringify(history.suggestions, null, 2).slice(0, 22000)}`;
  const parsed = await review(user, 'suggestion-duplicate-review', prompt);
  return filterOpportunityDuplicateDecisions(proposed, parsed.decisions);
}
...
async function runOpportunitySuggester(user) {
  ...
  const retiredDuplicates = retireRepeatedOpportunitySuggestions(user);
  if (retiredDuplicates) {
    console.warn(`[suggestions] retired ${retiredDuplicates} repeated open opportunity suggestion(s) for ${user}`);
  }
  ...
  const signals = unseenOpportunitySignals(
    user,
    activeOpportunitySignals(user, { limit: 12 }).filter(isActionableOpportunitySignal),
  );
...
AUTHORITATIVE TASK HISTORY (the same concrete action must not be suggested again):
${JSON.stringify(suggestionTaskHistory(user), null, 2).slice(0, 30000)}
...
${salienceContexts.map(ctx => [
  ...
  let reviewed = [];
  try {
    reviewed = await reviewOpportunityDuplicates(user, proposed);
  } catch (err) {
    // Duplicate review fails closed: silence is safer than re-creating work,
    // but it must remain visible as a source-backed review/error receipt.
    console.error('[suggestions] opportunity duplicate review failed closed:', err.message);
    for (const candidate of proposed) {
      const candidateSignalIds = Array.isArray(candidate?.signal_ids)
        ? candidate.signal_ids.filter(id => byId.has(id))
        : [];
      recordOpportunitySuppression(user, candidate, {
        signals,
        signalIds: candidateSignalIds,
        stage: 'suggestion_duplicate_review',
        status: 'error',
        reason: 'duplicate review failed; candidate was not admitted',
        runId,
  ...
  // The review helper returns the original candidate objects when they are
  // kept.  Any omitted candidate therefore has an auditable review outcome,
  // even when the model marked it already handled or lacked confidence.
  const reviewedSet = new Set(reviewed);
  for (const candidate of proposed) {
    if (reviewedSet.has(candidate)) continue;
    const candidateSignalIds = Array.isArray(candidate?.signal_ids)
      ? candidate.signal_ids.filter(id => byId.has(id))
      : [];
    recordOpportunitySuppression(user, candidate, {
      ...
      stage: 'suggestion_duplicate_review',
      status: 'review',
      reason: 'duplicate review did not confidently keep candidate',
      runId,
    });
  }
  for (const s of reviewed) {
    ...
    if (!isAdmissibleOpportunitySuggestion(s)) {
      rejection.push('missing causal admission fields');
    ...
    if (!String(s?.title || '').trim() || !String(s?.body || '').trim()) rejection.push('missing title or body');
    if (!signalIds.length) rejection.push('no valid source signal');
    const confidence = Number(s?.confidence);
    if (!Number.isFinite(confidence) || confidence < 0.55) rejection.push('low confidence');
    if (rejection.length) {
      recordOpportunitySuppression(user, s, {
        signals,
        signalIds,
        stage: 'suggestion_admission',
        status: 'review',
        reason: rejection.join('; '),
        runId,
      });
      continue;
    }
    const window = String(s.relevance_window || dublinTodayIso().slice(0, 7)).slice(0, 40);
    const dedupKey = `opportunity:${signalIds.sort().join('+')}`;
    const row = createSuggestion(user, {
      domain: 'opportunity',
...
module.exports = {
  createSuggestion,
  writeSuggestionReceipt,
  recordOpportunitySuppression,
  listSuggestions,
  listOpenSuggestions,
  ...
  runContactSuggester,
  runOpportunitySuggester,
  opportunitySignalIds,
  unseenOpportunitySignals,
  suggestionTaskHistory,
