 ...
 * 14 days. At most 2 new ones are pushed to Google Chat per run — the rest
 * wait in the morning briefing. Accepting one creates a task; nothing is
...
const MAX_CHAT_PUSHES_PER_RUN = 2;
...
async function runContentSuggester(user) {
  ...
  const topics = hub.prepare(`
    SELECT headline, category FROM nl_topics WHERE user = ? AND created_at > ?
...
async function runSuggesters(user) {
  ...
  const toPush = created.slice(0, MAX_CHAT_PUSHES_PER_RUN);
  if (toPush.length) {
    const { deliverChat } = require('./reminders');
    for (const s of toPush) {
      await deliverChat(user,
        `💡 *Suggestion #${s.short_code}* — ${s.title}\n${s.body}\nReply: accept ${s.short_code} · dismiss ${s.short_code} · why ${s.short_code}`);
    }
  }
  return created;
}
