...
const { formatSuggestionLessons } = require('./suggestion-learning');
...
async function acceptSuggestionRow(user, s, {
  ...
  db.hub().prepare("UPDATE suggestions SET status = 'accepted' WHERE id = ? AND user = ?")
    .run(s.id, user);
...
function dismissSuggestionById(user, id) {
  ...
  return result.changes
    ...
    : { ok: false, message: 'No open suggestion found.' };
}
