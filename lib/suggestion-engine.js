...
const SALIENCE_ADMISSION_CONTRACT = `Pipeline admission contract (this supplements any configured prompt above):
...
${salienceContexts.map(ctx => [
  ...
  const created = [];
  for (const s of (parsed.suggestions || []).slice(0, 3)) {
    if (!isAdmissibleOpportunitySuggestion(s)) continue;
