...
const { getSystemModelKey, getSystemModelId, getSystemPrompt } = require('./settings');
...
function fallbackNarrative({ title, capturedAt }) {
  return [
    `# ${title}`,
    '',
    ...
    '## Narrative',
    '',
    'The narrative model was unavailable. The faithful interview transcript is preserved in the dedicated canonical transcript section below.',
    '',
    '## Follow-ups',
    '',
...
async function buildNarrative({ transcript, title, capturedAt, user, model }) {
  ...
  const prompt = [
    ...
    'Use these sections: Narrative, Decisions, People, Projects, Actions, Open Questions.',
    'Do not reproduce or add a Transcript/Raw Transcript section; the system preserves the faithful raw transcript separately.',
  ...
  try {
    ...
    return fallbackNarrative({ title, capturedAt });
  }
}
...
function buildMarkdown({ user, title, capturedAt, source, narrative, transcript }) {
  return [
    ...
    '---',
    '',
    '## Transcript (Canonical Raw Evidence)',
    '',
    '<!-- hub:canonical-transcript:start -->',
    '',
    transcript.trim(),
    '',
  ].join('\n');
...
// provider: 'openrouter' forces the OpenRouter path and ignores any OPENAI_*
// key. Required by the OpenRouter-only rule, and it also avoids a live prod
// footgun: prod has OPENAI_TRANSCRIPTION_API_KEY set to an OpenRouter key
// (sk-or-...), which the default 'auto' path would send to api.openai.com and
// get a 401. Callers that must stay on OpenRouter should pass this explicitly.
async function transcribeAudioBuffer({ buffer, filename, mimetype, model, provider = 'auto' }) {
  if (provider === 'openrouter') {
    throw new Error('Audio transcription via OpenRouter is retired. Configure local Whisper (WHISPER_BIN) or a non-OpenRouter OPENAI_TRANSCRIPTION_API_KEY.');
  }

  // Never send sk-or-* keys anywhere — OpenRouter is retired.
  const rawOpenAiKey = process.env.OPENAI_API_KEY || process.env.OPENAI_TRANSCRIPTION_API_KEY;
  const openAiKey = rawOpenAiKey && !String(rawOpenAiKey).startsWith('sk-or-') ? rawOpenAiKey : null;
  if (openAiKey) {
    return transcribeWithOpenAI({ buffer, filename, mimetype, apiKey: openAiKey, model });
  }
  if (process.env.WHISPER_BIN) {
    throw new Error('Local Whisper binary is configured but the WHISPER_BIN integration path is not yet wired; transcription remains degraded without OpenRouter.');
  }

  throw new Error('Audio transcription needs a local Whisper path (WHISPER_BIN) or a non-OpenRouter OPENAI_TRANSCRIPTION_API_KEY. OpenRouter is retired.');
}

async function transcribeWithOpenAI({ buffer, filename, mimetype, apiKey, model }) {
  const boundary = `----mclellan-workday-${Date.now().toString(16)}`;
  const parts = [
    Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="model"\r\n\r\n${model || process.env.WORKDAY_TRANSCRIPTION_MODEL || 'gpt-4o-mini-transcribe'}\r\n`),
  ...
  const text = Object.prototype.hasOwnProperty.call(json, 'text') ? json.text : body;
  if (typeof text !== 'string') throw new Error('Transcription returned no text.');
...
async function transcribeWithOpenRouter({ buffer, filename, mimetype, apiKey, model: modelOverride }) {
  const fallback = process.env.WORKDAY_TRANSCRIPTION_MODEL || 'openai/whisper-large-v3';
  const model = modelOverride || getSystemModelId('workday_transcription', 'system', fallback);
  const started = Date.now();
  const res = await fetch('hub-model://v1/audio/transcriptions', {
    method: 'POST',
    headers: openRouterHeaders(TASK_CODES.WORKDAY_INGEST, { apiKey }),
    body: JSON.stringify({
      model,
      language: 'en',
      input_audio: {
  ...
  logUsageFromResponse({
    user: 'system',
    feature: 'workday_transcription',
    modelKey: 'workday_transcription',
    fallbackModelId: model,
    data: json,
    taskCode: TASK_CODES.WORKDAY_INGEST,
