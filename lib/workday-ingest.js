...
async function transcribeAudioBuffer({ buffer, filename, mimetype, model, provider = 'auto' }) {
  ...
  if (provider === 'openrouter') {
    if (!openRouterKey) throw new Error('Audio transcription needs OPENROUTER_API_KEY in the server environment.');
    return transcribeWithOpenRouter({ buffer, filename, mimetype, apiKey: openRouterKey, model });
  }

  // An OpenRouter key (sk-or-...) placed in an OPENAI_* var must not be sent to
  // api.openai.com — that only yields a 401. Treat it as absent so we fall
  // through to the OpenRouter path (this is what silently broke transcription
  // in prod, where OPENAI_TRANSCRIPTION_API_KEY holds the OpenRouter key).
  const rawOpenAiKey = process.env.OPENAI_API_KEY || process.env.OPENAI_TRANSCRIPTION_API_KEY;
  const openAiKey = rawOpenAiKey && !rawOpenAiKey.startsWith('sk-or-') ? rawOpenAiKey : null;
  if (openAiKey) {
