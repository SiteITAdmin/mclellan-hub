...
async function buildNarrative({ transcript, title, capturedAt, user, model }) {
  ...
  try {
    const result = await routeMessage({
      model: model || process.env.WORKDAY_NARRATIVE_MODEL || 'free',
      user,
      noSearch: true,
      messages: [
        { role: 'system', content: 'You convert dictated workday interviews into concise, searchable Markdown notes for a private knowledge base.' },
