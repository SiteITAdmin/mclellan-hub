...
async function deriveLesson(user, evidence, userReason) {
  ...
  const prompt = `${getSystemPrompt('task_rule_learner', 'system', PROMPTS.task_rule_learner)}
...
${existingRules.length ? existingRules.map(item => `- ${item.lesson_key}: ${item.rule}`).join('\n') : '(none)'}`;
  ...
  const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: openRouterHeaders(TASK_CODES.TASK_LEARNING),
    body: JSON.stringify({
      model: modelId,
  ...
  logUsageFromResponse({
    ...
    data,
    taskCode: TASK_CODES.TASK_LEARNING,
