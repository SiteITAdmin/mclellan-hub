...
function parseSuggestionsJson(text, { topic, tone, planDate }) {
...
const SYSTEM_INSTRUCTIONS = `You are driving a research pipeline for a LinkedIn content research suggestion tool. Given a topic, tone, and date:
...
3. Once you have evidence back from run_last30days AND your own web/X search results, identify 3 distinct, SPECIFIC angles for a LinkedIn post - each grounded in a real stat, quote, or story from the evidence. Do not write generic angles ("AI is changing everything") - anchor each one to something concrete you found.
4. Respond with ONLY a JSON object, no other text: {"suggestions": [{"title": "...", "summary": "...", "source_url": "...", "source_title": "..."}, ...]}. title is a specific, newsy headline (not generic). summary is 1-2 sentences citing the concrete evidence. source_url/source_title point to the single strongest piece of evidence for that angle.

Never fabricate a stat, quote, or URL. If evidence is thin for an angle, say so in the summary rather than inventing detail.`;

async function driveLast30DaysResearch({ user = 'system', topic, tone = 'professional', planDate, saveDir } = {}) {
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    if (finalText) {
      const suggestions = parseSuggestionsJson(finalText, { topic, tone, planDate });
