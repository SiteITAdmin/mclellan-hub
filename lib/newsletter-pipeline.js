...
{"topics":[{"headline":"...","summary":"...","category":"..."}]}`;
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: modelId,
        messages: injectPromptAddition('newsletter_extractor', 'system', [{ role: 'user', content: prompt }]),
...
${topicBlock}${wikiContext}`;
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: modelId,
      messages: injectPromptAddition('newsletter_briefing', user, [{ role: 'user', content: prompt }]),
      temperature: 0.4,
      max_tokens: maxTokens,
    }),
  });
...
async function generateCreatorBriefing({ user, creatorSlug, creatorName, dateFrom, dateTo }) {
  ...
  const prompt = `You are writing a personal intelligence briefing for Douglas McLellan based on articles from ${creatorName}.

Summarise the key ideas, arguments, and actionable insights from these articles (${from} to ${to}). Write in sections by theme or article. Be substantive — this is a reading digest, not a headline list. Aim for 200–400 words per article, focusing on what is most useful to someone working in AI strategy and technology leadership.
