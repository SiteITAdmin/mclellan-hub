...
${evidence.join('\n\n---\n\n')}`;
  ...
  const html = markdownToHtml(text, periodLabel, format.name);

  // Store briefing
  const id = uuid();
  hub.prepare(`
    INSERT INTO nl_briefings
      (id, user, week_key, format_id, format_name, topic_count, html_content, text_content, date_from, date_to)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, user, weekKey || getWeekKey(new Date(`${from}T12:00:00Z`)),
    format.id, format.name, items.length, html, text, from, to);

  return { id, text, html, topicCount: items.length, formatName: format.name, periodLabel, dateFrom: from, dateTo: to };
}

// ── Simple markdown → HTML for email ─────────────────────────────────────────

function markdownToHtml(md, periodLabel, formatName = 'Intelligence Briefing') {
  const body = md
    .replace(/^## (.+)$/gm, '<h2>$1</h2>')
    .replace(/^### (.+)$/gm, '<h3>$1</h3>')
...
async function sendBriefing({ user, briefingId }) {
  ...
  await amSend({
    ...
    html: markdownToHtml(briefing.text_content, briefingPeriodLabel(briefing), briefingFormatName(briefing)),
    attachments: [{
      filename,
...
function buildBriefingPdfHtml(textContent, weekLabel, formatName = 'Intelligence Briefing') {
...
${articleBlocks}`;
  ...
  const html = markdownToHtml(text, periodLabel, formatName);

  const id = uuid();
  hub.prepare(`
    INSERT INTO nl_briefings (id, user, week_key, format_id, format_name, topic_count, html_content, text_content, date_from, date_to)
    VALUES (?, ?, ?, NULL, ?, ?, ?, ?, ?, ?)
  `).run(id, user, `creator-${creatorSlug}-${from}`, formatName, articles.length, html, text, from, to);
