...
const EXTRACTION_PROMPTS = {
  ...
  full: (categoryList) => getSystemPrompt('newsletter_extractor_full', 'system', PROMPTS.newsletter_extractor_full)
    .replace('[CATEGORIES]', categoryList),

  minimal: (categoryList) => getSystemPrompt('newsletter_extractor_minimal', 'system', PROMPTS.newsletter_extractor_minimal)
    .replace('[CATEGORIES]', categoryList),
};
...
- Keep each content field under 20,000 characters.`;
  ...
  let attemptCount = 0;
  let extractionUsage = { tokensIn: 0, tokensOut: 0, costUsd: 0 };
  try {
    const result = await requestModelObject({
      ...
      defaults: { items: [] },
      label: 'Newsletter extraction response',
      // Retry with a smaller budget so a truncation-prone model can finish
      // the JSON object cleanly.
      maxTokens: attempt => (attempt === 1 ? maxTokens : 24000),
      retryInstruction: 'This is a retry. Return strictly valid JSON and reduce the number of items if necessary to finish the JSON object cleanly.',
      meta: true,
    });
    actualModelId = result.modelId;
    extractionUsage = result.usage;
    attemptCount = result.attempts;
    topics = Array.isArray(result.object.items) ? result.object.items : [];
  } catch (err) {
    attemptCount = 2;
    extractionError = err;
    console.warn(`[newsletter] extraction failed for "${email.subject}":`, err.message);
  }
  if (extractionError) {
  ...
  try {
    ...
    finishExtractionRun(runId, {
      status: 'failed',
      actualModelId: extractionError ? null : actualModelId,
      attemptCount,
      itemCount: inserted.length,
      tokensIn: extractionUsage.tokensIn,
      tokensOut: extractionUsage.tokensOut,
  ...
  finishExtractionRun(runId, {
    status: extractionError ? 'fallback' : 'complete',
    actualModelId: extractionError ? null : actualModelId,
    attemptCount,
    itemCount: inserted.length,
    tokensIn: extractionUsage.tokensIn,
...
function sourcePriority(item) {
  const priority = Number(item.briefing_priority);
...
async function retrieveIntelligenceItems({ user, focus, dateFrom, dateTo, limit = 40 }) {
  ...
  const candidates = hub.prepare(`
    SELECT i.*, d.title AS document_title, d.sender_name, d.sender_email,
           ...
           COALESCE(s.briefing_priority, 3) AS briefing_priority,
    ...
    WHERE i.user = ? AND i.selected = 1
      AND COALESCE(s.briefing_priority, 3) > 1
  ...
  const shortlist = candidates
    .map(item => {
      const lexicalScore = lexicalRelevance(item, terms);
      const priority = sourcePriority(item);
      return {
        ...item,
        lexical_score: lexicalScore,
        source_priority: priority,
        ranking_score: lexicalScore + ((priority - 3) * 2),
      };
    })
  ...
  if (!focus || !shortlist.length) {
    return shortlist
      .sort((a, b) => b.source_priority - a.source_priority || b.published_at - a.published_at)
      .slice(0, limit);
  }
  ...
  const parentModelId = getSystemModelId('newsletter_extractor', 'system', 'google/gemini-2.5-flash-lite');
  // Prefer a dedicated retrieval slot when configured; otherwise share the topic extractor model.
  const modelId = getSystemModelId('newsletter_retrieval', 'system', parentModelId);
  const system = getSystemPrompt('newsletter_retrieval', 'system', PROMPTS.newsletter_retrieval);
  try {
    const response = await fetch('hub-model://v1/chat/completions', {
      method: 'POST',
      headers: openRouterHeaders(TASK_CODES.NEWSLETTER_RETRIEVAL),
      body: JSON.stringify({
        model: modelId,
        messages: [{
          role: 'user',
...
${JSON.stringify(catalogue)}`,
    ...
    logUsageFromResponse({
      user,
      feature: 'newsletter_retrieval',
      modelKey: 'newsletter_retrieval',
      fallbackModelId: modelId,
      data,
      taskCode: TASK_CODES.NEWSLETTER_RETRIEVAL,
    });
...
${evidence.join('\n\n---\n\n')}`;
  ...
  logUsageFromResponse({
    ...
    fallbackModelId: modelId,
    data,
    taskCode: TASK_CODES.NEWSLETTER_BRIEFING,
  });
...
function mdToReportHtml(md) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (line.includes('|') && isMarkdownTableDivider(lines[index + 1])) {
      ...
      index -= 1;
      out.push('<div class="rpt-table-wrap"><table class="rpt-table">');
      out.push(`<thead><tr>${headers.map(cell => `<th>${inlineMd(cell)}</th>`).join('')}</tr></thead>`);
      out.push(`<tbody>${rows.map(row => `<tr>${row.map(cell => `<td>${inlineMd(cell)}</td>`).join('')}</tr>`).join('')}</tbody>`);
      out.push('</table></div>');
    } else if (/^## /.test(line)) {
      closeList();
...
.rpt-table-wrap{width:100%;margin:4mm 0 7mm}
.rpt-table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:8.2pt;line-height:1.4;color:#3d3b46}
.rpt-table thead{display:table-header-group}
.rpt-table tr{break-inside:avoid;page-break-inside:avoid}
.rpt-table th,.rpt-table td{border:1px solid #ded9e8;padding:2.2mm 2.5mm;text-align:left;vertical-align:top;overflow-wrap:anywhere}
.rpt-table th{background:#f1edff;color:#2f284f;font-weight:800}
.rpt-table tbody tr:nth-child(even){background:#faf9fd}
.rpt-table a{color:#5f4bd8;text-decoration-color:#c9c0ff;text-underline-offset:2px}
...
// ── Send to NoteMax (Google Drive) ─────────────────────────────────────────────

async function sendBriefingToNoteMax(briefingId, user) {
  ...
  const folderId = await resolveFolderPath(drive, ['Onyx', 'NoteMax', 'Briefing', briefingFormatName(briefing)]);
  ...
  return { ok: true, fileId: file.data.id, webViewLink: file.data.webViewLink, filename };
}
