...
// main fetch skips. This selects the subset worth full classification: a sender
// the taxonomy already has a rule for, or one a Gmail filter has already filed
// under a canonical label other than the high-volume Newsletters archive. The
// generic newsletter bulk is intentionally excluded so it stays appraisal-only.
...
async function processNewEmails(user) {
  ...
  // Promotions stay out of the main fetch, but Gmail files senders Douglas cares
  // about (MasterClass, Maven) there. The bounded lane below appraises promotions
  // for time-sensitive offers; on top of that we re-admit the *known/curated*
  // subset — senders with a taxonomy rule, or already filed by a Gmail filter
  // under a canonical label other than the Newsletters bulk archive — to full
  // classification, so they are summarised, labelled and taught to the learner
  // instead of being invisible. The ~1,100 generic newsletters are deliberately
  // left on the appraisal-only path.
  ...
  try {
    ...
    const promotionRetries = hub.prepare(`
      ...
      ORDER BY last_failed_at ASC LIMIT 10
    `).all();
      ...
      if (safeLabel && gmail) {
        try {
          ...
          hub.prepare(`
            ...
            SET gmail_label = ?, gmail_label_source = 'hub', gmail_label_checked_at = unixepoch()
...
module.exports = { appraisePromotionEmails, classifyEmail, processNewEmails, processSentEmails, selectKnownPromotionEmails };
