...
async function processNewEmails(user) {
  ...
  const sent = await processSentEmails(user, gmail).catch(err => {
...
async function processSentEmails(user, existingGmailClient) {
  const hub = db.hub();
  ...
  let emails;
  try {
    ({ emails } = await fetchSentEmails(user, existingGmailClient));
  } catch (err) {
