...
async function pollFiledLabels(user, gmail, emailLabels, seenEmailIds) {
  ...
  for (const name of emailLabels || []) {
    ...
    let fetched;
    try {
      fetched = await fetchMessagesByLabelId(gmail, labelId, sinceTs);
    } catch (err) {
  ...
  return { classifyEmails, intelTopics };
}
...
module.exports = { appraisePromotionEmails, classifyEmail, pollFiledLabels, processNewEmails, processSentEmails };
