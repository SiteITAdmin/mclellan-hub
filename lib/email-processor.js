...
function parseRyanairReservation(body) {
  // HTML itineraries contain the label "Reservation Number" before the PNR.
  // `Reservation:?` plus six alphanumerics therefore captured "Number".
  const matches = [...String(body || '').matchAll(/Reservation:\s*([A-Z0-9]{6})\b/gi)];
...
function detectRyanairFlights(email) {
  ...
  const ref = parseRyanairReservation(body);
...
function contactMatchesSender(contact, email, contacts = [contact]) {
  const identity = resolveContactIdentityFromLoaded(contacts, email.fromName, email.fromEmail);
  return identity.status === 'matched' && identity.contact.id === contact.id;
}

function resolveContactIdentityFromLoaded(contacts, name, address) {
  return resolveContactIdentityFromContacts(contacts, {
    names: [name],
    addresses: [address],
  });
}
...
function subjectExplicitlyMentionsContact(contact, email) {
  ...
  return contactNameTerms(contact).some(contactName => contactName.length >= 3
    && new RegExp(`(^|\\b)${contactName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(\\b|$)`, 'i').test(subject));
}
...
async function classifyEmail(email, contacts, projects, emailLabels, user = 'douglas', taskSource = 'email', identity = null) {
  const { formatLearnedTaskRules } = require('./task-learning');
  const contactList = contacts.length
    ? contacts.map(c => {
      const aliases = parseAliases(c.aliases);
      return `- ${c.name}${aliases.length ? ` (aka ${aliases.join(', ')})` : ''}${c.email ? ` <${c.email}>` : ''}`;
    }).join('\n')
    : '(none yet)';
...
- If uncertain about ANY field, set it to null — do not guess`;
  const prompt = (getSystemPrompt('email_classifier', 'system', PROMPTS.email_classifier || defaultPrompt)
    ...
    + `\n\nDeterministic source identity: ${identity?.contact
      ? `${identity.contact.name} [${identity.contact.id}] — exact ${identity.matchedBy.join(' + ')}. Use this canonical sender; do not substitute a similar name.`
      : 'unresolved. Do not guess from a similar name.'}`
...
// Gmail files curated senders (MasterClass, Maven) into Promotions, which the
// main fetch skips and the label poll cannot see until the message is labelled.
// This selects the subset worth full classification: a sender the taxonomy
// already has a rule for, or one a Gmail filter has already filed under a
// canonical label other than the high-volume Newsletters archive. The generic
// newsletter bulk is intentionally excluded so it stays appraisal-only.
const PROMOTION_ARCHIVE_LABEL = 'resources/newsletters';

async function selectKnownPromotionEmails(user, gmail, promotionEmails, emailLabels) {
  if (!promotionEmails || !promotionEmails.length) return [];
  const curatedNames = new Set(
    (emailLabels || [])
      .map(name => String(name || '').toLowerCase())
      .filter(name => name && name !== PROMOTION_ARCHIVE_LABEL)
  );
  let curatedLabelIds = new Set();
  try {
    const labelsResponse = await gmail.users.labels.list({ userId: 'me' });
    curatedLabelIds = new Set(
      (labelsResponse.data.labels || [])
        .filter(label => curatedNames.has(String(label.name || '').toLowerCase()))
        .map(label => label.id)
    );
  } catch (err) {
    console.warn('[email-promotions] label lookup failed; rule match only:', err.message);
  }
  return promotionEmails.filter(email => {
    if (matchEmailTaxonomy(user, email)) return true;
    return (email.labelIds || []).some(id => curatedLabelIds.has(id));
  });
}
...
async function retryPendingGmailLabels(user, gmail, emailLabels = []) {
  ...
  for (const row of pending) {
    ...
    try {
      ...
      if (isGmailNotFound(err)) {
        ...
        gone++;
        console.warn(`[email-label] message ${row.gmail_message_id} no longer exists (deleted) — skipping label`);
        continue;
      }
      failed++;
  ...
  return { retried: pending.length, moved, failed, gone };
}
...
async function processNewEmails(user) {
  ...
  const contacts = hub.prepare('SELECT id, name, email, phone, aliases FROM contacts WHERE user = ?').all(user);
  ...
  // bounded lane below still appraises them for time-sensitive offers. On top of
  // that we re-admit the known/curated subset — senders with a taxonomy rule, or
  // already filed by a Gmail filter under a canonical label other than the
  // Newsletters bulk archive — to full classification, so they are summarised,
  // labelled and taught to the learner even before any label exists on the
  // message. Without this, MasterClass/Maven mail that Gmail buries in
  // Promotions never reaches the classifier at all.
  let promotionResult = { appraised: 0, candidates: 0, suggestions: 0 };
  let knownPromotionEmails = [];
  try {
    ...
    for (const retry of promotionRetries) {
      ...
      try {
        ...
        if (isGmailNotFound(err)) {
          resolveProcessingFailure('gmail_promotion', retry.external_id);
          markGmailMessageGone(user, retry.external_id);
          console.warn(`[email-promotions] message ${retry.external_id} no longer exists (deleted) — skipping`);
          continue;
        }
    ...
    knownPromotionEmails = await selectKnownPromotionEmails(user, gmail, promotionEmails, emailLabels);
  } catch (err) {
  ...
  for (const promoEmail of knownPromotionEmails) {
    if (seenEmailIds.has(promoEmail.id)) continue;
    emails.push(promoEmail);
    seenEmailIds.add(promoEmail.id);
    console.log(`[email-promotions] admitting curated promotion to classifier: "${promoEmail.subject}"`);
  }
  ...
  for (const row of retryRows) {
    ...
    try {
      ...
      if (isGmailNotFound(err)) {
        resolveProcessingFailure('gmail', row.external_id);
        markGmailMessageGone(user, row.external_id);
        console.warn(`[email] message ${row.external_id} no longer exists (deleted) — clearing failure`);
        continue;
      }
  ...
  for (const row of capturedRows) {
    ...
    try {
      ...
      if (isGmailNotFound(err)) {
        resolveProcessingFailure('gmail', row.gmail_message_id);
        markGmailMessageGone(user, row.gmail_message_id);
        console.warn(`[email] raw capture ${row.gmail_message_id} no longer exists (deleted) — marking gone`);
        continue;
      }
  ...
  for (const email of emails) {
    try {
      const sourceIdentity = captureRawGmailEmail(hub, user, email);
      ...
      const result = await classifyEmail(email, enrichedContacts, projects, emailLabels, user, 'email', sourceIdentity);
      if (sourceIdentity.contact && !isAutomatedAlertEmail(email)) {
        result.contact_name = sourceIdentity.contact.name;
      }
      ...
      let contactId = sourceIdentity.contact?.id || null;
      let matchedContact = sourceIdentity.contact || null;
      if (result.contact_name && sourceIdentity.status !== 'ambiguous') {
        ...
        if (match) {
          ...
          const canAttachToContact = !isAutomatedAlertEmail(email)
            && (contactMatchesSender(enriched, email, enrichedContacts) || subjectExplicitlyMentionsContact(enriched, email));
...
async function classifySentEmail(email, contacts, projects, user = 'douglas', identity = null) {
  const contactList = contacts.length
    ? contacts.map(c => {
      const aliases = parseAliases(c.aliases);
      return `- ${c.name}${aliases.length ? ` (aka ${aliases.join(', ')})` : ''}${c.email ? ` <${c.email}>` : ''}`;
  ...
  const identityBlock = identity?.contact
    ? `\n\nDeterministic recipient identity: ${identity.contact.name} [${identity.contact.id}] — exact ${identity.matchedBy.join(' + ')}. Use this canonical person; do not substitute a similar name.`
    : '\n\nDeterministic recipient identity: unresolved. Do not guess from a similar name.';
  ...
  const modelId = getSystemModelId('email_classifier', 'system', 'google/gemini-2.5-pro-preview');
  return requestClassifierJson({
    prompt: prompt + identityBlock + knowledgeBlock,
    modelId,
...
async function processSentEmails(user, existingGmailClient, sinceTs) {
  ...
  for (const email of emails) {
    try {
      const sourceIdentity = captureRawGmailEmail(hub, user, email, { direction: 'sent' });
      const result = await classifySentEmail(email, contacts, projects, user, sourceIdentity);
