...
async function classifyEmail(email, contacts, projects, emailLabels) {
    ...
    try {
      ...
      for (const f of ryanairLegs) {
        ...
        if (!exists) {
          ...
          // Trigger mycelium to create prep/check-in tasks immediately
          const { connectFlightsToTasks } = require('./mycelium');
          setImmediate(() => connectFlightsToTasks(user, hub, []).catch(() => {}));
        }
      }

      const taxonomyMatch = matchEmailTaxonomy(user, email);
      const result = await classifyEmail(email, enrichedContacts, projects, emailLabels);
      ...
      if (isNewsletter(email)) {
        setImmediate(() => {
          extractTopicsFromEmail(email, user)
...
module.exports = { classifyEmail, processNewEmails };
