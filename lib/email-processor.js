...
const { fetchEmailById, fetchNewEmails, fetchSentEmails, moveToLabel } = require('./gmail');
...
async function requestClassifierJson({ prompt, modelId, feature, defaults, label }) {
  let lastError = null;
  const maxAttempts = 3;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      if (attempt > 1) {
        ...
        console.warn(`[email] ${feature} retrying in ${delay / 1000}s (attempt ${attempt}) after model response error`);
      ...
      if (!resp.ok) {
        // This is a network/API error, so we should retry.
      ...
      // This is where parseModelObject is called. If IT throws, the catch
      // block will handle it and trigger a retry.
      return parseModelObject(data.choices?.[0]?.message?.content, defaults, label);
    } catch (err) {
      lastError = err;
    }
  }

  const finalError = new Error(`[email] ${feature} failed after ${maxAttempts} attempts: ${lastError?.message}`);
