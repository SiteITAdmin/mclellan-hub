...
async function processNewEmails(user) {
  ...
  if (!emails.length) {
    if (process.env.EMAIL_POLL_DEBUG === '1') {
      console.log(`[email] no new emails for ${user}`);
    }
    return { processed: 0, moved: 0, learned: learned.length };
  }
  ...
  return { processed, moved, learned: learned.length };
}
...
Return ONLY valid JSON:
{
  "contact_name": "exact name from the contacts list if the TO address matches a known contact — otherwise null",
  "project_slug": "exact slug from the projects list if clearly relevant — otherwise null",
  "summary": "one plain-English sentence describing what Douglas communicated in this email",
  "fact": "a CRM fact in third person: what Douglas communicated, offered, committed to, or sent (e.g. 'Douglas sent the signed contract — June 2026', 'Douglas confirmed he will call on Thursday') — null only if nothing factually useful",
  "commitment_task": "if Douglas made a promise or said he would do something, a short imperative task title from his perspective (e.g. 'Send invoice to Tom by Friday', 'Call Sarah re: proposal') — otherwise null"
}

Rules:
- contact_name MUST match the TO address (recipient), not Douglas himself
- fact must be in third person and directly evidenced by the email body — do not invent
- commitment_task only when Douglas explicitly stated he would do something specific
- project_slug only when the email clearly belongs to that category — otherwise null`;
...
async function processSentEmails(user) {
  const hub = db.hub();
  ...
  let emails;
  try {
    ({ emails } = await fetchSentEmails(user));
  } catch (err) {
  ...
  for (const email of emails) {
    try {
      ...
      let matchedContact = null;
      if (result.contact_name) {
        const match = contacts.find(c => c.name.toLowerCase() === (result.contact_name || '').toLowerCase());
        if (match) { contactId = match.id; matchedContact = match; }
      }

      // Also try direct email match on the TO address
      if (!contactId) {
      ...
      `).run(
        ...
        email.sentAt, result.summary, validSlug, contactId
      );
      ...
      if (contactId && result.fact) {
        ...
        `).run(factId, user, contactId, result.fact);
        console.log(`[email-sent] CRM fact for ${result.contact_name}: ${result.fact}`);
        appendContactVaultEntry(
          matchedContact.name,
