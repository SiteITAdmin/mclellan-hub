...
// The subsystem a receipt is about, so the capo failure, the remediation pass
// that could not heal it, and the quality board it puts in veto all collapse
// onto one problem instead of reaching Douglas as three.
function subsystemKey(row, payload) {
  if (payload.capo) return String(payload.capo);
  if (row.source_kind === 'hub_module') return String(row.source_id);
  if (row.source_kind === 'hub_quality' || row.source_kind === 'hub_underboss') {
    return String(row.source_id).replace(/^(capo|underboss):/, '');
  }
  return `${row.source_kind}:${row.source_id}`;
}

// The check that actually failed and its evidence, wherever this receipt shape
// keeps them. A veto receipt only names the board check it tripped
// ("capo_checks_are_not_failing"), which tells a human nothing about what to
// fix, so the cause has to travel with the escalation.
function rootCause(payload) {
  if (payload.check) {
    return {
      root_check: String(payload.check),
      root_evidence: String(payload.check_evidence || ''),
      diagnosis: String(payload.advisory?.diagnosis || ''),
    };
  }
  const failing = Array.isArray(payload.checks) ? payload.checks.filter(c => c.verdict === 'fail') : [];
  if (failing.length) {
    return {
      root_check: failing.map(c => c.name).join(', '),
      root_evidence: failing.map(c => c.evidence).filter(Boolean).join('; '),
      diagnosis: '',
    };
  }
  return { root_check: '', root_evidence: '', diagnosis: '' };
}
...
function questionFor(row, payload, reason) {
  ...
  if (reason === 'veto') {
    // A board veto is computed from the capo beneath it and has no override
    // switch anywhere in the Hub; the only thing that clears it is the
    // underlying check passing.
    return `Fix the failing check behind ${target}; the veto clears on the next board run.`;
  }
  if (reason === 'failure') {
...
function escalationItems(rows) {
  ...
  for (const row of rows) {
    ...
    const facts = {
      subsystem: subsystemKey(row, payload),
      ...rootCause(payload),
      self_heal_attempted: row.source_kind === 'hub_remediation',
    };
