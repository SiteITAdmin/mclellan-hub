...
function buildResearchQuery({ topic, tone, date }) {
...
async function researchPlannedTopic(user, { date = dublinDate(), topic, tone = 'professional', limit = 3 } = {}) {
  ...
  const query = buildResearchQuery({ topic: cleanTopic, tone: normalizedTone, date: planDate });
  const searchResult = await currentInfoSearch(query, { provider: 'auto', days: 3, limit: Math.max(5, limit * 3) });
  ...
  const count = replaceSuggestions(user, { planDate, topic: cleanTopic, tone: normalizedTone, suggestions });
  return { ok: true, count, query, provider: searchResult.provider, warning: searchResult.warning || null };
}
