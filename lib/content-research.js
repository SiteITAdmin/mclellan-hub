...
function useGrokDriver() {
  return process.env.CONTENT_RESEARCH_DRIVER === 'grok';
}
...
async function researchPlannedTopic(user, { date = dublinDate(), topic, tone = 'professional', limit = 3 } = {}) {
  ...
  // Taxonomy description + searchQuery travel with the job / local driver so
  // Grok does not plan searches off the bare two-word label alone.
  const topicContext = resolveTopicContext(user, cleanTopic);

  // Per-user switch on the Plan page: off = plain web search, regardless of
  // which driver the host is configured for.
  const wantsLast30Days = getContentCadencePolicy(user).topicPlan.useLast30Days;

  // VPS path: enqueue for the Mac pull-worker (Grok+last30days lives on the mini).
  if (wantsLast30Days && useMacWorkerDriver()) {
  ...
  if (wantsLast30Days && useGrokDriver()) {
