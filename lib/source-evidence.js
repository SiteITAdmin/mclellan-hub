...
function sourceMaterial(sourceKind, row) {
  ...
  if (sourceKind === 'meeting_intake') {
    ...
    const { assessTranscriptIntelligibility, UNINTELLIGIBLE_COMPLETENESS } = require('./transcript-quality');
    let storedQuality = {};
    try { storedQuality = JSON.parse(string(row.extraction) || '{}').transcript_quality || {}; }
    catch (_) { storedQuality = {}; }
    const quality = assessTranscriptIntelligibility(transcript);
    const unintelligible = Boolean(quality.unintelligible || storedQuality.unintelligible);
    const hasTranscript = Boolean(transcript.trim());
    const completeness = unintelligible
      ? UNINTELLIGIBLE_COMPLETENESS
      : hasTranscript ? 'complete'
        : summary.trim() ? 'summary_only_missing_transcript'
          : 'missing_transcript';
    return {
      text: textWithHeader([
      ...
      complete: hasTranscript && !unintelligible,
      completeness,
      provenance: {
