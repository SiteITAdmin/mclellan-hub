...
function isCanonicalEvidenceExcluded(evidence) {
  if (evidence?.source_kind === 'document') return isGeneratedCanonicalDocument(evidence.row);
  if (evidence?.source_kind !== 'email_summary') return false;
  const row = evidence.row || {};
  if (['__skip', '__system'].includes(row.project_slug)) return true;
  ...
  if (reportSubject || Boolean(address && string(row.from_email).toLowerCase() === address)) return true;
  ...
  return row.project_slug == null && hasListUnsubscribeHeader(row);
}
...
function stableActionKey(evidence, span) {
  ...
  if (!evidence?.revision_hash || span?.exact !== true || !span?.chunk_id || !Number.isInteger(span.start) || !Number.isInteger(span.end)) {
    return null;
  }
  return hash([
