...
const RUNNERS = Object.freeze({
  ...
  m365_daily_briefing: Object.freeze({ env: 'M365_DAILY_BRIEFING_RUNNER', runner: 'claude', model: 'opus', effort: 'high' }),
  us_block_special_briefing: Object.freeze({ env: 'US_BLOCK_BRIEFING_RUNNER', runner: 'claude', model: 'opus', effort: 'high' }),
  nakai_briefing_quality_review: Object.freeze({ env: 'NAKAI_BRIEFING_QUALITY_REVIEW_RUNNER', runner: 'codex', model: 'gpt-5.6-luna', effort: 'medium' }),
...
function runProcess({ command, args, input, timeoutMs }) {
  return new Promise((resolve, reject) => {
    ...
    // A runner can reject the invocation before consuming stdin.  Keep that
    // broken pipe attached to the child result rather than crashing the pull
    // worker and leaving its VPS job claimed until the stale-claim window.
