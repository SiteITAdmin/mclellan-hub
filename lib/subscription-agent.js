...
const fs = require('fs');
const os = require('os');
const { resolveFeatureRunner, MODELS } = require('./feature-runners');
...
function configuredRunner(feature, { force = false } = {}) {
  ...
  return {
    ...
    tier: config.tier,
    fallbackTo: config.fallbackTo || null,
  };
}
...
function runProcess({ command, args, input, timeoutMs }) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', code => {
      ...
      if (code !== 0) {
        // Some CLIs print failures like "model at capacity" to stdout, leaving
        // stderr empty — include a stdout tail so stored job errors are diagnosable.
        const detail = (stderr.trim() || stdout.slice(-800)).trim();
        return reject(new Error(`local subscription runner exited ${code}: ${detail}`));
      }
      resolve(stdout);
    });
...
function fallbackConfig(config) {
  // One availability retry on the declared fallback lane (Opus → Grok 4.6).
  // Skipped when unset, unknown, or pointing at the same runner/model.
  if (!config?.fallbackTo) return null;
  const next = MODELS[config.fallbackTo];
  if (!next) return null;
  if (next.runner === config.runner && next.model === config.model) return null;
  if (!cliAvailable(next.runner)) return null;
  return {
    ...config,
    runner: next.runner,
    model: next.model,
    effort: next.effort || config.effort,
    tier: config.fallbackTo,
    fallbackTo: null,
  };
}

async function runWithConfig(config, { feature, systemPrompt, userPrompt, timeoutMs }) {
...
async function runSubscriptionText({
  ...
  try {
    return await runWithConfig(config, { feature, systemPrompt, userPrompt, timeoutMs });
  } catch (err) {
    const fb = fallbackConfig(config);
    if (!fb) throw err;
    console.warn(
      `[subscription-agent] ${feature}: ${config.runner}/${config.model} unavailable (${err.message}) — falling back to ${fb.runner}/${fb.model}`
    );
    const result = await runWithConfig(fb, { feature, systemPrompt, userPrompt, timeoutMs });
    return { ...result, fallbackFrom: `${config.runner}/${config.model}` };
  }
}
