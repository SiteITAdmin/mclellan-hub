...
function isJunkTitle(title) {
  ...
  if (/^\[email[^\]]*protected\]/i.test(title.trim())) return true;
  if (/^email:\s*/i.test(title.trim())) return true;
  // Titles that are just an email address
  if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(title.trim())) return true;
  return false;
}

function isJunkSynopsis(text) {
  if (!text) return false;
  return /email.{0,40}protection page|not.{0,30}regulatory document|technical email|email address rather than/i.test(text);
}
