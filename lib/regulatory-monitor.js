...
async function runRegulatoryMonitor() {
  ...
  if (allNew.length) {
    ...
    if (stored > 0) {
      const chatUser = process.env.GOOGLE_CHAT_USER || 'douglas';
      const preview = allNew.slice(0, 3).map(i => `• *[${i.site}]* ${i.title}`).join('\n');
      const more = allNew.length > 3 ? `\n_…and ${allNew.length - 3} more. Type "regs" to see all._` : '\n_Type "regs" to see details._';
      const chatMsg = `🔔 *${allNew.length} new regulatory item${allNew.length > 1 ? 's' : ''}*\n\n${preview}${more}`;
      const { deliverChat } = require('./reminders');
      deliverChat(chatUser, chatMsg).catch(e => console.warn('[reg-monitor] chat push:', e.message));
    }
  }
