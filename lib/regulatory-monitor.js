...
// Search is used only for discovery. Results are admitted to the monitor when
// their URL resolves to an official federal/state authority or a state
// financial regulator in the CSBS-backed registry. Press coverage can reveal
// that a story exists, but can never cross this authority boundary.
async function discoverOfficialUsRegulatoryItems(runId) {
  const hub = db.hub();
  const candidates = [];
  const providers = new Set();
  const warnings = [];

  for (const query of PRODUCT_REGULATORY_QUERIES) {
    const result = await currentInfoSearch(query, { provider: 'auto', days: 14, limit: 10 });
    if (result.provider) providers.add(result.provider);
    if (result.warning) warnings.push(result.warning);
    for (const source of officialDiscoverySources(result.sources)) {
      candidates.push({ ...source, query, provider: result.provider });
    }
  }
  ...
  const fresh = unique
    .filter(candidate => !hub.prepare('SELECT 1 FROM reg_monitor_items WHERE url = ? LIMIT 1').get(candidate.url))
    .slice(0, 12);
  const items = [];
  let relevantCount = 0;
  ...
  for (const candidate of fresh) {
    let item;
    try {
      const source = await fetchPublicationContent(candidate.url);
      const snippet = String(candidate.snippet || '').trim();
      if (snippet) source.text = `${source.text || ''}\n\nSearch discovery extract: ${snippet}`.trim();
      item = await assessPublication(candidate.regulator, candidate.title, candidate.url, source);
    } catch (err) {
      console.warn(`[reg-monitor] US official discovery assessment failed for ${candidate.url}: ${err.message}`);
      item = parseAssessment('', candidate.regulator, candidate.title, candidate.url, {
        status: `assessment-error: ${err.message}`,
        text: candidate.snippet || '',
      });
    }
    item.source_kind = 'official_search';
    item.detail_json = {
      discovery_query: candidate.query,
      search_provider: candidate.provider,
      authority_boundary: 'official-us-regulator',
    };
    ...
    items.push({ ...item, id, relevant, reason });
  }
  ...
  return {
    ...
    relevantCount,
    providers: [...providers].filter(Boolean).join('+') || 'none',
    warning: warnings.join('; ') || null,
  };
}

async function ingestOfficialUsRegulatoryUrl({ url, title, runId = null }) {
  const regulator = regulatorForUrl(url);
  if (!regulator) throw new Error(`URL is outside the official US regulator authority boundary: ${url}`);
  const existing = db.hub().prepare('SELECT id FROM reg_monitor_items WHERE url = ? LIMIT 1').get(url);
  if (existing) return { id: existing.id, skipped: true };
  const source = await fetchPublicationContent(url);
  const item = await assessPublication(regulator, title || url, url, source);
  item.source_kind = 'official_direct';
  item.detail_json = { authority_boundary: 'official-us-regulator', intake: 'direct-official-url' };
  const reason = exclusionReason(item);
  const relevant = reason === null;
  const id = storeAssessedItem(item, runId, relevant, reason);
  return { id, item, relevant, reason, skipped: false };
}
...
async function runRegulatoryMonitor() {
  ...
  try {
    console.log('[reg-monitor] checking US official regulator discovery…');
    const discovery = await discoverOfficialUsRegulatoryItems(runId);
    allItems.push(...discovery.items);
    sourceSummary.push({
      name: 'US official regulator discovery',
      discovery: true,
      linksExtracted: discovery.candidatesFound,
      newLinks: discovery.newLinks,
      relevantCount: discovery.relevantCount,
      provider: discovery.providers,
      error: !!discovery.warning,
      errorMessage: discovery.warning,
    });
    console.log(`[reg-monitor] US official regulator discovery: ${discovery.candidatesFound} official candidate(s), ${discovery.newLinks} new, ${discovery.relevantCount} relevant`);
  } catch (err) {
    console.error('[reg-monitor] US official regulator discovery error:', err.message);
    sourceSummary.push({
      name: 'US official regulator discovery',
      discovery: true,
      newLinks: 0,
...
module.exports = {
  ...
  discoverOfficialUsRegulatoryItems,
  ingestOfficialUsRegulatoryUrl,
  _test: {
    parseEsmaQaDetail,
    parseEsmaQaRows,
    qaAssessment,
