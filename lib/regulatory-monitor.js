...
async function ingestRegulatoryPublication({ regulator, title, url, runId = null }) {
  if (!regulator || !title) throw new Error('Regulator and title are required');
  let parsed;
  try { parsed = new URL(url); } catch { throw new Error(`Invalid regulatory publication URL: ${url}`); }
  if (parsed.protocol !== 'https:') throw new Error(`Regulatory publication URL must use HTTPS: ${url}`);
  const existing = db.hub().prepare(
    'SELECT id FROM reg_monitor_items WHERE site = ? AND url = ?'
  ).get(regulator, url);
  if (existing) return { id: existing.id, skipped: true };
  const source = await fetchPublicationContent(url);
  const item = await assessPublication(regulator, title, url, source);
