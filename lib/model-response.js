...
function parseModelObject(content, defaults, label = 'AI response') {
  ...
  if (typeof content === 'string') {
    ...
    try {
      ...
      // Common model failures, repaired in order: prose around the JSON object
      // ("Here is the JSON: {...}"), then raw control characters inside string
      // values, then unescaped double quotes inside string values.
      const s = stripped.indexOf('{');
      const e = stripped.lastIndexOf('}');
      const sliced = (s >= 0 && e > s) ? stripped.slice(s, e + 1) : stripped;
      try {
        value = JSON.parse(sliced);
      } catch (_) {
        try {
          value = JSON.parse(escapeControlCharsInStrings(sliced));
        } catch (_2) {
          try {
            value = parseRepairingUnescapedQuotes(escapeControlCharsInStrings(sliced));
          } catch (_3) {
