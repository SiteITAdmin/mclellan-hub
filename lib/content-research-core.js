...
const { getContentTopics, DEFAULT_TOPICS } = require('./content-taxonomy');
...
/** Taxonomy description + optional searchQuery for a named topic.
 *  Falls back to DEFAULT_TOPICS searchQuery when the user's saved taxonomy
 *  still has the abstract label with no practitioner query of its own. */
function resolveTopicContext(user, topicName) {
  const clean = String(topicName || '').trim().slice(0, 80);
  if (!clean) return { description: '', searchQuery: '' };
  const topic = getContentTopics(user).find((t) => t.name === clean);
  const fallback = DEFAULT_TOPICS.find((t) => t.name === clean);
  return {
    description: String(topic?.description || fallback?.description || '').trim().slice(0, 600),
    searchQuery: String(topic?.searchQuery || fallback?.searchQuery || '').trim().slice(0, 300),
  };
}

/** Inclusive 30-day window start (YYYY-MM-DD) for a plan date. */
function researchWindowStart(planDate) {
