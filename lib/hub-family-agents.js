...
function capoChecks(capoKey, user = 'douglas', options = {}) {
  ...
  switch (capoKey) {
    ...
    case 'crm': {
      ...
      try { journeyAudit = require('./crm-knowledge-engine').auditCrmKnowledgeJourneys(user, { nowTs: ts }); }
      catch (err) { journeyAudit = { error: err.message, issues: [], repairableIssues: [], escalations: [] }; }
      return [
        check('crm_knowledge_engine_health', warning ? 'fail' : 'pass', warning || 'CRM knowledge activity is healthy'),
        check(
          'crm_knowledge_journey_outcomes',
          journeyAudit.error ? 'warn' : journeyAudit.repairableIssues.length ? 'fail' : 'pass',
          journeyAudit.error
            ? `CRM journey audit unavailable (${journeyAudit.error})`
            : journeyAudit.repairableIssues.length
              ? `${journeyAudit.repairableIssues.length} CRM source journey(s) have a repairable unmet outcome: ${journeyAudit.repairableIssues.slice(0, 3).map(i => `${i.source_kind}:${i.source_id} ${i.reason}`).join('; ')}`
              : 'Every recent CRM source journey is complete or legitimately in progress',
          {
            purpose: journeyAudit.purpose,
            issue_count: journeyAudit.repairableIssues.length,
            issues: journeyAudit.repairableIssues.slice(0, 10).map(i => ({ source_kind: i.source_kind, source_id: i.source_id, stage: i.stage, reason: i.reason })),
          },
        ),
        check(
          'crm_knowledge_journey_escalations',
          journeyAudit.error ? 'warn' : journeyAudit.escalations.length ? 'fail' : 'pass',
          journeyAudit.error
            ? `CRM journey escalation audit unavailable (${journeyAudit.error})`
            : journeyAudit.escalations.length
              ? `${journeyAudit.escalations.length} CRM source journey(s) need model/config repair rather than replay: ${journeyAudit.escalations.slice(0, 3).map(i => `${i.source_kind}:${i.source_id} ${i.reason}`).join('; ')}`
              : 'No CRM journey requires model/config escalation',
          {
            purpose: journeyAudit.purpose,
            repairable: false,
            issue_count: journeyAudit.escalations.length,
            issues: journeyAudit.escalations.slice(0, 10).map(i => ({ source_kind: i.source_kind, source_id: i.source_id, stage: i.stage, disposition: i.disposition, reason: i.reason })),
          },
        ),
    ...
    case 'linkedin_content': {
      ...
      const receipted = recentPost?.id
        ...
        : null;
      let journeyAudit = null;
      try { journeyAudit = require('./linkedin-journey').auditLinkedInJourneys(user, { nowTs: ts }); }
      catch (err) { journeyAudit = { error: err.message, issues: [], waiting: [] }; }
      return [
        ...
        check(
          'linkedin_journey_outcomes',
          journeyAudit.error ? 'warn' : journeyAudit.issues.length ? 'fail' : 'pass',
          journeyAudit.error
            ? `LinkedIn journey audit unavailable (${journeyAudit.error})`
            : journeyAudit.issues.length
              ? `${journeyAudit.issues.length} LinkedIn journey(s) have no achieved outcome: ${journeyAudit.issues.slice(0, 3).map(i => `${i.post_id} ${i.action} (${i.reason})`).join('; ')}`
              : `${journeyAudit.waiting.length} journey(s) actively processing; all others reached a legitimate outcome`,
          {
            purpose: journeyAudit.purpose,
            issue_count: journeyAudit.issues.length,
            issues: journeyAudit.issues.slice(0, 10).map(i => ({ post_id: i.post_id, action: i.action, reason: i.reason, blockers: i.blockers })),
          },
        ),
      ];
    }
    ...
    case 'model_governance': {
      ...
      return [tableCountCheck('model_config', 'model config'), recentCountCheck('request_logs', 'ts', 'request logs', since7d), shapeCheck];
    }
    case 'token_burn': {
...
function writeUnderbossReceipt(underboss, user = 'douglas', options = {}) {
  ...
  const checks = [
    check('all_capos_have_purpose', reports.some(r => !r.purpose) ? 'fail' : 'pass', `${reports.filter(r => !r.purpose).length} capo(s) without a declared purpose`, { missing: reports.filter(r => !r.purpose).map(r => r.capo) }),
  ...
  const payload = {
    ...
    purpose: underboss.purpose || null,
