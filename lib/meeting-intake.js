...
function resolveOrCreateContact(hub, user, contacts, name, context = {}) {
...
function applySpeakerMap(transcript, speakerMap = {}) {
  ...
  for (const [label, name] of entries) {
    ...
    const re = new RegExp(`(^|\\n)(\\s*)${escaped}\\s*[:：-]`, 'gi');
    resolved = resolved.replace(re, (_, prefix, indent) => `${prefix}${indent}${name}:`);
  }
  return resolved;
}
...
async function previewMeetingIntake(user, intakeId) {
  ...
  const inputs = {
    transcript: cleanText(intake.transcript, 200000),
  ...
  const extraction = await extractMeetingIntelligence(user, inputs, context);
  const primaryProject = exactProject(context.projects, inputs.projectSlug)
    || exactProject(context.projects, (extraction.meeting || {}).projects?.[0]) || null;
...
async function processMeetingTranscript(user, {
  ...
  const inputHash = extractionInputHash({ transcript: cleanTranscript, title, meetingDate, projectSlug });
  const extraction = (existingExtraction.preview && existingExtraction.preview_hash === inputHash)
    ? existingExtraction.preview
    : await extractMeetingIntelligence(user, {
  ...
  for (const attendee of extractedMeeting.attendees || []) {
    const contact = findContact(context.contacts, attendee.matched_contact)
      || resolveOrCreateContact(hub, user, context.contacts, attendee.name, context);
  ...
  for (const update of extraction.crm_updates || []) {
    const project = exactProject(context.projects, update.project_slug)
      || findProject(context.projects, update.subject)
      || primaryProject;
    const subject = findContact(context.contacts, update.matched_contact)
      || resolveOrCreateContact(hub, user, context.contacts, update.subject, context);
  ...
  if (legacyDirectCrmWritesEnabled()) {
    for (const action of actionItems) {
      ...
      const project = exactProject(context.projects, action.project_slug) || primaryProject;
  ...
  const linkedProjectSlugs = [...new Set([
    ...
    ...derivedProjectNotes.map(note => note.project_slug),
    ...factsCreated.map(fact => fact.projectSlug),
  ].filter(Boolean))];
  for (const action of actionItems) {
    if (action.project_slug && !linkedProjectSlugs.includes(action.project_slug)) {
      linkedProjectSlugs.push(action.project_slug);
    }
  }
  ...
  if (existingIntake) {
    ...
    `).run(
      ...
      cleanTranscript, summary,
      // preview served its purpose once processed — drop it so the stored
      // extraction is the single authoritative copy
      JSON.stringify((({ preview, preview_hash, ...rest }) => rest)({ ...existingExtraction, ...extraction, action_register: actionItems })),
    ...
    `).run(
      ...
      cleanTranscript, summary, JSON.stringify({ ...extraction, action_register: actionItems }), JSON.stringify(counts)
    );
  }
...
function intakeDeleteInventory(user, intakeId) {
  ...
  const inventory = { meeting: null, tasks: 0, facts: 0, documents: 0 };
