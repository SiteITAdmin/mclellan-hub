...
function meetingMarkdown({ title, date, summary, attendees, outcomes, actionItems, projectNotes, openQuestions, transcript }) {
  const lines = [
    `# ${title}`,
    '',
    `Date: ${date}`,
...
async function processMeetingTranscript(user, {
  ...
  if (existingIntake) {
    ...
    `).run(
      ...
      cleanTranscript, summary, JSON.stringify({ ...existingExtraction, ...extraction, action_register: actionItems }), JSON.stringify(counts),
      intakeId, user
    );
  } else {
    hub.prepare(`
