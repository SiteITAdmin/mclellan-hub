...
const FIXERS = {
  ...
  crm_knowledge_journey_outcomes: { label: 'CRM journey repair',      jobType: 'crm_knowledge_repair',   mode: 'deferred' },
  linkedin_journey_outcomes:      { label: 'LinkedIn journey repair', jobType: 'linkedin_journey_repair', mode: 'deferred' },
};
...
function recentDispatchCount(capoKey, checkName, user, since, currentCheck = null) {
  try {
    const history = db.hub().prepare(`
      ...
      WHERE user = ? AND source_kind = ? AND stage = ? AND source_id = ? AND status = 'warn' AND created_at >= ?
        AND created_at > COALESCE((
          SELECT MAX(created_at) FROM knowledge_receipts
          WHERE user = ? AND source_kind = ? AND stage = ? AND source_id = ? AND status = 'pass'
        ), 0)
      ORDER BY created_at DESC, rowid DESC
    `).all(
      user, SOURCE_KIND, `${STAGE_PREFIX}:${capoKey}`, checkName, since,
      user, SOURCE_KIND, `${STAGE_PREFIX}:${capoKey}`, checkName,
    );
    const currentCount = Number(currentCheck?.details?.issue_count);
    if (Number.isFinite(currentCount) && history.length) {
      let prior = {};
      try { prior = JSON.parse(history[0].payload || '{}'); } catch (_) { prior = {}; }
      const priorCount = Number(prior?.check_details?.issue_count);
      // A shrinking backlog proves the dispatched repair is working. Start a
      // fresh retry budget instead of escalating merely because more cases remain.
      if (Number.isFinite(priorCount) && currentCount < priorCount) return 0;
    }
...
async function adviseAction({ capo, check, user }) {
  ...
  const prompt = [
    'You are the Hub remediation advisor. A health check on a background system has failed.',
    'You may ONLY recommend enqueuing one of these reversible job types, or decline:',
    ADVISOR_WHITELIST.map(t => `- ${t}`).join('\n'),
    '',
    `Capo (subsystem): ${capo.title}`,
    `Purpose: ${capo.purpose || 'No explicit purpose declared'}`,
    ...
    '{"diagnosis": string, "recommended_action": <one of the job types above, or "none">, "escalate": boolean, "message": string}',
...
async function remediateCheck({ capoKey, check, user = 'douglas', attempts = 2, nowTs = now(), useModel = true }) {
  ...
  if (!fixer && useModel && check.details?.repairable !== false) {
  ...
  if (fixer.mode === 'deferred') {
    ...
    const priorDispatches = recentDispatchCount(capoKey, check.name, user, nowTs - DISPATCH_WINDOW_SECONDS, check);
...
function writeRemediationReceipt(outcome, user = 'douglas', nowTs = now()) {
  ...
  const payload = {
    ...
    check_details: outcome.check.details || {},
