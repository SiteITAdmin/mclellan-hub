...
const FIXERS = {
  ...
  work_loop_guard_pending:         { label: 'Work-loop guard',        jobType: 'work_loop_guard',        mode: 'sync' },
  ...
  successful_work_loop:           { label: 'Work-loop containment',   jobType: 'work_loop_guard',        mode: 'deferred', warnActionable: false },
};
...
async function remediateFamily(user = 'douglas', options = {}) {
  ...
  for (const capo of CAPOS) {
    ...
    for (const check of checks) {
      ...
      const actionable = check.verdict === 'fail'
        || (check.verdict === 'warn' && FIXERS[check.name]?.warnActionable !== false && FIXERS[check.name]);
