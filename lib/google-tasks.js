...
async function repairTaskGoogleSync(user) {
  ...
  // Soft-deleted open tasks should not still sit on Google.
  ...
  // Index currently-open Google tasks once so we only delete what is still live.
  ...
  for (const list of allLists) {
    ...
    do {
      ...
      for (const t of resp.data.items || []) openRemote.set(t.id, list.id);
      pageToken = resp.data.nextPageToken;
    } while (pageToken);
  }

  for (const row of softDeleted) {
    const listId = openRemote.get(row.google_task_id);
    if (!listId) {
