...
const TASK_TAG_RE = /\[\s*(priority|effort|planner|after|assignee)\s*:\s*([^\]]+)\]/gi;

function parseTaskTags(notes) {
  const tags = { priority: null, effort_minutes: null, planner_lane: null, after: null, assignee: null };
...
function withTaskTags(notes, { priority, effortMinutes, plannerLane, after, assignee } = {}) {
  ...
  // after-dependency, and the assignee unless the caller explicitly supplies them.
