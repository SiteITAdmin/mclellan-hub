...
const TASK_TAG_RE = /\[\s*(priority|effort|planner)\s*:\s*([^\]]+)\]/gi;

function parseTaskTags(notes) {
  const tags = { priority: null, effort_minutes: null, planner_lane: null };
  String(notes || '').replace(TASK_TAG_RE, (_, key, value) => {
    ...
    if (key.toLowerCase() === 'planner' && ['work', 'personal'].includes(v)) tags.planner_lane = v;
    return '';
  });
  return tags;
}
...
function withTaskTags(notes, { priority, effortMinutes, plannerLane } = {}) {
  ...
  // Most existing callers edit only priority/effort. Preserve planner
  // selection unless the caller explicitly supplies plannerLane.
