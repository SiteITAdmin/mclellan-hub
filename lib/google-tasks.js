...
// Max Google Tasks lists pulled at once. One round-trip per project list adds up
// fast when done serially; 5 in flight keeps page loads snappy without risking
// the per-user Tasks API quota.
const SYNC_LIST_CONCURRENCY = 5;

// In-process clock of the last successful full sync per user. Every write path
// already updates the local cache, and a background job re-syncs every 15 min,
// so the blocking sync on a page load is only there to catch changes made in
// another Google client. syncTasksIfStale lets a GET skip that pull when a sync
// just ran — which is exactly the redundant work paid on the reload after a save.
...
// A task with a far-off start date does not belong in Douglas's working Google
// list yet — it lives on a dedicated "Later" list he can ignore in the Google
// Tasks app, and a nightly reconcile promotes it back to its home list once the
// start date is within START_DEFER_LEAD_DAYS. This is not a new store: the task
// is a normal Google task the whole time, carrying a Google-visible
// [start: YYYY-MM-DD] notes tag, so the single-door createTask/effect-gate and
// CRM dedup keep working. See CLAUDE.md → Task calendar planner (start dates).
const LATER_LIST_TITLE = 'Later — Hub';
const START_DEFER_LEAD_DAYS = 14;
...
// True when `startDate` (YYYY-MM-DD) is further out than the promotion window,
// i.e. the task should stay parked on the Later list and out of the planner.
function isStartDeferred(startDate, leadDays = START_DEFER_LEAD_DAYS) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(startDate || ''))) return false;
  return String(startDate).slice(0, 10) > addDaysIso(dublinTodayIso(), leadDays);
}

// The Later list id we last resolved for this user, read straight from
// crm_context without an API round-trip. Used to decide whether a task already
// sits on the Later list during an edit; null before the list has ever been
// ensured.
function cachedLaterListId(user) {
  const row = db.hub().prepare(
    "SELECT value FROM crm_context WHERE user = ? AND key = 'later_task_list_id'"
  ).get(user);
  return row?.value || null;
}

// Get or create the single "Later" Google Tasks list for a user and cache its id
// in crm_context. Verifies a cached id still exists so a list deleted in Google
// is re-created rather than written to blindly.
async function ensureLaterList(user, tasks = null) {
  const hub = db.hub();
  const client = tasks || getTasksClient(user);
  const cached = cachedLaterListId(user);
  if (cached) {
    try {
      await client.tasklists.get({ tasklist: cached });
      return cached;
    } catch (err) {
      if (!isNotFoundError(err)) throw err;
    }
  }
  const allLists = await getAllTaskLists(client);
  const match = allLists.find(l => l.title === LATER_LIST_TITLE)
    || allLists.find(l => l.slug === slugify(LATER_LIST_TITLE));
  let id = match?.id;
  if (!id) {
    const created = await client.tasklists.insert({ requestBody: { title: LATER_LIST_TITLE } });
    id = created.data.id;
    console.log(`[tasks] created "${LATER_LIST_TITLE}" list for ${user}`);
  }
  hub.prepare(`
    INSERT INTO crm_context (id, user, key, value)
    VALUES (?, ?, 'later_task_list_id', ?)
    ON CONFLICT(user, key) DO UPDATE SET value = excluded.value
  `).run(uuid(), user, id);
  return id;
}

// Which Google list a task should physically live on: the Later list when its
// start date is still beyond the promotion window, otherwise its project list
// (or the default list when unassigned). The task's project_slug is stored on
// the row regardless, so promotion always knows the home to return it to.
async function desiredListForTask(user, tasks, { projectSlug, startDate }) {
  if (isStartDeferred(startDate)) return ensureLaterList(user, tasks);
  return listIdForProject(user, tasks, projectSlug);
}
...
async function createTask(user, {
  ...
  // When the doer is someone other than Douglas, the task is captured but
  // attributed to them: it lands in "assigned to others" and never in Douglas's
  // planner. Written as a Google-visible [assignee: <name>] notes tag.
  assignee = null,
  ...
  // A task assigned to someone else is not Douglas's to schedule, so it carries
  // the assignee tag instead of a planner-ready effort default.
  const cleanAssignee = assignee ? String(assignee).trim().slice(0, 120) : null;
  const finalNotes = cleanAssignee
    ? withTaskTags(notes, { assignee: cleanAssignee })
    : withDefaultTaskEffort(notes);
  // A far-off start date parks the task on the Later list so it stays out of
  // Douglas's working Google list until promotion; project_slug is still stored
  // below so the nightly promote knows where to send it back.
  const startDate = parseTaskTags(finalNotes).start;
  const listId = await desiredListForTask(user, tasks, { projectSlug, startDate });
  const body = { title };
...
async function updateTask(user, localId, fields) {
  ...
  // Project ↔ Google list, and start date ↔ Later list. Reconcile when the
  // project or notes (which carry the [start:] tag) are edited, when a CRM
  // project is already set but the task sits on the wrong list, or when the task
  // currently sits on the Later list and may now be due to come back.
  ...
  const effectiveNotes = 'notes' in fields ? fields.notes : row.notes;
  const effectiveStart = parseTaskTags(effectiveNotes).start;
  const onLaterList = Boolean(cachedLaterListId(user)) && listId === cachedLaterListId(user);
  const shouldReconcileList = 'projectSlug' in fields || 'notes' in fields || Boolean(effectiveSlug) || onLaterList;
  if (shouldReconcileList) {
    const desiredListId = await desiredListForTask(user, tasks, { projectSlug: effectiveSlug, startDate: effectiveStart });
 ...
 */
async function syncTasks(user, { tasksClient } = {}) {
  const startedAt = Date.now();
  const hub = db.hub();
  const tasks = tasksClient || getTasksClient(user);
  ...
  // Fetch every list concurrently (bounded), then mutate SQLite single-threaded
  // in list order below — better-sqlite3 is synchronous, so the writes must not
  // interleave, but the network pulls can and should overlap.
  const fetched = await mapWithConcurrency(allLists, SYNC_LIST_CONCURRENCY, async (list) => {
    ...
    return { list, items: resp.data.items || [] };
  });

  for (const { list, items } of fetched) {
  ...
  lastFullSyncAt.set(user, Date.now());
  console.log(`[tasks] synced ${totalItems} task(s) across ${allLists.length} list(s) for ${user} in ${Date.now() - startedAt}ms`);
  return allSeenIds.map(id => ({ id }));
}

// Freshness-throttled sync for page-load GETs. A full pull is N sequential-ish
// Google round-trips; the reload right after a save was paying for it a second
// time even though the write already updated the local cache. Skip the pull when
// a sync (page load, write-through background job, or explicit /api/tasks/sync)
// completed within maxAgeMs. Returns false when skipped, true when it synced.
// Explicit "sync now" endpoints must keep calling syncTasks directly.
const SYNC_ON_LOAD_MAX_AGE_MS = 45 * 1000;

async function syncTasksIfStale(user, maxAgeMs = SYNC_ON_LOAD_MAX_AGE_MS, opts = {}) {
  const last = lastFullSyncAt.get(user) || 0;
  if (Date.now() - last < maxAgeMs) return false;
  await syncTasks(user, opts);
  return true;
}

// ── Complete / Delete / Restore ───────────────────────────────────────────────

async function completeTask(user, googleTaskId, { origin = null } = {}) {
  const hub = db.hub();
  const row = hub.prepare(
  ...
  `).run(googleTaskId);
  recordEffect(user, {
    effect: 'google_task_complete',
    origin,
    source: origin ? 'crm-engine' : 'manual',
    sourceId: row?.id || googleTaskId,
    title: row?.title || '',
    outcome: 'completed',
    externalId: googleTaskId,
  });
...
// sensitive.
// `assignee` names who is doing the task when it is NOT Douglas — either set by
// hand on the task page or attributed by the CRM engine from an action's owner.
// Its presence takes a task out of Douglas's calendar planner (it is not his to
// schedule) and moves it into the "assigned to others" list. Kept raw (case +
// spaces) like `after`, since it is a person's name, not an enum.
// `start` is a "not before" date ("[start: 2026-09-15]"): the task is not being
// worked until then. A far-off start parks the task on the Later list and keeps
// it out of the planner; resolveTaskFloor turns it into an earliest-start floor
// like `after`. Kept as a bare YYYY-MM-DD.
const TASK_TAG_RE = /\[\s*(priority|effort|planner|after|assignee|start)\s*:\s*([^\]]+)\]/gi;

function parseTaskTags(notes) {
  const tags = { priority: null, effort_minutes: null, planner_lane: null, after: null, assignee: null, start: null };
  String(notes || '').replace(TASK_TAG_RE, (_, key, value) => {
    ...
    else if (k === 'assignee' && raw) tags.assignee = raw.slice(0, 120);
    else if (k === 'start' && /^\d{4}-\d{2}-\d{2}$/.test(raw)) tags.start = raw;
    return '';
  });
  return tags;
}
...
function withTaskTags(notes, { priority, effortMinutes, plannerLane, after, assignee, start } = {}) {
  ...
  // Most existing callers edit only priority/effort. Preserve planner lane, the
  // after-dependency, the assignee, and the start date unless the caller
  ...
  const owner = assignee === undefined ? existing.assignee : (assignee ? String(assignee).trim().slice(0, 120) : null);
  if (owner) tags.push(`[assignee: ${owner}]`);
  const startDate = start === undefined ? existing.start : (start ? String(start).trim().slice(0, 10) : null);
  if (startDate && /^\d{4}-\d{2}-\d{2}$/.test(startDate)) tags.push(`[start: ${startDate}]`);
...
function getCachedTasks(user, filter = {}, includeHistory = false) {
  ...
  if (filter.projectSlug) {
    ...
    // Tasks belonging to a snoozed project drop out of every aggregate view
    // (the master task list, the planner inbox, contact/company task rollups).
    // They remain reachable on the project's own page, which always queries by
    // projectSlug, so a snoozed project can still be reviewed and woken.
    const { snoozedProjectSlugs } = require('./project-lifecycle');
    const snoozed = [...snoozedProjectSlugs(hub, user)];
    if (snoozed.length) {
      conditions.push(`(t.project_slug IS NULL OR t.project_slug NOT IN (${snoozed.map(() => '?').join(',')}))`);
      params.push(...snoozed);
    }
  }
  ...
  return rows.map(row => {
    ...
    if ((mapped.after || mapped.start) && mapped.status === 'needsAction' && !mapped.deleted_at) {
      try {
        const { applyTaskDependencyTiming } = require('./task-calendar-planner');
        return applyTaskDependencyTiming(user, mapped);
      } catch (err) {
        console.warn(`[tasks] dependency floor resolve failed for "${row.title}":`, err.message);
      }
    }
    mapped.is_overdue = mapped.status === 'needsAction' && !mapped.deleted_at
      && Boolean(mapped.deadline
        ? String(mapped.deadline).slice(0, 16) < new Date().toLocaleString('sv-SE', { timeZone: 'Europe/Dublin' }).slice(0, 16).replace(' ', 'T')
        : mapped.due && String(mapped.due).slice(0, 10) < dublinTodayIso());
    return mapped;
  });
}

// Promote any task parked on the Later list whose start date is now within the
// promotion window (or gone entirely) back to its home list — its project list,
// or the default list when unassigned. ID-stable via moveTaskToList, so planner
// provenance and CRM dedup survive the move. Runs off google_tasks_sync every
// 15 min, so a task reappears in Douglas's working list ~2 weeks before it
// starts. A Later-list task with no start tag is a stray and is promoted out
// rather than left stranded.
async function promoteDueStartTasks(user) {
  const hub = db.hub();
  const laterId = cachedLaterListId(user);
  if (!laterId) return { promoted: 0 };
  const rows = hub.prepare(`
    SELECT * FROM google_tasks
     WHERE user = ? AND task_list_id = ? AND status = 'needsAction'
  ...
  for (const row of rows) {
    const start = parseTaskTags(row.notes).start;
    if (isStartDeferred(start)) continue;
    const home = await listIdForProject(user, tasks, row.project_slug);
    if (!home || home === laterId) continue;
    try {
      await moveTaskToList(user, row, home, tasks);
      promoted++;
      console.log(`[tasks] promoted "${row.title}" from Later (start ${start || 'none'})`);
    } catch (err) {
      console.warn(`[tasks] promote failed for "${row.title}":`, err.message);
    }
  }
  return { promoted };
}
...
module.exports = {
  ...
  ensureLaterList, promoteDueStartTasks, isStartDeferred, LATER_LIST_TITLE, START_DEFER_LEAD_DAYS,
  parseTaskTags, stripTaskTags, withTaskTags, withPlannerTag, withDefaultTaskEffort,
  // test hooks
  _test: { isNotFoundError, toGoogleDueDate, slugify, addDaysIso, dublinTodayIso, desiredListForTask },
};
