...
const { MODEL_FAMILIES, FAMILY_LABELS, normalizeFamily, styleProfileBlock } = require('./model-style-profiles');
...
function pageTitleFromHtml(html = '') {
  ...
  return normalizeTitleText(htmlText(h1 || title || 'Imported HTML prompt kit').replace(/\s*\|\s*Unlock AI$/i, ''));
}

function promptTitleForPre(html, preIndex, fallbackTitle, ordinal) {
  const before = html.slice(0, preIndex);
  const articleStart = before.lastIndexOf('<article');
  const articleEnd = html.indexOf('</article>', preIndex);
  if (articleStart >= 0 && articleEnd > preIndex) {
    const article = html.slice(articleStart, articleEnd);
    const h3 = [...article.matchAll(/<h3[^>]*>([\s\S]*?)<\/h3>/gi)]
      .map(m => normalizeTitleText(htmlText(m[1])))
      .find(t => !isGenericPromptTitle(t));
    if (h3) return h3;
  }

  const headings = [...before.matchAll(/<h([2-4])[^>]*>([\s\S]*?)<\/h\1>/gi)]
    .map(m => normalizeTitleText(htmlText(m[2])))
    .filter(t => !isGenericPromptTitle(t));
  return headings.at(-1) || `${fallbackTitle} prompt ${ordinal}`;
}

function parsePromptKitHtml(html, sourceUrl = '') {
  const body = String(html || '');
  const kitTitle = pageTitleFromHtml(body) || 'Imported HTML prompt kit';
  ...
  let match;
  while ((match = preRe.exec(body)) !== null) {
    const rawPrompt = htmlText(match[1]);
    if (!rawPrompt || rawPrompt.length < 40) continue;
    const fingerprint = rawPrompt.replace(/\s+/g, ' ').trim().toLowerCase();
    if (seen.has(fingerprint)) continue;
    seen.add(fingerprint);
    const title = promptTitleForPre(body, match.index, kitTitle, prompts.length + 1);
    prompts.push({
      title,
      description,
  ...
  if (!prompts.length) {
    const mainText = htmlText(body.replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<style[\s\S]*?<\/style>/gi, ''));
    if (mainText) {
      prompts.push({
        title: kitTitle,
        description,
        raw_prompt: mainText,
        meta: { htmlFallback: true },
      });
    }
  }
  ...
  return {
    title: kitTitle,
    project: kitTitle,
    type: 'html-prompt-guide',
    label: 'HTML Prompt Guide',
    sourceUrl,
    intro: description,
    prompts,
  };
...
async function fetchPromptKitMarkdown(url) {
  ...
  if (ct.includes('text/html') && /<pre\b/i.test(text) && !text.includes('```prompt')) {
    return { markdown: text, markdownUrl, sourceUrl, format: 'html' };
  }
...
async function importPromptKitUrl(user, url, { purpose = '', force = false } = {}) {
  ...
  const { markdown, markdownUrl, sourceUrl, format } = await fetchPromptKitMarkdown(url);
  const kit = format === 'html'
    ? parsePromptKitHtml(markdown, sourceUrl)
    : parsePromptKitMarkdown(markdown, sourceUrl);
  const saved = [];
...
function savePrompt(user, input) {
  ...
  hub.prepare(`
    INSERT INTO prompt_library (
      ...
      target_model_family, created_at, updated_at
    )
    VALUES (
      @id, @user, @title, @description, @raw_prompt, @source_type, @source_ref, @source_title,
      @source_author, @source_url, @purpose, @function_type, @domain, @artifact_type,
      @interaction_type, @input_types, @tool_needs, @minimum_model_tier,
      @recommended_model_tier, @risk_level, @verification_type, @tags_json, @favourite,
      @target_model_family, unixepoch(), unixepoch()
    )
    ON CONFLICT(user, source_type, source_ref) DO UPDATE SET
      ...
      target_model_family = COALESCE(excluded.target_model_family, prompt_library.target_model_family),
  ...
  `).run({
    ...
    target_model_family: normalizeFamily(input.target_model_family || input.targetModelFamily) || null,
  });
...
function senseCheckPrompt(user, { roughPrompt = '', purpose = 'general', mode = 'quick', sourcePromptIds = [], targetModelFamily = '' } = {}) {
  ...
  const family = normalizeFamily(targetModelFamily);
  if (family) {
    const { getStyleProfile } = require('./model-style-profiles');
    if (!getStyleProfile(family)) {
      notes.push(`No style profile has been distilled for ${FAMILY_LABELS[family]} yet — the builder will run without model-specific shaping until the style profile job has run.`);
    }
    const familyMismatch = selectedExamples.filter(e => e.target_model_family && e.target_model_family !== family);
    if (familyMismatch.length) {
      issues.push(`Model family mismatch: ${familyMismatch.map(e => `"${e.title}" (${FAMILY_LABELS[e.target_model_family] || e.target_model_family})`).join(', ')} were shaped for a different model family than ${FAMILY_LABELS[family]}.`);
      suggestions.push('Reuse examples shaped for the same model family, or expect the builder to restyle borrowed structure for the selected family.');
    }
  }
  ...
  return {
    ...
    selected: {
      ...
      targetModelFamily: family || null,
      targetModelFamilyLabel: family ? FAMILY_LABELS[family] : null,
    },
    recommended: {
...
async function adaptPrompt(user, { roughPrompt, purpose = 'general', mode = 'quick', sourcePromptIds = [], targetModelFamily = '' } = {}) {
  const clean = String(roughPrompt || '').trim();
  if (!clean) throw new Error('Rough prompt is required');

  const family = normalizeFamily(targetModelFamily);
  const senseCheck = senseCheckPrompt(user, { roughPrompt: clean, purpose, mode, sourcePromptIds, targetModelFamily: family });
  ...
  const styleBlock = family ? styleProfileBlock(family) : '';
  ...
  const r = await fetch('hub-model://v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: modelId,
      messages: [
        { role: 'system', content: getSystemPrompt('prompt_adapter', 'system', PROMPTS.prompt_adapter) + styleBlock },
  ...
  const id = uuid();
  db.hub().prepare(`
    INSERT INTO prompt_adaptations (id, user, source_prompt_id, purpose, mode, raw_request, adapted_prompt, model_id, target_model_family)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, user, firstSource, normalizePurpose(purpose), mode || 'quick', clean, adapted, data.model || modelId, family || null);
  return {
    id,
    adapted,
    examples,
    ...
    targetModelFamily: family || null,
    styleProfileApplied: !!styleBlock,
  };
}
...
async function optimizePrompt(user, {
  ...
  const r = await fetch('hub-model://v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: modelId,
      messages: [
        { role: 'system', content: getSystemPrompt('prompt_optimizer', 'system', PROMPTS.prompt_optimizer) + styleBlock },
  ...
  db.hub().prepare(`
    INSERT INTO prompt_optimizations (
      ...
      logbook_json, model_id, target_model_family
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    id,
...
const PROJECT_ROOM_LABELS = Object.freeze([
  ...
  ['02_testing', 'Testing plans, samples, walkthrough notes, and workpapers generated during this run.'],
...
function parseProjectRoomLinks(text) {
  const lines = linesFromText(text);
  const byLabel = new Map();
  const extra = [];
  for (const line of lines) {
    const match = line.match(/^([A-Za-z0-9][A-Za-z0-9 _/-]{1,60})\s*(?::(?!\/\/)|=>|-)\s*(.+)$/);
    if (match) {
      const label = match[1].trim().replace(/\s+/g, '_');
  ...
  return { byLabel, extra };
}

function projectRoomTarget(input, label) {
  const { byLabel } = parseProjectRoomLinks(input.source_folders);
  return byLabel.get(label) || '';
}

function projectRoomMap(input) {
  const { byLabel, extra } = parseProjectRoomLinks(input.source_folders);
  const rows = PROJECT_ROOM_LABELS.map(([label, purpose]) => {
    const target = byLabel.get(label) || '[paste actual team folder link/path here if used]';
    return `- ${label}: ${target}\n  Purpose: ${purpose}`;
  });
  if (extra.length) {
    rows.push('Additional supplied source locations:');
    rows.push(...extra.map((line, index) => `- source_${String(index + 1).padStart(2, '0')}: ${line}\n  Purpose: Additional evidence or context supplied by the user. Classify it in the source inventory before using it.`));
  }
  return rows.join('\n');
}
...
function buildSharedAuditRules(input, stage) {
  const outputFolder = String(input.output_folder || '').trim() || projectRoomTarget(input, '05_outputs');
...
PROJECT ROOM MAP
Use these canonical labels in all plans, workpapers, evidence references, and handoffs even when the actual team folder names or shared-drive links differ. The folder/link after each label is the workspace Nakai supplied for this run.
${projectRoomMap(input)}

SOURCE WORKSPACE RULES
- Use only the supplied project room links, additional source locations, and user-provided context as audit evidence.
- Treat 00_source_evidence as read-only. Do not move, rename, delete, overwrite, or reclassify source evidence.
...
- If a canonical folder link is not supplied, return that artifact in chat and mark the missing folder as an evidence-room setup gap.
...
- Produce narrative deliverables as document files, preferably .docx. This includes scope notes, memos, draft findings, management packs, final reports, Audit Committee papers, executive summaries, talking points, and ready/not-ready verdicts.
- Produce tabular, tracker, testing, evidence, sample, action, issue, request, reconciliation, and log outputs as spreadsheet files, preferably .xlsx with clearly named tabs.
- Use Markdown only for chat responses, short run notes, or when the target tool cannot create document or spreadsheet files.
- Use CSV only as a simple interchange format when .xlsx is not available; do not make CSV the default final artifact when a workbook is expected.
- Name generated files with the audit name, stage, artifact purpose, and date or version where practical.
...
Act as the Orchestrator Agent for this audit stage. Manage the listed specialist agents, run the Head of Internal Audit reviewer challenge, maintain an evidence register, and produce only reviewable outputs that are tied to source material.

MANAGED AGENT OPERATING MODEL
You must manage the work as a small internal audit agent team, even if only one AI tool is executing the prompt.

- Orchestrator Agent: Own the stage plan, sequence the work, assign tasks to specialist agents, maintain the evidence register, track open items, and decide when the work is blocked or ready for review.
- Specialist Agents: Use the stage agent roster for focused work such as evidence indexing, testing, finding development, report drafting, action tracking, or committee summarisation.
- Reviewer Agent: Act in the persona of a Head of Internal Audit for a global fintech. Challenge the work with executive-level audit judgement, evidence discipline, regulatory awareness, independence, and professional scepticism.
- The Reviewer Agent must review conclusions before anything is treated as final, specifically checking for unsupported claims, overstated assurance, missing caveats, weak evidence, rating inconsistency, unclear management accountability, and gaps against the audit methodology.
- The Orchestrator Agent must document the Reviewer Agent's challenges, required fixes, unresolved disagreements, and final ready/not-ready verdict.
...
- Prefer .docx for narrative deliverables and .xlsx for inventories, trackers, workpapers, logs, and testing artifacts.
...
- Keep generated artifacts in the output folder, use the required .docx/.xlsx output formats where tool support allows, and name them by audit stage and purpose.
...
1. Confirm scope, project-room links, criteria, allowed actions, and stage-specific success conditions.
2. Build or update 01_inventory before drafting conclusions: source inventory, authority map, duplicate/version log, conflict log, and evidence register.
3. Keep testing work in 02_testing, draft findings in 03_findings, challenge notes in 04_review, and shareable outputs in 05_outputs.
4. Assign stage tasks to the listed agents or use the harness prompts as separate runs.
5. Record every conclusion against source evidence, judgement, management assertion, or evidence gap.
6. Run the challenge/review agent before anything is shared outside Internal Audit.
7. Produce the final stage pack with outputs, evidence map, open items, and next-step recommendation.
