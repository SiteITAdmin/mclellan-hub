...
async function synthesiseJson(prompt, user, feature = 'work-daily-brief-json') {
  const modelId = getSystemModelId('work_daily_brief', 'system', 'anthropic/claude-haiku-4-5');
  const started = Date.now();
  const resp = await fetch(OR_URL, {
    method:  'POST',
  ...
  const data = await resp.json();
  logUsageFromResponse({
    user, feature, modelKey: 'work_daily_brief',
    fallbackModelId: modelId, data, durationMs: Date.now() - started,
    taskCode: TASK_CODES.DAILY_DIGEST,
  });
  return parseModelObject(
    ...
    { highlights: [] },
    'Work brief project salience response'
  );
}
...
async function gatherProjectSalience(user, radarItems = []) {
  ...
  const prompt = `Today is ${fmtDate(dublinToday())}. You are reading Douglas McLellan's daily work briefing.
