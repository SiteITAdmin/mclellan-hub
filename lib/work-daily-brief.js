...
function buildHtml(data, yesterdayNarrative, todayNarrative) {
...
function buildText(data, yesterdayNarrative) {
  const { today, yesterday, meetingsToday, meetingsUpcoming, tasksDueSoon, allOpenTasks,
  ...
  if (meetingsToday.length) {
    for (const m of meetingsToday) lines.push(`  ${m.meeting_time ? m.meeting_time.slice(0,5) : '--:--'} ${m.title}${m.attendees ? ' · ' + m.attendees : ''}`);
  } else {
...
async function sendWorkDailyBrief(user = 'douglas') {
  ...
  const html    = buildHtml(data, yesterdayNarrative, todayNarrative);
  const text    = buildText(data, yesterdayNarrative);
...
module.exports = { sendWorkDailyBrief };
