...
.write { flex: 1; min-height: 2in; border: 1px solid #999; margin-top: 0.08in;
  background-image: radial-gradient(#c4c4c4 0.6px, transparent 0.6px);
  background-size: 0.2in 0.2in; background-position: 0.1in 0.1in; }
...
const TASK_SECTIONS = [
  ...
  { slug: 'assigned', heading: 'Assigned to others', list: 'assigned' },
];
...
function tasksPages(model) {
  ...
  for (const section of TASK_SECTIONS) {
    ...
    groups.forEach((group, index) => {
      const rows = group.map(task => taskRowHtml(model, task, section.blocks
