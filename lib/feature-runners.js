...
// Luna and Terra moved from Codex to Grok on 4 August 2026 after Codex hit its
// usage limit (951/951 calls failed, no reset until 2 September).
//
// Grok then ran out of credits on 8 August 2026, taking the same ~80% of the
// Hub's reasoning down with it — email classification, CRM triage/duplicate/
// action projection, atom extraction. The two bulk tiers (Luna/Terra) now point
// at the OpenCode free tier (opencode/deepseek-v4-flash-free, $0, no quota) via
// the `opencode` CLI — the same binary that serves the opencode/T3 session. It
// keeps the Hub flowing until Grok credits return. Sonnet/Opus stay on Claude.
//
// Flip back by reinstating the intended runner in MODELS below (e.g.
// luna -> grok, terra -> grok) when the Grok subscription resets.
const MODELS = Object.freeze({
  luna: Object.freeze({ runner: 'opencode', model: 'deepseek-v4-flash-free', effort: 'low' }),
  terra: Object.freeze({ runner: 'opencode', model: 'deepseek-v4-flash-free', effort: 'medium' }),
