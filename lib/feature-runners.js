...
// Luna and Terra moved from Codex to Grok on 4 August 2026 and stay there.
// Codex hit its subscription usage limit that day — 951 of 951 calls failed,
// with no reset until 2 September — which silently took out email
// classification, CRM source triage and action projection, roughly 80% of the
// Hub's reasoning. Grok has its own quota and was carrying almost nothing.
// The tier names are unchanged so every feature mapping below still reads as
// task-family-first; only the runner behind them differs.
const MODELS = Object.freeze({
  luna: Object.freeze({ runner: 'grok', model: 'grok', effort: 'low' }),
  terra: Object.freeze({ runner: 'grok', model: 'grok', effort: 'medium' }),
  sonnet: Object.freeze({ runner: 'claude', model: 'sonnet', effort: 'high' }),
  opus: Object.freeze({ runner: 'claude', model: 'opus', effort: 'high' }),
  grok: Object.freeze({ runner: 'grok', model: 'grok', effort: 'high' }),
...
const FEATURE_RUNNERS = Object.freeze({
  ...
  meeting_intake: slot('terra', { maxInputChars: 100000, maxEvidenceRecords: 1, escalateTo: 'sonnet' }),
  'meeting-intake': slot('terra', { maxInputChars: 100000, maxEvidenceRecords: 1, escalateTo: 'sonnet' }),
  ...
  work_daily_brief: slot('terra', { jsonMode: false, escalateTo: 'sonnet' }),
  work_brief_recap: slot('terra', { jsonMode: false }),
  ...
  repair_agent: slot('grok', { allowTools: true, jsonMode: false, timeoutMs: 300000 }),
