...
function admitSource(user, sourceKind, sourceIdOrRow, {
  ...
  const excluded = isCanonicalEvidenceExcluded(evidence);
  ...
  const status = excluded ? 'skipped' : evidence.complete ? 'done' : 'review';
  const summary = excluded
    ? `${ingester} captured an excluded source (${evidence.completeness}).`
    : evidence.complete
...
function admissionHealth(user, { sinceSeconds = 86400, database = null } = {}) {
  ...
  for (const row of rows) {
    ...
    const entry = byIngester.get(key) || {
      ingester: key, captured: 0, complete: 0, incomplete: 0, unreadable: 0, excluded: 0, reasons: {},
    };
    entry.captured += 1;
    if (row.status === 'error') {
      entry.unreadable += 1;
