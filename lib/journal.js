...
async function buildJournalEntry(user, transcript, date = new Date()) {
  ...
  const crmStr = activeFacts.length
    ? activeFacts.map(f => `- ${f.name}: ${f.fact}`).join('\n')
    : '(no active items)';
...
- Use [[Name]] wikilink syntax for people (e.g. [[Dad]], [[Wayne]]), projects (e.g. [[Beacon]], [[M365]]), and significant topics.
