...
function journalFilename(date) {
  const isoDate = date.toLocaleDateString('sv-SE', { timeZone: 'Europe/London' });
...
async function writeJournalEntry(user, transcript, date = new Date()) {
  ...
  const tagLine = `tags: [${tags.map(t => JSON.stringify(t)).join(', ')}]`;

  // Compose the note section
  const lines = [
    `## Entry - ${date.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/London' })}`,
    ``,
    `*${dateStr}*`,
    ``,
    `> Morning briefing: ![[${isoDate}#Morning Briefing]]`,
    ``,
    withLinks,
  ];
  ...
  // Cross-reference back from the daily note so the two are linked
  try {
    writeNote({
      notePath: `Daily/${isoDate}.md`,
      content: `\n\n## Evening Journal\n_[[${filename}]]_\n`,
      mode: 'append',
    });
  } catch (_) {}
...
module.exports = { writeJournalEntry, injectWikilinks };
