...
const SENSITIVE_PATH_RE = /(?:^|\/)(?:\.env(?:[.~_-].*)?|\.git(?:\/.*)?|\.aws(?:\/.*)?|\.npmrc|\.yarnrc|\.kube(?:\/.*)?|wp-config\.php|[^/]*(?:credential|client[_-]?secret|service[_-]?account|private[_-]?key)[^/]*\.(?:json|ya?ml|ini|txt)|secrets?\.(?:json|ya?ml|ini|txt))$/i;

function parseLines(lines, since = Date.now() - 24 * 60 * 60 * 1000) {
  const uniqueIps  = new Set();
  const pages      = {};
  const referrers  = {};
  const countries  = {};
  const statuses   = {};
  const securityIps = new Set();
  const securityPaths = {};
  ...
  for (const line of lines) {
    ...
    if (SENSITIVE_PATH_RE.test(url.split('?')[0])) {
      securityProbes++;
      securityIps.add(ip);
      securityPaths[url] = (securityPaths[url] || 0) + 1;
      if (/^4\d\d$/.test(status)) securityBlocked++;
      continue;
    }
    ...
    const successful = /^[23]\d\d$/.test(status);

    if (isPage && successful) {
      pageViews++;
...
function parseLog() {
  ...
  return parseLines(fs.readFileSync(LOG_FILE, 'utf8').split('\n').filter(Boolean));
}
...
function formatStats(stats, date) {
  ...
  if (stats.security?.probes) {
    const allBlocked = stats.security.blocked === stats.security.probes;
    lines.push(
      '',
      `Security probes:        ${stats.security.probes} from ${stats.security.sourceIps} IP${stats.security.sourceIps === 1 ? '' : 's'}`,
      `Sensitive-file access:  ${allBlocked ? 'all blocked (4xx)' : `WARNING: only ${stats.security.blocked}/${stats.security.probes} returned 4xx`}`,
    );
    for (const [url, count] of top(stats.security.paths, 5)) {
      lines.push(`  ${url.padEnd(35)} ${count}`);
    }
  }
...
module.exports = { formatStats, parseLines, sendRhStats };
