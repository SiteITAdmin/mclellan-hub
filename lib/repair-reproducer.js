...
// The snapshot tree lives OUTSIDE the iCloud-synced ~/Documents. A launchd job
// appending to a cloud-evicted file under Documents fails with EDEADLK
// ("Resource deadlock avoided"), and iCloud would otherwise sync the whole
// ~327MB prod DB copy up to Apple. Keep it on local-only Library. The bash
// snapshot scripts default to the identical path.
const SNAPSHOT_ROOT = process.env.HUB_SNAPSHOT_ROOT
  || path.join(os.homedir(), 'Library', 'Application Support', 'mclellan-hub', 'prod-snapshots');
const SNAPSHOT_DIR = path.join(SNAPSHOT_ROOT, 'latest');
