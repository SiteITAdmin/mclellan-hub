...
//   3. knowledge_receipts  — truthful stage/journey errors caught before a job throws
//   4. hub.service.log     — runtime error lines captured with the snapshot
//
...
const ERROR_CLASSES = [
  ...
  {
    name: 'upstream_permanent_http',
    pattern: /(?:OpenRouter|AgentMail|Gmail API)\s+(400|401|402|403|404|405|410|422)\b/i,
    deterministic: false,
    fixable: false,
    verdictHint: 'config_fix',
  },
...
const LOG_TAGS = {
  ...
  '[content]': { capo: 'linkedin_content', job_type: 'linkedin_journey_repair' },
  '[linkedin-journey]': { capo: 'linkedin_content', job_type: 'linkedin_journey_repair' },
...
function sourceForJobType(type) {
  ...
  if (t.includes('linkedin')) return { capo: 'linkedin_content' };
  if (t.includes('crm')) return { capo: 'crm' };
  if (t.includes('synthesis') || t.includes('knowledge')) return { capo: 'knowledge' };
  return { capo: 'hub' };
}

// Mine the four snapshot sources for candidate failures. Returns raw
...
function mineSnapshot({ snapshotDir = SNAPSHOT_DIR, sinceSeconds = 3 * 86400 } = {}) {
  ...
  // Some prompt-led pipelines deliberately catch per-source errors so one bad
  // item does not crash the whole batch. Their error receipts are still real
  // failures and must reach the same reproducer/repair venue as thrown errors.
  try {
    const receipts = db.prepare(`
      SELECT source_kind, source_id, stage, summary, payload, created_at
      FROM knowledge_receipts
      WHERE status = 'error' AND created_at >= ?
        AND (
          stage IN ('crm_source_triage','crm_duplicate_reviewed','crm_action_projected','crm_knowledge_synthesised')
          OR stage IN ('agent:linkedin_artifact','agent:linkedin_managing_editor','agent:linkedin_journey_supervisor')
        )
    ...
    for (const row of receipts) {
      ...
      const message = String(payload.error || row.summary || '').trim();
      if (!message) continue;
      const linkedin = row.source_kind === 'linkedin_post' || row.stage.includes('linkedin');
      candidates.push({
        origin: 'knowledge_receipts',
        capo: linkedin ? 'linkedin_content' : 'crm',
        job_type: linkedin ? 'linkedin_journey_repair' : 'crm_knowledge_repair',
        message,
        attempts: 1,
        resolved: false,
        sample_input_hash: sha256(`${row.source_kind}:${row.source_id}:${row.stage}`),
