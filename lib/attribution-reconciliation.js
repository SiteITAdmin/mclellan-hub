...
const AUTO_UNLINK_CONFIDENCE = 0.97;
const FIRST_PERSON = /\b(?:i|i'm|i’m|i'll|i’ll|i've|i’ve|my|me)\b/i;
...
function factSubjectGate(decision, packet) {
  ...
  if (FIRST_PERSON.test(decision.evidence_quote) && speakerMatchesContact(speaker, proposed, packet.contacts)) return null;
  return 'fact_subject_not_explicit_in_evidence';
}

function currentContactFrom(decision, packet) {
  if (decision.current_contact?.id) {
    return packet.contacts.find(contact => contact.id === decision.current_contact.id) || decision.current_contact;
  }
  return contactForName(packet.contacts, decision.current_contact?.name || decision.written_name);
}

// True when the compiled fact is actually about the linked person (they are
// named in it, or they are speaking a first-person self-claim). Otherwise the
// person is only the speaker of an organisational/project fact.
function personIsFactSubject(decision, packet) {
  const current = currentContactFrom(decision, packet);
  if (!current) return false;
  if (mentionsContact(decision.context, current, packet.contacts)) return true;
  const speaker = speakerForEvidence(packet.intake.transcript, decision.evidence_quote);
  return FIRST_PERSON.test(decision.evidence_quote) && speakerMatchesContact(speaker, current, packet.contacts);
}

function distinctNamedPeople(review = {}) {
  const names = [];
  const add = value => {
    const cleaned = norm(value);
    if (!cleaned) return;
    if (['unresolved', 'douglas/unassigned', 'unknown'].includes(cleaned)) return;
    if (/^speaker[_\s-]?\d+$/.test(cleaned)) return;
    names.push(cleaned);
  };
  add(review.current_contact?.name);
  add(review.proposed_contact_name);
  add(review.written_name);
  for (const owner of review.owners || []) add(owner?.name);
  const unique = [];
  for (const name of names) {
    const idx = unique.findIndex(other => other === name || other.startsWith(`${name} `) || name.startsWith(`${other} `));
    if (idx < 0) unique.push(name);
    else if (name.length > unique[idx].length) unique[idx] = name;
  }
  return unique;
}

// /crm/questions is for decisions a human can actually make (this person or
// that person; was this scheduled attendee in the room). Organisational facts
// hanging off a speaker, inexact quotes, and "the model is unsure" are not
// questions — they are either auto-applied or dropped.
function isAskableAttributionReview(review = {}) {
  const kind = String(review.target_kind || '');
  const verdict = String(review.verdict || '');
  const gate = String(review.gate_reason || '');
  if (kind === 'fact_subject') return false;
  if (kind === 'task_owner_conflict' || gate === 'linked_action_outcomes_disagree_on_owner') return true;
  if (kind === 'attendee') {
    if (verdict === 'remove_link') return false;
    const label = review.written_name || review.current_contact?.name;
    if (/^(speaker|participant)[_\s-]?\d*$/.test(norm(label))) return false;
    return true;
  }
  if (gate === 'scheduled_attendee_requires_review') return true;
  if (verdict === 'correct_link' && (kind === 'action_owner' || kind === 'projected_action_owner')) return true;
  if (verdict === 'needs_review' && (kind === 'action_owner' || kind === 'projected_action_owner')) {
    return distinctNamedPeople(review).length >= 2;
  }
  return false;
}
...
function automaticDecision(decision, packet) {
  ...
  if (decision.verdict === 'remove_link') {
    if (decision.target_kind === 'fact_subject') {
      if (personIsFactSubject(decision, packet)) {
        return { automatic: false, reason: 'fact_still_about_current_person' };
      }
      return decision.confidence >= AUTO_UNLINK_CONFIDENCE
        ? { automatic: true }
        : { automatic: false, reason: 'below_auto_unlink_confidence' };
    }
    return { automatic: false, reason: 'destructive_unlink_requires_review' };
  }
  return { automatic: false, reason: 'model_requested_review' };
}
...
async function reconcileOneMeeting(user, intake, {
  ...
  for (const decision of normalized) {
    ...
    if (!gate.automatic) {
      if (['already_correct', 'target_already_linked', 'human_correction_is_authoritative'].includes(gate.reason)) {
        ...
        const item = reviewItem(decision, gate.reason, intake);
        if (isAskableAttributionReview(item)) reviews.push(item);
      }
      continue;
    }
    try {
  ...
  for (const oldReview of priorPayload.reviews || []) {
    ...
    if (!targetStillExists
      ...
      || newReviewTargets.has(oldReview.target_key)
      || !isAskableAttributionReview(oldReview)) continue;
...
function reviewAsDecision(review) {
  return {
    target_key: review.target_key,
    target_kind: review.target_kind,
    verdict: review.verdict,
    contact_id: review.proposed_contact_id || null,
    contact_name: review.proposed_contact_name || null,
...
function latestReviewReceipts(user) {
  const rows = db.hub().prepare(`
    SELECT * FROM knowledge_receipts
...
// Re-evaluate stored Terra reviews with the current gates: auto-apply
// organisational fact unlinks, keep only human-decidable questions, drop the
// rest. No new model call. Used to repair the queue that the first corpus pass
// dumped onto /crm/questions.
async function settleOpenAttributionReviews(user = 'douglas', {
  apply = false,
  updateTaskFn = updateTask,
} = {}) {
  const result = {
  ...
  for (const receipt of latestReviewReceipts(user)) {
    if (receipt.status !== 'review') continue;
    const payload = parseJson(receipt.payload, {});
    const stored = payload.reviews || [];
    if (!stored.length) continue;
    const intake = db.hub().prepare('SELECT * FROM meeting_intakes WHERE id = ? AND user = ?').get(receipt.source_id, user);
    if (!intake) continue;
    result.sources += 1;
    result.questions_before += stored.length;
    let packet = buildPacket(user, intake);
    const applied = [];
    const kept = [];
    const dropped = [];
    ...
    for (const review of stored) {
      const decision = reviewAsDecision(review);
      const item = { ...review, ...decision };
      if (!packet.targets.some(target => target.key === decision.target_key)
        && !(decision.target_key || '').startsWith('task:')) {
        dropped.push({ ...item, drop_reason: 'target_gone' });
        continue;
      }
      const gate = automaticDecision(decision, packet);
      if (gate.automatic) {
        if (!apply) {
          applied.push({ target_key: decision.target_key, verdict: decision.verdict, dry_run: true });
          continue;
        }
        try {
          const one = applyDecision(user, intake.id, decision);
          applied.push({ ...one, decision });
          if (one.task_id) affectedTasks.set(one.task_id, one.outcome_id || null);
          packet = buildPacket(user, db.hub().prepare('SELECT * FROM meeting_intakes WHERE id = ? AND user = ?').get(intake.id, user));
        } catch (err) {
          result.errors += 1;
          const failed = reviewItem(decision, `apply_failed:${err.message}`, intake);
          if (isAskableAttributionReview(failed)) kept.push(failed);
          else dropped.push({ ...failed, drop_reason: failed.gate_reason });
        }
        continue;
      }
      const queued = reviewItem(decision, gate.reason, intake);
      queued.owners = decision.owners || review.owners || [];
      queued.task = decision.task || review.task || queued.task;
      if (isAskableAttributionReview(queued)) kept.push(queued);
      else dropped.push({ ...queued, drop_reason: gate.reason });
    }
    const taskUpdates = [];
    if (apply) {
      for (const [taskId, outcomeId] of affectedTasks) {
        try {
          const taskResult = await reconcileTaskAssignee(user, taskId, { updateTaskFn, outcomeId });
          taskUpdates.push(taskResult);
          if (taskResult.review) {
            const conflict = {
              review_key: shortHash([ATTRIBUTION_RECONCILIATION_VERSION, intake.id, 'task', taskId, taskResult.reason]),
              target_key: `task:${taskId}`,
              target_kind: 'task_owner_conflict',
              verdict: 'needs_review',
              current_contact: taskResult.current_assignee ? { name: taskResult.current_assignee } : null,
              proposed_contact_id: null,
              proposed_contact_name: null,
              written_name: taskResult.current_assignee || 'Douglas/unassigned',
              confidence: 0,
              evidence_quote: '',
              reason: taskResult.reason,
              gate_reason: taskResult.reason,
              context: taskResult.title,
              owners: taskResult.owners || [],
              task: { id: taskId, title: taskResult.title, current_assignee: taskResult.current_assignee },
            };
            if (isAskableAttributionReview(conflict) && !kept.some(item => item.target_key === conflict.target_key)) {
              kept.push(conflict);
            }
          }
        } catch (err) {
    ...
    result.questions_after += kept.length;
    if (!apply) continue;
    packet = buildPacket(user, db.hub().prepare('SELECT * FROM meeting_intakes WHERE id = ? AND user = ?').get(intake.id, user));
    const evidence = resolveSourceEvidence(user, 'meeting_intake', intake);
    writeReceipt(user, 'meeting_intake', intake.id, ATTRIBUTION_RECONCILIATION_STAGE, {
      status: kept.length ? 'review' : 'done',
      summary: `${applied.length} stored attribution correction(s) applied; ${kept.length} still need a person decision; ${dropped.length} dropped as not human-answerable.`,
      payload: {
        ...
        dropped: dropped.map(item => ({
          ...
          drop_reason: item.drop_reason || item.gate_reason,
        })),
        task_updates: taskUpdates,
        state_hash_before: payload.state_hash_before || null,
        state_hash_after: stateHash(packet),
        follow_up_required: false,
        derived_by: 'attribution_review_settlement',
      },
      modelKey: 'attribution_reconciliation',
