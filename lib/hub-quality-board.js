...
function buildLinkedInQualityReview({ post = {}, receipts = {} } = {}) {
  ...
  const checks = [
    ...
    check(
      ...
      stats.chars >= 400 && overallScore >= 3.5 ? 'pass' : 'fail',
      `${stats.chars} chars; overall_score=${overallScore || 'missing'}`,
      { words: stats.words, overall_score: overallScore || null },
      true,
    ),
    check(
      'Douglas Voice Guardian',
...
function writeLinkedInQualityReview({ user = 'douglas', postId, post = null } = {}) {
  ...
  const receipt = buildLinkedInQualityReview({ post: row, receipts: stageReceiptMap(user, postId) });
