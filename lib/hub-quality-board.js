...
function intakeClarificationIssues(intakes = [], contacts = []) {
  ...
  const { assessTranscriptIntelligibility } = require('./transcript-quality');
  ...
  for (const intake of intakes) {
    if (intake.status === 'error') continue;
    if (assessTranscriptIntelligibility(intake.transcript).unintelligible) continue;
...
function runCrmQualityBoards(user = 'douglas') {
  ...
  const clarifications = (() => {
    try { return require('./crm-clarifications'); } catch (_) { return null; }
  })();
  const answered = clarifications?.answeredKeys(user) || null;
  const attributionQuestions = clarifications
    ? clarifications.openClarifications(user).filter(item => item.kind === 'attribution_correction')
    : [];
  const meetingReview = buildMeetingIntakeQualityReview({ intakes, contacts, answered });
  meetingReview.checks.push(check(
    'Whole-source Attribution Reviewer',
    'attribution_corrections_need_confirmation',
    attributionQuestions.length ? 'warn' : 'pass',
    attributionQuestions.length
      ? `${attributionQuestions.length} whole-source attribution correction(s) need confirmation`
      : 'No whole-source attribution corrections need confirmation',
    { clarifications: attributionQuestions },
  ));
  meetingReview.clarification_requests.push(...attributionQuestions);
  meetingReview.verdict = verdictFor(meetingReview.checks);
  const receipts = [
    writeCrmSectionQualityReceipt(user, 'crm:people', buildCrmPeopleQualityReview({ contacts, projectEvidence, answered })).receipt,
    writeCrmSectionQualityReceipt(user, 'crm:meeting_intake', meetingReview).receipt,
