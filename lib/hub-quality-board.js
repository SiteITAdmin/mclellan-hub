...
function likelyAudioNameVariant(a, b) {
  ...
  // at all, e.g. a bare "Alan"). Two contacts with clearly different surnames —
  // "Alan Garland" vs "Alec Hirst" — are separate people who merely share a
  // project, not one person the audio spelled two ways. Without this guard the
  // near-name heuristic fired nightly on real colleagues and got projected into
  // a "possible duplicate contact" task. Only treat as an audio variant when the
  // surnames match or at least one side has no surname to compare.
...
function intakeClarificationIssues(intakes = [], contacts = []) {
  ...
  for (const intake of intakes) {
    ...
    if (speakerReview?.speakers?.length) {
      issues.push({
        intake_id: intake.id,
        title: intake.title,
...
function taskActionIssues({ tasks = [], intakes = [] } = {}) {
...
function buildCrmPeopleQualityReview({ contacts = [], projectEvidence = [] } = {}) {
...
function buildMeetingIntakeQualityReview({ intakes = [], contacts = [] } = {}) {
  ...
  const clarifications = intakeClarificationIssues(intakes, contacts);
  const checks = [
    check(
...
function buildTaskActionQualityReview({ tasks = [], intakes = [] } = {}) {
  const issues = taskActionIssues({ tasks, intakes });
  const checks = [
    check(
...
function runCrmQualityBoards(user = 'douglas') {
  ...
  const receipts = [
    writeCrmSectionQualityReceipt(user, 'crm:people', buildCrmPeopleQualityReview({ contacts, projectEvidence })).receipt,
    writeCrmSectionQualityReceipt(user, 'crm:meeting_intake', buildMeetingIntakeQualityReview({ intakes, contacts })).receipt,
    writeCrmSectionQualityReceipt(user, 'crm:tasks', buildTaskActionQualityReview({ tasks, intakes })).receipt,
