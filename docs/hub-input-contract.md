...
| Recurring meeting attribution reconciliation | Existing processed `meeting_intakes` transcript plus its compiled extraction, atoms, action outcomes, and linked task state | Whole-source GPT-5.6 Terra synthesis is change-driven. Exact-quote-backed missing links and organisational fact unlinks may be learned; high-confidence wrong person-links may be corrected; only person-decidable leftovers become `/crm/questions` declarations. A human attribution correction is local evidence, never a durable person alias. | Corrected frozen extraction/atoms/outcomes, `knowledge_receipts`, and (through the existing action outbox/effect gate only) the assignee tag on an already-projected open Google Task. |
...
| Google Tasks/manual tasks | Local `google_tasks` mirror and remote Google Tasks | Task sync preserves state. New tasks created through the effect gate default to a Google-visible 30-minute effort tag. `task-router` queries compiled atoms and attaches only a high-score, clearly separated entity; ambiguous/low-confidence cases receive a review receipt and remain unlinked. `/crm/planner` derives unscheduled work only from open tasks explicitly tagged `[planner: work]` or `[planner: personal]` and existing task-planner event receipts/cache; it does not create a planner task copy. Completed tasks are evidence of state, not requests for replacement work. | `/crm/tasks`, `/crm/planner`, reminders, briefings, action/source views. Routing and calendar placement are operational hints, never relationship truth. |
...
| WhatsApp/Hermes and Apple Messages | `messaging_messages` with the complete text body, parseable provider envelope, platform/chat/sender/direction, and full external id. WhatsApp begins at bridge connection; the read-only Mac Messages worker begins at the current `chat.db` row and captures both received and sent text, never silently scraping history. Reactions, system events, and attachment-only bubbles are skipped. | Capture resolves only a unique canonical name/alias/address before any model and stores the contact anchor plus resolution method in the envelope; conflicting or near-name signals stay ambiguous. CRM knowledge engine then performs triage, synthesis, and action projection over deterministic chunks. Same-chat neighbours are prompt context only; a later turn may resolve an earlier same-chat task. Automatic selection holds a new message for five minutes so a reply can land. Historical backfill is evidence-only and blocks current task projection and live-task resolution. | Source-backed atoms/actions, tasks after projection, completed tasks when a later reply answers the earlier ask, Ask/briefings, receipts, and per-ingester/worker health in daily INGEST. |
| Calendar events | Google Calendar and local `meetings` cache | Inbound sync supplies context. A CRM action with a specific date and time may create an event through the stable action outbox; an ambiguous or unreconciled pending event fails closed and stays visible. Explicit task planning creates or patches one event carrying private `hubTaskId` provenance, reconciles before retry, and records `calendar_planner_effect`; unscheduling removes only that event. Saved work/evening/weekend preferences constrain both manual and automatic task placement. Provider exactly-once delivery is not assumed. | Calendar, `/crm/planner`, meeting pages, and action receipts. |
