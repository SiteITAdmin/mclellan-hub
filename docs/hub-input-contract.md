...
| Gmail label-filed mail (any category) | `email_summaries` for classification labels; `intel_documents`/`intel_items` for intelligence labels; Gmail label state | `email_process` label-driven poll (`pollFiledLabels`): category-blind fetch by label id from a `_gmail_label_poll_last_check_ts` cursor. `Resources/Newsletters`/`Resources/Research` → `extractTopicsFromEmail`; all other canonical labels → classifier carrying the filed label as authoritative (never re-filed). Dedup: intel keys on message id, classification skips already-summarised ids | `intel_items` (briefing) or `knowledge_receipts`/atoms/tasks + label learning, per label | Intelligence briefing gets Promotions-tabbed newsletters it previously missed; filed senders like MasterClass/Maven (under `Resources/Learning`) surface in the Hub and become learnable regardless of Gmail category |
| Gmail promotions email | `opportunity_signals` | `email_process` promotions lane: bounded opportunity appraisal | `opportunity_signals`; synthesised offer suggestions | Opportunity suggestions, briefings |
...
| Sealed historical messaging archive | `messaging_archive_buckets` + `messaging_archive_messages`; bucket is database-sealed after one import | Project reports read only messages whose original timestamps fall inside the requested window; `messaging_archive_release` releases at most 10 per Dublin day into the existing `messaging_message` CRM path with historical-action blocking | Source-backed `knowledge_receipts` and `knowledge_atoms`; no direct current task projection from historical evidence | Dad project reports, compiled project/contact knowledge, `/crm/knowledge`, system job visibility |
