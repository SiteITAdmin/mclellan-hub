...
| Gmail promotions email | `opportunity_signals` for all; `email_summaries` + Gmail label state + `processing_failures` for the curated subset | `email_process` promotions lane: opportunity appraisal for all; `selectKnownPromotionEmails` re-admits taxonomy-rule senders and Gmail-filter-labelled non-newsletter mail (e.g. MasterClass, Maven under `Resources/Learning`) to the full classifier + label learning | `opportunity_signals`; for the curated subset, same compiled layer as received email (`knowledge_receipts`, atoms, tasks, label learning) | Opportunity suggestions; curated senders also surface in CRM/briefings/digest and become learnable. Generic newsletter bulk stays appraisal-only |
