...
| Outlook work email (Microsoft 365) | Designated mail folders only (`_outlook_sync_folders`; Outlook rules act as intake filter); `inbound_email_records` with `source='outlook'`; compatibility `email_summaries` rows (`outlook:<id>`); `processing_failures` on errors | `outlook_email_process` job; classifier; `crm_knowledge_engine` (reads `email_summaries`); `embed_backfill`; `synthesis_run` | `knowledge_receipts`, `knowledge_atoms`, `google_tasks` via action projection | `/admin/microsoft` status page, CRM/contact/project pages, tasks, digests, system report |
| Outlook calendar (Microsoft 365) | Graph calendarView via OAuth; synced `meetings` with `source='outlook_calendar'` | `outlook_calendar_sync` job (30 min); attendee→contact matching; debrief context; work brief; interest synthesis | Meeting records, interest atoms/cache | Morning/work brief, meeting prep, CRM meetings, `/admin/microsoft`, interest radar |
...
| Calendar events | Google Calendar via OAuth; synced `meetings` where applicable | `syncCalendarMeetings`; debrief context; work brief; interest synthesis | Meeting records, interest atoms/cache | Morning/work brief, meeting prep, CRM meetings, interest radar |
...
| LinkedIn/content generation | `linkedin_posts`, content research suggestions, cadence policy in `crm_context` | `linkedin-pipeline`; content research; publish capture | Published-post knowledge via `knowledge-format`; wiki note on publish | Content dashboard, queue, published archive, portfolio/public knowledge bundle |
...
| Model/admin settings | `model_config`, `crm_context`, `request_logs`, `test_runs` | Admin model slots; prompt override system; style profile run | Model/prompt configuration and usage evidence | `/admin/models`, `/admin/reports`, prompt shaping, cost/quality review |
| System jobs and failures | `system_jobs`, `processing_failures`, logs | `processJobs`; retry/refetch paths; system report | Health state and failure counts | `/admin/jobs`, daily system report, diagnostic follow-up |
