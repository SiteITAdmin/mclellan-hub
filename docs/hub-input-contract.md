...
| Google Tasks/manual tasks | Local `google_tasks` mirror and remote Google Tasks | Task sync preserves state. `task-router` queries compiled atoms and attaches only a high-score, clearly separated entity; ambiguous/low-confidence cases receive a review receipt and remain unlinked. Completed tasks are evidence of state, not requests for replacement work. | `/crm/tasks`, reminders, briefings, action/source views. Routing is an operational hint, never relationship truth. |
...
| Calendar events | Google Calendar and local `meetings` cache | Inbound sync supplies context. A CRM action with a specific date and time may create an event through the stable action outbox; an ambiguous or unreconciled pending event fails closed and stays visible. Provider exactly-once delivery is not assumed. | Calendar, meeting pages, work brief, and action receipts. |
